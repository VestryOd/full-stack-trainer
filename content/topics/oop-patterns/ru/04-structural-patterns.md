...
Структурные паттерны описывают **как компоновать классы и объекты в более крупные структуры**, сохраняя гибкость. Порождающие паттерны отвечают на вопрос, как создать объект. Структурные отвечают на другой вопрос. Как соединить объекты так, чтобы интерфейс системы оставался простым, а внутренняя сложность — инкапсулированной?
...
Поведенческие → "как объекты взаимодействуют
                 и распределяют ответственность"
```
...
Bridge — структурный паттерн.
  Реализация выбирается при создании объекта и обычно
  не меняется. Это архитектурное решение о том,
  как объект устроен.

Strategy — поведенческий паттерн.
  Алгоритм можно подменять во время выполнения. Это решение
  о том, как объект ведёт себя в конкретный момент.

Граница размытая: часто одно и то же делают и так, и так.
Ключевой вопрос: постоянная структура или сменяемое поведение?
```
...
Composite нужен всякий раз, когда структура данных рекурсивна: файловая система, дерево DOM (document object model — объектная модель документа), организационная иерархия, категории товаров.
...
```txt
Controller (GRASP) — тонкий слой между HTTP и бизнес-логикой.
  Своей логики не содержит.

Facade — содержит логику оркестрации между подсистемами
  и скрывает их от клиента.

Часто в NestJS: Controller → Facade → отдельные Services.
Facade — это не Controller. Это слой между контроллером
и доменом.
```
...
```txt
Виды Proxy:
  Virtual Proxy    — ленивая инициализация (объект создаётся
                     при первом обращении)
  Protection Proxy — контроль доступа (проверка прав до вызова)
  Caching Proxy    — кеширует результаты дорогих операций
  Remote Proxy     — представляет удалённый объект
                     (удалённый вызов процедур, микросервисы)
  Logging Proxy    — логирует обращения (как Decorator,
                     но через Proxy)
```
...
```txt
Adapter
  Переводит чужой интерфейс в свой.
  Пример: клиент Sendgrid, обёрнутый в EmailService.

Bridge
  Разделяет абстракцию и реализацию.
  Пример: Компонент × Тема.

Composite
  Дерево объектов за единым интерфейсом.
  Пример: React-дерево, файловая система.

Decorator
  Оборачивает объект, добавляя поведение.
  Пример: компонент высшего порядка, HTTP-клиент
  с повтором запроса и логированием.

Facade
  Упрощённый интерфейс к подсистеме.
  Пример: OrderFacade, слой backend-for-frontend.

Flyweight
  Разделяет состояние, общее для многих объектов.
  Пример: пул строк, пул соединений.

Proxy
  Суррогат, добавляющий контроль вокруг настоящего объекта.
  Пример: кеш, проверка прав, встроенный объект Proxy.
```
...
- **Composite только для файловых систем** — это самый известный пример. Composite встречается везде, где есть рекурсивная структура: DOM, AST, организационные иерархии, меню и подменю, категории с подкатегориями. AST расшифровывается как abstract syntax tree — дерево разобранного кода.
...
- **Не знать, что Express middleware — это структурный паттерн** — цепочку middleware через `app.use()` можно и нужно описывать через паттерны. Decorator добавляет поведение, а Chain of Responsibility передаёт управление дальше через `next()`. Это показывает понимание паттернов в реальном коде, а не только в учебниках.
