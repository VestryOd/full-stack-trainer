...
Симптомы кода, нарушающего SOLID:
  ...
  - "Чтобы добавить новый тип пользователя, надо поменять 12 файлов"
  - "Я не могу написать тест для класса без поднятия базы данных"
  - "Этот класс нельзя переиспользовать — он тащит всё приложение"

Каждый симптом — нарушение одного или нескольких принципов SOLID.
```

Важное предупреждение с самого начала: SOLID — это **инструмент**, а не религия. В конце каждого принципа будет раздел "когда строгое соблюдение вредит". Слепое следование принципам создаёт переусложнённый код, и поддерживать его так же тяжело, как код вообще без принципов.
...
В NestJS нарушение SRP — это сервис, который делает HTTP-запросы, пишет в базу данных, форматирует ответ и отправляет метрики. Каждый из этих слоёв должен быть отдельным инжектируемым провайдером. Тест `UserService` не должен требовать поднятого почтового сервера.

### Когда строгое SRP вредит

Если приложение маленькое и ответственности **никогда** не меняются независимо — искусственное дробление только нагружает голову и не даёт реальной пользы. "Регистрация пользователя" в минимально жизнеспособном продукте из 200 строк кода не требует четырёх классов.
...
Преждевременная абстракция хуже, чем её отсутствие. Если у вас два типа каналов и третьего никогда не будет — interface + switch честнее, чем сложная система регистрации плагинов. **YAGNI (you aren't gonna need it — «вам это не понадобится») до второй-третьей вариации, OCP после.**
...
class ReadOnlyRepository<T> extends Repository<T> {
  async save(entity: T): Promise<void> {
    // ← нарушение LSP: код с Repository не ожидает этого
...
Слишком мелкие интерфейсы приходится собирать обратно длинной цепочкой. Если у вас 8 интерфейсов по одному методу, это тоже проблема. Код становится трудно читать, а пересечения типов в TypeScript (`A & B & C & D`) теряют смысл. Ориентир: **интерфейс должен соответствовать одной "роли"** (Readable, Writable, Searchable), а не одному методу.
...
Практически: **бизнес-логика не должна знать, какая база данных, какой HTTP-клиент, какой email-провайдер используется**. Она работает с интерфейсами. Конкретные реализации подставляются снаружи (Dependency Injection).
...
NestJS строит всю систему вокруг DIP: `@Injectable()` провайдеры регистрируются в модуле, а классы получают зависимости через конструктор. Токен `IUserRepository` в `provide` и `useClass: PostgresUserRepository` — это DIP в действии.

Именно поэтому тестирование NestJS-сервисов через `Test.createTestingModule` настолько удобно: конкретные реализации заменяются моками без изменения тестируемого кода.

### Когда строгое DIP вредит

Для небольших утилит (вспомогательные функции, форматтеры, валидаторы) — внедрение абстракций ради абстракций только раздувает код. Если функция `formatPrice(amount: number): string` никогда не поменяет реализацию и не требует мокирования в тестах — ей не нужен интерфейс. DIP критичен для **зависимостей с побочными эффектами** (ввод-вывод, сеть, время) и для **вариативного поведения** (несколько email-провайдеров).
...
OCP → расширение классами → требует ISP (не "жирные" интерфейсы)
ISP → маленькие интерфейсы → упрощает DIP (инжектируем что нужно)
DIP → зависимость от абстракций → проще тесты (подмена реализаций)
LSP → правильная иерархия → полиморфизм предсказуем (нет сюрпризов)
```

Нарушение одного принципа часто тянет за собой другие. Класс с тремя ответственностями (SRP) обычно ещё и жёстко привязан к конкретным зависимостям (DIP). А добавление нового поведения требует править сам класс, и это уже нарушение OCP.
...
- **LSP только как "прямоугольник и квадрат"** — это каноничный пример. На практике LSP чаще нарушается через `throw new Error('Not implemented')` в методах интерфейса. Умение распознать эту форму нарушения важнее знания канонического примера.

- **ISP и DIP путают** — это разные оси. ISP про интерфейсы со стороны клиента ("не давать лишних методов"). DIP про направление зависимости ("зависеть от абстракции, не от конкретики").
