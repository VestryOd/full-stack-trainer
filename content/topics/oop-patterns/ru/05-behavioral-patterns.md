...
Поведенческие паттерны — самая большая группа в каталоге GoF (Gang of Four — «банда четырёх»). Они решают три вопроса:

- Как избежать жёсткой связности между отправителем и получателем сообщения?
- Как инкапсулировать алгоритм?
- Как реагировать на изменение состояния?
...
Observer — один из самых фундаментальных паттернов во фронтенд и Node.js разработке. EventEmitter, события DOM (document object model — объектная модель документа), состояние React, RxJS — всё это реализации Observer.
...
Проблемы Observer в крупных приложениях:
  - "Цепочки реакций": событие A → событие B → событие C → ...
    трудно отследить, что происходит при конкретном действии
  - Утечки памяти: подписчик живёт дольше Observable —
    никто не отписался, и память не освобождается
...
Chain of Responsibility:
  ...
  - Следующий обработчик вызывается, только если текущий
    решил передать управление дальше
  - Цель: найти обработчик среди кандидатов

Express middleware — оба паттерна одновременно:
  - cors(), helmet() → Decorator (добавляют поведение
    и всегда вызывают next)
  - authenticate() → Chain of Responsibility (может вернуть 401
    и не вызвать next)
```
...
Observer
  ...
  Где встречается: EventEmitter, стор Redux.

Strategy
  Взаимозаменяемый алгоритм.
  Где встречается: стратегии Passport.js, Array.sort.

Command
  Действие как объект.
  Где встречается: экшены Redux, задачи BullMQ.

Iterator
  Обход без раскрытия структуры.
  Где встречается: генераторы, потоки, for-await-of.

Template Method
  Скелет алгоритма с хуками.
  Где встречается: DataImporter, жизненный цикл React.

State
  Поведение зависит от текущего состояния.
  Где встречается: XState, статусы заказа.

Mediator
  Посредник вместо прямых связей.
  Где встречается: стор Redux, шина событий, комната Socket.IO.

Chain of Responsibility
  Цепочка обработчиков, которую можно прервать.
  Где встречается: middleware Express, guards в NestJS.
```
