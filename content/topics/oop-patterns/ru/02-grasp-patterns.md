...
GRASP сформулировал Крэйг Ларман в книге "Applying UML and Patterns" (1997). UML (unified modeling language — язык графических диаграмм классов и последовательностей) — стандартная нотация для схем проектирования. GoF (Gang of Four — «банда четырёх») — это прозвище четырёх авторов книги 1994 года, откуда пошли Singleton, Observer и остальные.

В отличие от паттернов GoF, GRASP менее известен на собеседованиях. Именно поэтому знание GRASP отличает кандидата, который думает об архитектуре, от кандидата, который просто выучил названия паттернов.
...
Это самый фундаментальный принцип GRASP и одновременно самый часто нарушаемый. У нарушения всегда одна и та же форма: сервис берёт данные объекта и считает что-то по ним снаружи. А вычисление надо было делегировать самому объекту.
...
SOLID — это акроним пяти принципов проектирования: единственная ответственность, открытость/закрытость, подстановка Лисков, разделение интерфейсов и инверсия зависимостей. Information Expert — это практическая реализация SRP (single responsibility principle — принцип единственной ответственности) на уровне данных.

У класса, который владеет данными, здесь одна причина изменяться: меняются сами данные или правила их обработки. Если логика вынесена наружу — появляется дополнительная причина изменений у внешнего класса.
...
> Класс B должен создавать экземпляры класса A, если B содержит A,
> агрегирует A, инициализирует A или владеет данными для создания A.
...
Creator связан с SRP: если класс создаёт объекты, которые ему "не принадлежат", это добавляет лишнюю ответственность. Creator также перекликается с паттерном Factory, который разобран в статье [Порождающие паттерны](./03-creational-patterns.md). Factory нужна тогда, когда условие "содержит или использует" не выполняется и нужна выделенная точка создания.
...
Controller — это посредник между слоем интерфейса или API и бизнес-логикой. Он не содержит логики сам — он делегирует её доменным объектам или сервисам.
...
"Тонкий контроллер" — прямое следствие двух принципов. SRP говорит, что контроллер меняется только при изменении HTTP-интерфейса. DIP (dependency inversion principle — принцип инверсии зависимостей) говорит, что контроллер зависит от абстракции сервиса, а не от конкретной логики. В NestJS это выражается в том, что контроллер инжектирует сервис, а не создаёт его.
...
Виды связанности, от менее к более проблемной:
  ...
  Data coupling (хорошо)
    Классы обмениваются только простыми данными.

  Stamp coupling (нейтрально)
    Передают объект целиком, хотя нужно одно поле.

  Control coupling (плохо)
    Один класс управляет поведением другого через флаг.

  Content coupling (очень плохо)
    Один класс читает или пишет внутренние данные другого.
```
...
Признаки низкой связности (low cohesion):
  - Класс называется XxxManager, XxxHelper, XxxUtil — общие имена
...
Это один из фундаментальных принципов ООП (объектно-ориентированного программирования). GRASP формулирует его как рекомендацию по распределению ответственности. **Ответственность за вариативное поведение должна лежать на самом объекте**, а не на внешнем коде, который его проверяет.
...
Polymorphism — это механизм реализации OCP (open/closed principle — принцип открытости/закрытости) на практике. "Открыт для расширения, закрыт для модификации" достигается именно через полиморфизм: новое поведение = новый класс, реализующий интерфейс.

---

## 7. Pure Fabrication — "иногда нужен класс без аналога в реальном мире"

> Если ни один доменный класс не подходит на роль владельца ответственности,
> создайте искусственный класс-сервис. Это тот случай, когда назначение
> ответственности доменному классу нарушило бы High Cohesion или Low Coupling.

Pure Fabrication даёт разрешение создать класс без доменного смысла. Большинство сервисов в слоистой архитектуре — это Pure Fabrications: Repository, Mapper, Formatter, Gateway. В реальном мире не существует `UserRepository` — это техническая абстракция.
...
Иногда **все** классы в проекте — Pure Fabrications: сервисы, менеджеры и хелперы, и ни одного доменного объекта. Это называется Anemic Domain Model — процедурный код, замаскированный под ООП. Доменные классы — `Order`, `User`, `Product` — должны содержать логику, специфичную для предметной области (расчёт скидок, валидация инвариантов). Pure Fabrication дополняет их, а не заменяет.
...
Pure Fabrication   → SRP + ISP (выделяем инфраструктурную часть)
```
...
- **Не различать High Cohesion и SRP** — оба принципа про "делай одно", но с разного угла. SRP про причину изменений: какой внешний актор её потребует. High Cohesion про внутреннюю связность методов: используют ли они общие данные и знания. Можно нарушить один, не нарушая другой.
