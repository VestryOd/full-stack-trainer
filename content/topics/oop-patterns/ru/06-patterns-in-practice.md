...
Класс, который знает слишком много и делает слишком много. Нарушает сразу три ориентира: SRP (single responsibility principle — принцип единственной ответственности), High Cohesion и Low Coupling.
...
### Golden Hammer — "если в руках молоток, всё выглядит как гвоздь"

```txt
Проявления Golden Hammer:
  - "Мы используем Redux для всего состояния" → даже для
    локального состояния одного компонента
  - "Мы используем микросервисы" → для приложения,
    которое пишут два человека
...
Паттерн — ответ на вопрос "почему?", а не на "что взять дальше?"
```
...
Один тип уведомления
  Соблазн: Observer
  Вердикт: нет — достаточно вызова функции.

Два типа, и третьего никогда не будет
  Соблазн: Strategy
  Вердикт: нет — if/else честнее.

Простое создание, чтение, правка и удаление без бизнес-логики
  Соблазн: Repository
  Вердикт: возможно нет — запросы прямо в сервисе достаточны.

Один шаг "алгоритма"
  Соблазн: Template Method
  Вердикт: нет — достаточно метода.

Простой конфиг из 2-3 полей
  Соблазн: Builder
  Вердикт: нет — литерал объекта понятнее.
```

**Правило трёх:** паттерн оправдан, когда есть (или почти наверняка будет) три или более различных случая применения. До этого момента действует YAGNI — you aren't gonna need it, «вам это не понадобится».

---

## Типичные ошибки на интервью

- **"Я использую паттерны везде"** — красный флаг. Правильный ответ включает понимание компромиссов и YAGNI. Паттерн — инструмент под конкретную задачу, а не стиль кода по умолчанию.
...
- **"Anemic Domain Model — это нормально"** — смотря для чего. Для CRUD-сервиса (create, read, update, delete — создание, чтение, правка, удаление) без бизнес-логики это приемлемо. Для доменно-насыщенных систем — финансы, торговля, логистика — это техдолг, который нарастает с каждой итерацией.
...
- **Repository как "просто обёртка над ORM"** — ORM (object-relational mapper — объектно-реляционное отображение) превращает строки таблиц в объекты. Repository решает другую задачу: изоляцию бизнес-логики от деталей хранилища. Если методы Repository повторяют API ORM один-в-один без доменной семантики, это действительно просто обёртка, и паттерн использован неосмысленно.

- **God Object → немедленный рефакторинг** — в legacy-коде God Object — норма жизни. Переписывать всё сразу рискованно. Вытесняйте God Object постепенно: выделяйте новые сервисы по принципу "открытости-закрытости" и оставьте God Object фасадом на переходный период.

- **Не различать паттерны и принципы** — принципы дают рекомендации о том, как думать об архитектуре. Это SOLID (пять принципов проектирования) и GRASP (general responsibility assignment software patterns — принципы распределения ответственности). Паттерны GoF (Gang of Four) — конкретные решения конкретных задач. Паттерн может реализовывать несколько принципов. Принцип может реализовываться через несколько паттернов.
