...
Порождающие паттерны решают один вопрос: **как создавать объекты так, чтобы код, использующий объект, не зависел от деталей его создания**.

Это прямое следствие DIP (dependency inversion principle — принцип инверсии зависимостей). DIP — это буква D в SOLID: пять принципов проектирования, которым посвящена [статья 01](./01-solid-principles.md) этого топика. Если `new ConcreteClass(arg1, arg2, arg3)` разбросан по всему коду, смена реализации требует правки во всех местах.
...
С фабрикой или контейнером внедрения зависимостей:
...
Factory Method — это не обязательно метод в подклассе, как описано в книге GoF (Gang of Four — «банда четырёх»). В TypeScript/JavaScript он чаще встречается как **функция или статический метод**, скрывающий детали создания.
...
Ключевое слово — **семейство**. Он нужен, когда несколько объектов должны быть согласованы между собой: одна тема интерфейса, одна платёжная система, один облачный провайдер.

### Пример: компоненты интерфейса для разных тем
...
```txt
Нужен:
  - Создание объекта требует сложной инициализации: компиляция
    регулярного выражения, парсинг схемы, установка соединения
  - Нужно много похожих объектов с небольшими различиями
  - Иммутабельные обновления (Redux, Immer)

Не нужен (достаточно обычного new):
...
```txt
Singleton
  Проблема: один экземпляр на всё приложение
  Где нужен: логгер, конфиг, кеш (но лучше внедрять зависимость)

Factory Method
  Проблема: создать один объект с вариативной реализацией
  Где нужен: логгер по окружению, компонент по типу

Abstract Factory
  Проблема: создать семейство согласованных объектов
  Где нужен: тема интерфейса, облачный провайдер

Builder
  Проблема: пошаговое построение сложного объекта
  Где нужен: SQL-запрос, HTTP-запрос, конфиг из >4 полей

Prototype
  Проблема: клонировать объект как основу для вариаций
  Где нужен: дорогая инициализация, тест-фикстуры, иммутабельность
```

## Типичные ошибки на интервью

- **"Singleton — хороший паттерн для общих ресурсов"** — без упоминания проблем: скрытые зависимости, глобальное состояние, проблемы с тестируемостью. Правильный ответ добавляет: лучше DI (dependency injection — внедрение зависимостей) с одним экземпляром в контейнере.
...
- **"Prototype — это прототипная цепочка JavaScript"** — это разные вещи. Паттерн Prototype — про клонирование объектов. Прототипная цепочка JS — про делегирование поиска свойств по цепочке `__proto__`. На собеседовании разграничьте эти понятия явно.
...
- **Builder без валидации в `build()`** — полноценный Builder должен проверять обязательные параметры и инварианты в методе `build()`, а не в каждом сеттере. Это часто упускают при реализации на интервью.
