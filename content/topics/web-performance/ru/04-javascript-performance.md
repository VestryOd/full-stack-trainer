...
```txt
200 KB изображения:
  Скачать → Декодировать → Отрисовать   (вне main thread)
...
Изображение обрабатывается вне главного потока. С JavaScript не так: парсинг, компиляция и выполнение идут прямо на нём. **Одно только выполнение может занять сотни миллисекунд, и всё это время main thread заблокирован** — никакой реакции на клики, никаких анимаций, ничего.
...
Long Task — любая задача на main thread продолжительностью **более 50ms**. Именно из Long Tasks складывается TBT (Total Blocking Time — суммарное время блокировки главного потока). Они же портят INP (Interaction to Next Paint — задержку отклика на действие пользователя).
...
Откуда Long Tasks берутся в реальных приложениях:

1. **Гидратация SPA** (single-page application — одностраничное приложение) на React, Vue или Angular. Весь JS-бандл парсится и выполняется при первой загрузке, и на слабом Android это занимает 500ms и больше.
2. **Тяжёлые обработчики событий** — клик, который синхронно фильтрует или сортирует большой массив данных.
3. **Сторонние скрипты** — аналитика, чаты, A/B-тестирование. Часто они вне вашего контроля, но момент их загрузки можно отложить.
4. **Большие перерисовки DOM** (Document Object Model — дерево элементов страницы). React синхронно перерисовывает компонент с тысячами узлов.
5. **`JSON.parse()` больших данных** — JSON на мегабайт парсится примерно 50–100ms на среднем устройстве.
...
`scheduler.yield()` делит задачу во времени, но JS всё равно выполняется на main thread. Часть работы упирается в CPU (центральный процессор), а не в ожидание сети. Такой работе нужен другой инструмент: **Web Worker** выполняет код в отдельном потоке операционной системы, и main thread остаётся полностью свободным.
...
Code splitting — разбивка бандла на части, которые загружаются по требованию. Это главный инструмент снижения TTI (Time to Interactive — время до готовности к взаимодействию) и TBT.
...
Tree shaking — механизм сборщиков (webpack, Rollup, esbuild) для удаления неиспользуемого кода из бандла. Работает он только с ES-модулями. ES — это ECMAScript, стандарт языка JavaScript, а его модули — это синтаксис `import`/`export`.
...
1. **Дубликаты** — одна и та же библиотека попала в сборку несколько раз, из разных версий в `node_modules` или из разных точек входа.
2. **Неожиданно большие зависимости.** Три встречаются чаще всего:
   - `moment.js` на 300KB — заменить на `date-fns`.
   - `lodash` на 70KB — заменить на `lodash-es`, чтобы заработал tree shaking.
   - Полный Ant Design на 1MB — брать только нужные компоненты.
3. **Код, которого в бандле быть не должно.** В клиентский код утекают серверные модули Node.js вроде `fs` и `path`. Туда же попадают тестовые утилиты, данные-заглушки и зависимости для разработки.
...
В панели **Performance** в Chrome DevTools:

1. Нажмите запись, поработайте со страницей, остановите запись.
2. На дорожке main thread красные флажки над задачами отмечают Long Tasks. Нажмите на задачу и откройте `Bottom-up` — там видно, кто виноват. `Script Evaluation` — это парсинг и компиляция JS, а `Parse HTML`, прерванный на `Compile Script`, означает синхронный скрипт.
3. Откройте вкладку `Coverage` (⋮ → More tools → Coverage). После загрузки она показывает, какая доля JS так и не выполнилась. Красные полосы в файле отмечают код, который при загрузке не запускался, и такие файлы — хорошие кандидаты на code splitting.
4. Application → Storage → Clear storage, потом перезагрузите страницу и посмотрите на поведение при первом визите, без кэша V8.

В Lighthouse проверка "Reduce unused JavaScript" перечисляет конкретные файлы и количество неиспользованных байтов в каждом. Это прямой сигнал разбить код или выкинуть зависимость.

## Связь с другими темами

- [Performance Metrics](./02-performance-metrics.md) — из Long Tasks складывается TBT, а code splitting напрямую снижает TTI.
- [Core Web Vitals](./01-core-web-vitals.md) — Long Tasks главный враг INP, а размер бандла гидратации влияет на LCP (Largest Contentful Paint).
- [Resource Loading](./03-resource-loading.md) — динамический `import()` вместе с `prefetch` даёт предзагрузку следующего маршрута.
- [Rendering Performance](./06-rendering-performance.md) — тяжёлые перерисовки DOM создают Long Tasks, а React Concurrent Mode разбивает их автоматически.

## Типичные ошибки на интервью

- **"Tree shaking работает с любым JS"** — только с ES-модулями. CommonJS (`require`) tree shaking невозможен по определению, потому что `require()` — рантайм-вызов. Если библиотека публикует только CJS — формат CommonJS, — tree shaking не поможет.
...
- **"Barrel-файлы удобны и не влияют на производительность"** — влияют. Сборщик может не удалить неиспользуемые экспорты из barrel-файла, если не настроен `sideEffects: false`. В итоге `import { one } from '@/utils'` тянет все 120KB вместо 1KB нужного модуля.
...
- **"Проверил в DevTools на своём MacBook — всё быстро"** — топовый MacBook работает в 5–10 раз быстрее среднего Android. Lighthouse замедляет процессор в 4 раза, чтобы имитировать реальное устройство. Всегда проверяйте на настоящем телефоне или с включённым замедлением.
