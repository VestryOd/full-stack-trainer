...
```txt
401 Unauthorized — не аутентифицирован (название историческое)
...
SameSite=Strict — cookie не отправляется при cross-site совсем
SameSite=Lax    — отправляется при top-level GET (дефолт)
SameSite=None   — всегда отправляется; нужно, если API на другом
                  домене, и требует Secure
...
Redis           → ✅ Быстро, TTL встроен, легко масштабируется
```
...
JWT — это стандарт (RFC 7519, документ из серии request for comments) для передачи данных в виде подписанного JSON. Главное отличие от сессий: **сервер не хранит состояние** — вся информация внутри токена.
...
Три части передаются склеенными точками: header.payload.signature
...
Минусы:
  ...
  ❌ Растут при добавлении claims — каждый запрос несёт payload
  ❌ Сложная ротация ключей
  ❌ Много способов ошибиться (alg:none, не проверять exp)
```
...
Authorization Server — выдаёт токены (Google, GitHub, ваш сервер)
...
Authorization server проверяет: SHA256(code_verifier) == challenge
```
...
OIDC:       "Кто этот пользователь?" → id_token (sub, email, name)
```
...
Для высокозащищённых API — платёжные системы, комплект разработки Amazon Web Services (AWS) — подписывается весь запрос, а не только ключ. Подпись — это hash-based message authentication code (HMAC).
...
├──────────────┼─────────────┼──────────┼────────────┼──────────┤
│ Масштаб      │ ⚠️ Redis    │ ✅ Просто │ ✅ Просто   │ ⚠️ Redis │
├──────────────┼─────────────┼──────────┼────────────┼──────────┤
│ Cross-domain │ ❌ Сложно    │ ✅ Просто │ ✅ Нативно  │ ✅ Просто │
├──────────────┼─────────────┼──────────┼────────────┼──────────┤
│ 3rd-party    │ ❌ Нет       │ ❌ Нет    │ ✅ Для него │ ❌ Нет    │
├──────────────┼─────────────┼──────────┼────────────┼──────────┤
...
- **"JWT хранить в localStorage — удобно"** — опасно. localStorage доступен любому JS на странице (XSS — cross-site scripting — крадёт токен). Access token — в памяти JS (теряется при перезагрузке), refresh token — в HttpOnly cookie (недоступен XSS).
...
- **"bcrypt для API-ключей"** — нет. bcrypt медленный намеренно (для паролей). API-ключи — это длинные случайные строки, для них достаточно дайджеста secure hash algorithm (SHA-256). bcrypt для хранения паролей, SHA-256 для хранения API-ключей.
