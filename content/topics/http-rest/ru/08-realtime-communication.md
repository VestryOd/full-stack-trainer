...
Когда нужно, чтобы клиент получал обновления без явного запроса, есть четыре механизма: polling, long polling, server-sent events (SSE) и WebSockets. Они выглядят похоже, но устроены принципиально по-разному. Разберём каждый по отдельности — потом сравним.
...
Клиент: [GET /updates]  [GET /updates]  [GET /updates]
           ↓               ↓               ↓
Сервер: [200: пусто]    [200: пусто]    [200: данные!]
...
Клиент: [GET /updates] ────── hold ────── [данные!] [GET /updates]
Сервер:                ждёт данных...        ↑       ждёт снова
...
Минусы:
  ...
  ❌ Сложно масштабировать: у каждого инстанса свои waitingClients
     → нужен общий pub/sub (Redis), чтобы найти нужный инстанс
...
- Исторически: Comet, клиенты XMPP (extensible messaging and presence protocol), старые версии WhatsApp Web
...
Сервер: HTTP/1.1 200 OK
        ...
        id: 43\n\n
        ↑ соединение открыто, сервер шлёт события по мере
          их появления
```
...
event: имя-события               ← опционально ("message")
id: 42                           ← опционально, Last-Event-ID
retry: 3000                      ← опционально, мс до реконнекта
...
Плюсы:
  ...
  ✅ Переподключение с Last-Event-ID встроено в браузер
  ...
  ✅ Простое масштабирование (для нескольких инстансов нужен
     только pub/sub)

Минусы:
  ❌ Только сервер → клиент (однонаправленный)
  ❌ Только текст (нет бинарного протокола)
  ❌ EventSource не поддерживает custom заголовки — токен
     нельзя передать как Bearer
...
WebSockets — это **полнодуплексное постоянное соединение** поверх TCP (RFC 6455) — документ из серии request for comments. Клиент и сервер могут отправлять сообщения друг другу в любой момент.
...
`Sec-WebSocket-Accept = base64(SHA1(key + "258EAFA5-E914-47DA-95CA-C5AB0DC85B11"))` — защита от случайных HTTP-серверов, принявших запрос WS (WebSocket).
...
Masking: все фреймы от клиента маскируются — защита от
...
Плюсы:
  ...
  ✅ Низкий overhead после handshake: фреймы без HTTP-заголовков
...
Минусы:
  ❌ Stateful: инстанс помнит своих клиентов → сложнее масштаб
  ❌ Нет автоматического переподключения, в отличие от SSE
...
├─────────────────┼──────────┼───────────┼───────────┼───────────┤
│ Переподключение │ клиент   │ клиент    │ браузер ✅ │ ❌ нет     │
├─────────────────┼──────────┼───────────┼───────────┼───────────┤
│ Через прокси    │ ✅ всегда │ ✅ всегда  │ ✅ всегда  │ ⚠️ иногда │
├─────────────────┼──────────┼───────────┼───────────┼───────────┤
│ Бинарные        │ ❌        │ ❌         │ ❌         │ ✅         │
├─────────────────┼──────────┼───────────┼───────────┼───────────┤
│ Масштаб         │ ✅ просто │ ⚠️ Redis  │ ⚠️ Redis  │ ❌ сложно  │
├─────────────────┼──────────┼───────────┼───────────┼───────────┤
│ Сложность       │ ⭐        │ ⭐⭐        │ ⭐⭐        │ ⭐⭐⭐⭐      │
├─────────────────┼──────────┼───────────┼───────────┼───────────┤
│ HTTP/2          │ ✅        │ ✅         │ ✅✅        │ ❌ свой    │
...
```txt
Начните с вопроса: нужен ли push от клиента к серверу
в реальном времени?

НЕ нужен (только сервер → клиент):
  Используйте SSE.
  Примеры: уведомления, лента новостей, прогресс задачи,
...
WebSockets оправданы только когда реально нужен двунаправленный
real-time push.
Масштабировать и реализовать WebSockets заметно сложнее SSE.
```
...
Совместный редактор (Google Docs, Figma):
  → WebSockets (OT/CRDT + двунаправленный поток изменений)
...
Новый стандарт от W3C (World Wide Web Consortium) и IETF (Internet Engineering Task Force). Работает поверх HTTP/3 (QUIC) и сочетает преимущества WebSockets и QUIC:

```txt
WebTransport преимущества над WebSockets:
  - Несколько независимых стримов в одном соединении
  - Unreliable datagrams (как UDP — игры, видео, где важнее
    скорость, а не надёжность)
...
- **"WebSockets не работают через HTTPS"** — работают. HTTPS (шифрованный HTTP) меняет схему, а не механизм upgrade. `ws://` — незашифрованный (как http://), `wss://` — зашифрованный (как https://). В production обязательно `wss://`.
