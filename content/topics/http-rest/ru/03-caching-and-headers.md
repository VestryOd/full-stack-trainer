...
Они управляют и браузерным кэшем, и любым общим кэшем перед источником — например, сетью доставки контента (CDN).
...
```
Кэш хранить **можно**, но перед использованием нужно **валидировать** с сервером (conditional request). Если сервер говорит "не изменилось" → 304, кэш используется. Это *не* означает "не кэшировать".
...
Cache-Control: max-age=0   # Принять кэш свежее 0 секунд (= всегда валидировать)
...
- **"ETag — это хэш файла"** — не обязательно. ETag может быть хэшем, версией из базы данных (БД), timestamp или любым строковым идентификатором, уникально представляющим версию ресурса. Сервер сам определяет формат.
