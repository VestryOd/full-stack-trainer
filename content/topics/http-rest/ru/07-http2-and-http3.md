...
Браузеры обходили это открывая **6 параллельных TCP-соединений** на домен. Но каждое соединение — отдельный handshake TCP (transmission control protocol) плюс TLS (transport layer security).
...
HTTP/2 (2015, RFC 7540 — документ из серии request for comments) решает эти проблемы сохраняя семантику HTTP — те же методы, статусы, заголовки. Меняется только **транспорт**.
...
HTTP/2 (упрощённо):
  [Length: 3Б][Type: 1Б][Flags: 1Б][Stream ID: 4Б][Payload: N Б]
...
Динамическая таблица:
  Первый запрос: Authorization: Bearer eyJ... → в таблицу
  Второй запрос: Authorization: Bearer eyJ... → просто индекс,
                 1-4 байта вместо 500+
...
Почему не взлетел:
```txt
1. Браузеры кэшируют ресурсы. Сервер не знает, что уже в кэше.
...
Формально HTTP/2 поддерживает работу без TLS (h2c — cleartext). На практике все браузеры требуют TLS для HTTP/2. Поэтому HTTP/2 = HTTPS (HTTP поверх TLS).
...
HTTP/2 поверх TCP:
  ...
  Потерян TCP-пакет из середины — все стримы ждут его заново.
...
### QUIC — UDP (user datagram protocol) + надёжность
...
Это критично для мобильных пользователей и для IoT (internet of things).
...
- REST (representational state transfer) в API
...
HTTP/1.1 оптимизации, которые во HTTP/2 НЕ нужны:
  ❌ Domain sharding (CDN-домены ради обхода лимита 6 соединений)
  ❌ CSS/JS спрайты (один файл ради меньшего числа запросов)
  ❌ Inline CSS (встраивание стилей чтобы сэкономить запрос)

HTTP/2 оптимизации:
  ✅ Много маленьких файлов ≈ один большой (мультиплексинг
     уравнивает их)
  ✅ Granular caching (маленькие файлы кэшируются независимо,
     изменение одного не инвалидирует всё)
...
[S1 frame][S3 frame][S5 frame][S1 frame][X LOST ][S1 frame]
                                                    ↑
                        Все стримы ждут, пока TCP пришлёт X
...
- **"HTTP/2 = быстрее всегда"** — не всегда. На быстрых сетях без потерь HTTP/1.1 с несколькими соединениями может быть сопоставим. HTTP/2 показывает большой выигрыш на высоколатентных или нестабильных соединениях (мобильные или CDN — сеть доставки контента — с >100ms RTT).

- **"HTTP/3 на UDP — значит ненадёжный"** — QUIC реализует надёжную доставку поверх UDP на уровне приложения (подтверждения, повторные передачи, порядок). UDP выбран как транспорт потому что TCP нельзя изменить без обновления ОС (операционной системы) всех узлов в интернете.
...
- **"Domain sharding хорошо для HTTP/2"** — наоборот. Domain sharding требует нескольких DNS-резолюций (domain name system) и TLS handshake. В HTTP/2 всё мультиплексируется в одном соединении — domain sharding только мешает.
