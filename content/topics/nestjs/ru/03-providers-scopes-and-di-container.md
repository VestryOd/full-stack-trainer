...
## Symbol-токены — уникальные и защищённые от опечаток

Токен-строка работает, но у неё две беды. Опечатку в `'JWT_SECRET'` компилятор не заметит, а два модуля, выбравшие одну и ту же строку, столкнутся друг с другом.

`Symbol` в экспортированной константе решает и то и другое. Имя пишется один раз, а каждый символ уникален по построению, поэтому два модуля пересечься не могут.

```typescript
// Строковые токены ('JWT_SECRET') — опечатки и коллизии
// Решение: Symbol в общей константе
...
// Инъекция — в константе не опечатаешься:
constructor(@Inject(JWT_SECRET) private jwtSecret: string) {}
// Тип по-прежнему берётся из вашей аннотации, а не из токена
```

И чего токен не умеет. `InjectionToken<T>` в `@nestjs/common` — это **тип**, а не класс: это объединение `string | symbol | Type<T> | Abstract<T> | Function` всего, что может служить токеном. Создавать через `new` там нечего, и тип значения не хранит ни один токен Nest — для этого и нужна аннотация в конструкторе.
...
- **"Токен должен быть строкой"** — нет. Токеном может быть класс (самый частый случай), строка или `Symbol`. `InjectionToken<T>` — это *тип* этого объединения, а не класс, который создают через `new`. Для своих токенов берите `Symbol` в экспортированной константе: в ней не опечатаешься на инъекции, и она не столкнётся с токеном другого модуля.
