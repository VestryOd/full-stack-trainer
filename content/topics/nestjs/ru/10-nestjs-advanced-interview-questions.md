...
**Q: Чем `Symbol`-токен лучше строкового?**

Тем, что в символе нельзя опечататься и он ни с чем не столкнётся. Строковый токен перепечатывают на каждой инъекции, поэтому опечатка в `'DATABASE_URL'` падает в рантайме, а не на компиляции. Две строки `'DB'` из разных модулей разрешаются в один токен и перезаписывают друг друга; два символа — никогда, даже с одинаковым описанием.

Чего символ **не** даёт, так это типа значения. Его не несёт ни один токен Nest: `InjectionToken<T>` — это тип-алиас для того, что может быть токеном, а не хранилище типа значения. Тип инъекции задаёт аннотация в конструкторе.

```typescript
// Строковый токен — перепечатывается везде, сталкивается между модулями
{ provide: 'DATABASE_URL', useValue: 'postgres://...' }
// Инъекция: @Inject('DATABASE_URL') url: string — опечатка здесь компилируется

// Symbol-токен — одна константа, уникальная по построению
export const DATABASE_URL = Symbol('DATABASE_URL');
{ provide: DATABASE_URL, useValue: 'postgres://...' }
// Инъекция: @Inject(DATABASE_URL) url: string — в имени не опечатаешься
