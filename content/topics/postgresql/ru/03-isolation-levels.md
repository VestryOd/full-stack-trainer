...
Запись всё равно может упасть. Если ваша транзакция обновляет или
удаляет строку, которую после создания снапшота изменила и
закоммитила другая транзакция, PostgreSQL откатит вашу с ошибкой:
ERROR:  could not serialize access due to concurrent update

Это сообщение принадлежит REPEATABLE READ, а не SERIALIZABLE.
Его SQLSTATE — 40001, так что логика повтора та же.
...
Так о себе сообщает проверка SSI, и выдать это сообщение может только
SERIALIZABLE. При этом транзакция на SERIALIZABLE может получить и
ошибку "concurrent update" выше: правило «первый писатель побеждает»
из REPEATABLE READ работает и здесь.

У обоих сообщений SQLSTATE 40001. Приложение ДОЛЖНО перехватывать
этот код и повторять транзакцию.
