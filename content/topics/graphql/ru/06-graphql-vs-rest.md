...
Оба подхода — HTTP-протокол, JSON, stateless-запросы. REST (Representational State Transfer) — это архитектурный стиль вокруг ресурсов и URL. Разница не в "технологии". Разница в том, кто контролирует форму ответа и какие архитектурные инварианты встроены в протокол.
...
GraphQL:
  - Схема и есть спецификация — сервер физически не может вернуть
...
## BFF (Backend For Frontend) — наиболее распространённый способ совместного использования
...
- **"GraphQL заменит REST"** — не понимать, что они решают разные задачи. У GraphQL нет нативного кэширования на CDN (content delivery network, сеть доставки контента), и загрузка файлов сложнее. Большинство публичных API останутся REST — именно из-за простоты потребления и HTTP-кэша.

- **"REST типизирован хуже, потому что нет схемы"** — не упоминать OpenAPI/Swagger как стандарт типизации REST API. Ещё и не объяснять разницу в гарантиях: документ OpenAPI может разойтись с кодом, а GraphQL-схема **и есть** runtime-контракт.

- **"В GraphQL нет версионирования — значит всё всегда ломается"** — не знать про `@deprecated` и additive evolution как основную стратегию. Про breaking-change detection через schema diffing в CI (continuous integration, непрерывная интеграция) тоже не говорят.
