...
Аналогия с HTTP-методами помогает на старте, но скрывает самое важное различие между Query и Mutation. Это различие — порядок выполнения top-level полей. Он определён спецификацией и напрямую влияет на согласованность данных.
...
и оно НЕ должно влиять на выбор архитектуры (например,
"давайте не будем разбивать это на отдельные запросы, чтобы
сохранить согласованность данных" — спецификация УЖЕ
...
PubSub (publish/subscribe) — это шина, которая доставляет событие от опубликовавшей его мутации во все открытые подписки.
...
логика разрыва соединения при истечении токена (таймер на
...
- **"Ошибки в GraphQL — это всегда `errors[]`"** — не знать о Payload Pattern с typed error unions для ожидаемых бизнес-ошибок. Trade-off между null bubbling и явной обработкой ошибок на клиенте тоже остаётся без объяснения.
