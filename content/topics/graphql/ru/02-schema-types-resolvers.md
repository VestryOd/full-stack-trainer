...
SDL (Schema Definition Language) — это формальный язык. Он описывает каждый возможный запрос к API и гарантии о форме ответа.

"Гарантии" здесь ключевое слово. `String!` — это не просто "обычно не null", это **обязательство** сервера. Нарушение этого обязательства приводит к ошибке выполнения (см. ниже про null bubbling). То есть схема влияет на runtime-поведение, а не только на статическую типизацию.
...
Без кастомного скаляра `createdAt: String!` технически работает, но теряет смысловую типизацию. Клиент не видит из схемы, что строка — это дата в ISO (международная запись даты и времени, `2026-08-22T10:00:00Z`). Codegen тоже сгенерирует `string`, а не `Date`/`DateTime`.
...
### `context` — создаётся один раз на запрос, а не на каждый резолвер
...
ПЕРЕДАВАЕМЫЙ ПО ССЫЛКЕ в КАЖДЫЙ resolver. Это идеальное
место для:
...
### `info` — редко используется, но решает конкретную проблему overfetching на уровне базы данных
...
- **"`parent` — это родительский GraphQL-запрос"** — не понимать, что `parent` — это **результат** родительского резолвера, обычный JS-объект. Резолвер по умолчанию просто читает из него нужное свойство.
