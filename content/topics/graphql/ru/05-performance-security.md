...
Эта асимметрия — причина, почему GraphQL API требуют дополнительного слоя защиты, которого нет у типичного REST (Representational State Transfer) API. Этот слой состоит из трёх частей:

- Depth limiting — ограничение глубины запроса.
- Query complexity analysis — анализ стоимости запроса.
- Timeout на резолверы.

Без них единственный валидный (с точки зрения схемы) запрос может стать вектором DoS (denial of service, отказ в обслуживании).
...
## Rate Limiting — GraphQL не отменяет классику, но единицей измерения должна быть сложность запроса
...
[Schema, Types, and Resolvers]    — null bubbling и его
                                      ...
                                      authorization
[N+1 Problem and DataLoader]       — DataLoader как часть общей
...
- **"Depth limiting решает проблему дорогих запросов"** — не упоминать Query Aliasing, который обходит depth limiting при глубине 1. Query Complexity с multipliers — более точный механизм, и о нём тоже не говорят.
