...
The point of HMR is not speed as such. It is that application state — a filled-in form, an open wizard step — survives the change.
...
Why it bubbles up rather than down is a matter of logic, not implementation. Suppose a module changed but does not know how to reinstall itself. The only party that can do it is whoever imported it: that module holds the reference and can pick up the new version. If it cannot either, we move on to its importer. Reaching the entry point without a boundary, the runtime honestly gives up and reloads the page.
...
`dispose` is the most underrated of them. A module may start a timer or subscribe to an event at top level. If no `dispose` is written, every hot replacement leaves the previous version running. The symptom is recognizable: after ten edits a request fires ten times instead of once. That is not an "HMR bug" but an unreleased resource.

## How this works in Webpack and in Vite

```txt
  WEBPACK: rebuild what was touched
┌────────────────────────────────────┐
...
└────────────────────────────────────┘
  the work depends on how many modules
  the rebuild touched

       VITE: one file on request
┌─────────────────────────────────────┐
│ after a file is saved               │
│                                     │
│ 1   the watcher noticed a change    │
...
**Webpack** keeps the built graph in memory and, after a file changes, rebuilds the affected portion. That portion is not one file. If the changed module belongs to a chunk, the chunk has to be recomputed — and with module concatenation enabled, its neighbours too.

The result is written into hot-update files (`.hot-update.json` and `.hot-update.js`), which the browser is told about over the WebSocket. Hence an important property: reaction time grows with the project, because the amount of rebuilding grows.

**Vite** holds no bundle, so there is nothing to rebuild. It marks the module stale and sends a message to the browser. The browser then requests **exactly one** module, transformed on demand like any other dev-mode request ([The Vite Model](./06-vite-model.md)). The amount of work depends neither on project size nor on which chunk the module would have landed in for production.
...
Fast Refresh is a plugin: `@vitejs/plugin-react` in Vite, `react-refresh-webpack-plugin` in Webpack. It inserts `accept` into files containing React components automatically, and it knows how to recreate a component while preserving its state. In other words, it is the mechanism that creates update boundaries on your behalf.
...
The practical rule: one file, one component declared as a named function. Keep constants and helpers in separate files.
...
**Changing hooks resets state.** And rightly so. Hook state is stored positionally. Adding a `useState` in the middle of a component leaves the old state out of step with the new code. Fast Refresh honestly resets it rather than feeding values into the wrong variables. Class components always remount for the same reason: Fast Refresh cannot preserve their state at all.

## Proxying to the backend, and CORS in dev

CORS is cross-origin resource sharing: the browser rule that a page may only call the origin it came from, unless the server says otherwise. In production the frontend and the API usually share a domain, or the API sits behind the same CDN (content delivery network). So the rule never comes up.

In dev the app runs on `localhost:5173` while the API runs on `localhost:3000`. Those are different origins, and the browser starts demanding CORS headers from the backend.

The right answer is not to allow CORS for the sake of development. It is to remove the cross-origin situation entirely: the dev server proxies API requests itself.
...
The shape of `proxy` is a practical upgrade trap. It was an object in `webpack-dev-server` 4 and an array of objects in 5. An old config simply stops working.
...
Separately, what proxying does *not* solve: **it is dev-only.** If your production app calls an API on a different domain, CORS has to be configured on the backend for real. The dev proxy hides that work rather than doing it.

The classic "works in dev, CORS in prod" bug belongs to the same family as the other dev/prod differences ([The Vite Model](./06-vite-model.md)). The CORS headers themselves are covered in the Security topic and in the topic on HTTP and REST (representational state transfer).
...
Decoding the name, one word at a time:

- `eval` — module code runs through `eval` with a source annotation, which is fast.
- `cheap` — line-only mappings, with no columns.
- `module` — loader-produced maps are honoured, so stack traces show your TypeScript rather than the result of transpiling it.

The full table of options and trade-offs is in [Tree Shaking and Optimization](./05-tree-shaking-and-optimization.md).
...
Work through the symptoms in one order. First "did the update arrive at all", then "was a boundary found", and only then the Fast Refresh rules.
...
| nothing happens at all | the watcher misses the file change | Docker, WSL (Windows Subsystem for Linux), a network drive: polling vs events |
...
**1. Did the update reach the browser?** Open the console and see whether an update message appears after a save. Silence means the problem is not HMR but file watching or the connection.

The first common cause is code inside a Docker container or on a mounted drive, where file system events are not propagated. The fix is to switch to polling: `server.watch.usePolling` in Vite, `watchOptions.poll` in Webpack. Polling costs processor time continuously, so turn it on only where it is needed.

The second common cause is a WebSocket that never connected. The app is opened through your own proxy or over https. The client then has to be told which address and port to use (`server.hmr.clientPort`, `server.hmr.protocol`).

**2. Was a boundary found?** If the message appears but the page reloads entirely, the bubbling reached the root. Start with the changed file rather than with the settings, and ask three questions:

- Does it export anything besides components?
- Is there a barrel file in the chain?
- Is it a file that everybody imports, a theme config for instance? Updating such a module naturally leads to a reload.

**3. The Fast Refresh rules.** If the page does not reload yet state is still lost, the issue is component recognition. Look for an anonymous `export default`, a name that is not PascalCase, a class component, or a changed set of hooks.
...
- [Why Bundlers Exist](./01-why-bundlers-exist.md) — the dev-versus-prod frame: why feedback speed is what matters in dev.
- [The Module Graph and Resolution](./02-module-graph-and-resolution.md) — the graph an update bubbles up through.
- [The Webpack Core Model](./03-webpack-core-model.md) — the `devServer` section and the model of one build for both modes.
- [The Vite Model](./06-vite-model.md) — transform-on-request, from which HMR's independence from project size follows.
- [Tree Shaking and Optimization](./05-tree-shaking-and-optimization.md) — the full table of source map options.
- [Ecosystem and Choosing a Tool](./08-ecosystem-and-choosing.md) — what to measure when the dev server drags.
- React topic — Fast Refresh from the component side, and the rules of hooks.
- HTTP/REST and Security topics — CORS for real, rather than via a proxy.
- Angular course — the dev server of the Angular command-line tool.
...
- **Not knowing why a full reload happens instead of HMR.** The expected answer is about bubbling. The update searches up the graph for a module with `accept`. If it finds none before the entry point, it falls back to a reload. "HMR must not be configured" without the mechanism is weak.
...
- **Being unable to explain why component state was lost.** This is the most practical question in the topic. A good answer lists concrete causes: an anonymous `export default`, a non-PascalCase name, a class component. Two more are a changed set of hooks and a non-component export from the file. The last one is the least known and the most frequently hit.
...
- **Not knowing about `dispose`.** The question is "after ten edits the request fires ten times — why?". It cleanly separates people who have written HMR handlers by hand from those who have only used ready-made ones. The answer: previous versions of the module left timers and subscriptions behind.
