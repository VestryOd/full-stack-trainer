...
```txt
         React SPA — это public client
┌──────────────────────────────────────────────┐
│ Весь код исполняется в браузере пользователя │
│ Любой "секрет", зашитый в JS-бандл, виден    │
│ в DevTools → Sources                         │
├──────────────────────────────────────────────┤
│ Значит: client_secret не нужен вообще        │
│ Вместо него code_verifier защищает           │
