...
2. **Разрозненный доступ к данным** — смена `prisma.order.create` на сырой SQL (Structured Query Language) потребует изменений во всех контроллерах
...
**Репозиторий (Repository)** предоставляет к доменной сущности интерфейс, похожий на коллекцию. С точки зрения вызывающего кода это выглядит как типизированная in-memory коллекция — вы вызываете `findById`, `save`, `delete`. Репозиторий инкапсулирует все SQL-запросы, вызовы ORM (object-relational mapper) или обращения к API.
...
Риск: универсальный `IRepository<T>` нередко порождает метод `findAll()` со сложными параметрами фильтрации и сортировки. Такой метод превращается в построитель запросов, то есть в повторную реализацию ORM поверх ORM. Держите универсальную базу тонкой (только `findById`, `save`, `delete`) и помещайте все доменно-специфичные запросы в конкретный интерфейс.

## Паттерн Service Layer

**Сервис (Service)** или **Сервисный слой (Service Layer)** — место, где живут бизнес-операции. Он оркестрирует репозитории и другие зависимости для выполнения use case — без знания об HTTP, WebSocket или командной строке (CLI).

```txt
Controller (HTTP)   CLI Command   Background Job
        │                │               │
        └────────────────┴───────────────┘
                         │ бизнес-язык: placeOrder, cancelOrder
                         ▼
                  ┌───────────────┐
                  │ OrdersService │  ← бизнес-логика и оркестрация
                  └──────┬────────┘
                         │ использует
             ┌──────────────────────────┐
             │                          │
...
- **«Репозиторий — это просто обёртка над Prisma»** — так описывают тонкий прокси, а не репозиторий. Прокси делает `return prisma.order.findMany(filter)` и пропускает типы Prisma `WhereInput` наверх в сервисный слой. Ценность репозитория — в абстракции. Сервис спрашивает все ожидающие заказы на доменном языке, а репозиторий переводит это в то, как данные хранятся. Если типы Prisma появляются в сервисе, репозиторного слоя по сути нет.

- **«Сервисы должны быть stateless, значит никаких полей»** — дело не в полях как таковых, а в состоянии экземпляра, которое меняется между вызовами. Внедрённые через конструктор зависимости (репозитории, другие сервисы) — это поля, и они в порядке: между вызовами они одни и те же. А вот изменяемое состояние, которое хранит промежуточные вычисления между вызовами (вроде `this.currentTransaction`), было бы проблемой в singleton-сервисе.
...
- **«В репозитории должен быть метод `findAll()`»** — `findAll()` на большой таблице — это источник проблем с производительностью. Если сервису нужно фильтровать данные, добавьте в репозиторий метод с доменным смыслом. Вернуть всё и фильтровать в памяти хуже, чем вызвать `findPendingOlderThan(date: Date)`. Если универсальная фильтрация действительно нужна, передавайте типизированный объект-фильтр, а не позволяйте типу `WhereInput` Prisma просачиваться в интерфейс.

- **«Сервисный слой — это место для всего, что не является роутингом»** — так описывают то, чем сервисный слой часто становится, а не то, чем должен быть. Управление транзакциями, кэширование, логика повторных попыток, ограничение частоты запросов — всё это имеет тенденцию просачиваться в сервисы. Сервис может заниматься бизнес-логикой **и** инвалидацией кэша **и** управлением повторными попытками сразу. Следующий шаг — вынести это в отдельные механизмы: декоратор кэширования, middleware повторных попыток. Задача сервиса — чистая бизнес-оркестрация.
