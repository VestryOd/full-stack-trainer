...
```ts
// ❌ Всё смешано — реальность кода в режиме "просто шипи"
...
- **Нельзя переиспользовать бизнес-правило** из CLI-скрипта, cron-задачи или WebSocket-обработчика — логика намертво привязана к HTTP
- **Изменение схемы БД** (переименование колонки) требует поиска по всем файлам роутов, где есть сырой SQL
