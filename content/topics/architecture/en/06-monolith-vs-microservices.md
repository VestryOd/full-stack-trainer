...
Monolith or microservices is a question about one number: how many separately deployable units your application has. Transactions, team autonomy and operational cost all follow from it.

> **Scope note:** This article sits at the boundary between application architecture (articles 01–05) and system design. Unlike the previous articles, it covers decisions about how many deployable units to have — a system-level decision.

It's included here because it follows directly from modular application design. Understanding module boundaries prepares you to decide when — or whether — to split those modules into separate services.
...
```txt
Big Ball of Mud → Layered Monolith → Modular Monolith
    → Service-Oriented → Microservices

Most teams should aim for the Modular Monolith and stop there,
before considering anything further to the right.
```

The decision isn't just about splitting services. It's about where your application sits on this spectrum. And it's about whether moving right is worth the cost at this point in the project's life.
...
1. **Synchronous calls** — REST (plain HTTP, where each URL is a resource) or gRPC. One service calls another and waits for a response.
2. **Asynchronous messaging** — Kafka, RabbitMQ, Amazon SQS (Simple Queue Service). One service publishes an event; others subscribe.
...
|---|---|---|
| Development speed | Fast — direct imports, one repo, one test suite | Slow — separate repos, contracts, versioned APIs, integration tests |
| Deployment | One pipeline, one artifact | N pipelines, N artifacts, container orchestration (Kubernetes) |
| Operational complexity | One log stream, one metrics dashboard | Distributed tracing, centralized logging, service mesh, N dashboards |
| Scaling | Scale the whole app | Scale only the bottleneck service |
| Transactions | ACID (atomicity, consistency, isolation, durability) — one database, one process | No ACID across services; Saga pattern or eventual consistency |
| Fault isolation | One failure can crash all | Failures contained to one service |
| Technology choice | One language, one runtime | Each service can use a different language or database (rarely worth it) |
| Team autonomy | Everyone works in the same repo; coordination needed | Teams deploy independently; less cross-team coordination |
...
The **Inverse Conway Maneuver** is a real strategy: deliberately structure your teams to match the architecture you want. If you want microservices, first organize teams around service boundaries, then let the architecture follow.

## The Strangler Fig Pattern — migrating from monolith to microservices

The **Strangler Fig Pattern** is the recommended way to migrate an existing monolith to microservices incrementally. It is named after a type of fig tree that grows around a host tree and eventually replaces it.

```txt
Phase 1: the monolith handles everything
         [Client] → [Monolith]

Phase 2: a gateway is added, one capability extracted
...
Phase 3: more capabilities extracted
         [Client] → [Gateway] → [Users Service]
                              → [Orders Service]
                              → [Monolith] (what is left)

Phase 4: the monolith is gone, or is a small legacy adapter
         [Client] → [Gateway] → [Users Service]
                              → [Orders Service]
...
app.use('/', proxy({ target: process.env.MONOLITH_URL })); // everything else
```
...
- You have (or are willing to build) the platform: container orchestration, distributed tracing, centralized logging, an API gateway, a build-and-deploy pipeline per service
...
- Transactions span multiple services and require saga patterns for operations that used to be a single database transaction
