...
Redux Toolkit (RTK) — официальный пакет, который убирает из Redux почти всю
ручную обвязку; дальше по тексту сокращаем его до RTK. Без таких
вспомогательных инструментов Redux требовал огромного количества шаблонного
кода (boilerplate) — кода, который приходится писать почти дословно
одинаково для каждой сущности. Вот типичный async-флоу на ванильном Redux:
...
RTK Query — встроенный в RTK инструмент для серверного состояния, то есть данных, которые живут на сервере. Для операций CRUD (создать, прочитать, обновить, удалить) он заменяет написанные вручную async-санки:
...
export const selectExpensiveItems = createSelector(
  [selectCartItems],
  // пересчёт запускается, только если items изменился
...
dispatch(action)
  ...
  Подписанные компоненты (useSelector) получают уведомление
      ↓
  React ре-рендерит компоненты, у которых изменился
  результат селектора
```
...
```ts
// Кастомный middleware — логирование действий
...
Возможности DevTools:
  - Лог всех задиспатченных действий с payload
  - Diff state до и после каждого действия
  - "Jump to" — прыжок к любому прошлому state без
    перезагрузки страницы
  - "Skip" — пропустить конкретное действие и увидеть
    state без него
  - Import/export state — воспроизвести баг с точным
    состоянием от пользователя
  - Тест редьюсеров прямо в DevTools (dispatch вручную)
```
...
**Time-travel отладка на практике**: пользователь сообщает о баге и экспортирует state из DevTools кнопкой "Export". Разработчик импортирует этот файл и видит приложение ровно в том состоянии, в котором случился баг. Это невозможно с мутабельным state (MobX, Zustand без middleware).
...
1. Предсказуемость как требование
   ...
   - time-travel debugging критичен для воспроизведения багов

2. Крупные команды с чётким разделением ответственности
   - Redux принудительно структурирует код (actions/reducers/
     selectors в отдельных файлах) — снижает вероятность
     хаоса при десятках контрибьюторов
