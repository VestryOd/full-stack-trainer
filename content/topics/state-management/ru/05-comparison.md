...
Главная ошибка при выборе между MobX, Redux и Zustand — считать их взаимозаменяемыми. Они решают похожую проблему, но исходят из принципиально разных мировоззрений. Под Redux дальше везде имеется в виду Redux вместе с Redux Toolkit (RTK) — официальным пакетом, который убирает почти всю ручную обвязку.

### MobX — реактивный граф: "объяви данные, а интерфейс сам подтянется"
...
**Парадигма**: императивное объектно-ориентированное программирование (ООП). Данные — это объекты с методами. Изменения происходят через мутации (внутри actions). Реактивность — скрытая инфраструктура.
...
// Производные значения — через селекторы вне store
...
const useOrderStore = create<OrderState>()((set, get) => ({
  ...
  // Производные значения — обычные методы (встроенного computed нет)
...
### Redux — ре-рендер управляется селектором

```tsx
// Redux: useSelector вызывается при КАЖДОМ dispatch любого action
// Если результат селектора изменился (по ===) — ре-рендер
...
### Zustand — ре-рендер по результату селектора

```tsx
// Zustand: аналогично Redux, но selector выбираете вы сами
// useStore подписывается и ре-рендерит при изменении результата селектора
...
```txt
MobX:    Автоматическая гранулярность. Ре-рендер только при
         изменении конкретных observables, прочитанных в рендере.
         Работает "магически", но требует понимания механизма,
         чтобы не потерять реактивность: деструктуризация вне
         observer, чтение вне реактивного контекста.

Redux:   Явный контроль через селекторы. Ре-рендер при изменении
         результата селектора. Требует мемоизации
         (createSelector) для объектов и массивов.
         Предсказуемо, но многословно.

Zustand: Явный контроль через инлайновые селекторы. Аналогично
         Redux, но без отдельного слоя селекторов — пишете
         прямо в хуке.
...
Для мобильного web разница ощутима. Для корпоративного одностраничного приложения — нет. Размер бандла не должен быть главным критерием выбора.
...
// Плюс: селекторы тестируются как обычные функции
...
// Шаг 4: добавить createSelector для мемоизации селекторов
// configureStore работает со старыми редьюсерами
```
...
Выбирайте MobX если:
  ...
  ✓ Большие объёмы мутируемых данных (финансовые таблицы,
    данные в реальном времени)
  ✓ Нужна минимальная конфигурация с максимальной реактивностью

Не выбирайте ни одно из трёх если:
...
|---|---|---|---|
| Парадигма | Реактивная, объектно-ориентированная | Функциональная, на событиях | Минималистичная, явные подписки |
| Мутабельность | Мутабельный, внутри action | Иммутабельный, Immer в RTK | Иммутабельный, `set` возвращает новый объект |
| Ре-рендеры | Автоматически, из реактивного графа | Явно, через селектор | Явно, через селектор |
| Производные данные | `computed` — лениво, с кэшем, автоматически | `createSelector` — вручную, с мемоизацией | Функции в сторе, кэша нет |
| Асинхронность | `flow` либо action плюс `runInAction` | `createAsyncThunk` | Обычные async-функции |
| Нужен ли провайдер | Опционально | Да | Нет |
| Шаблонный код | Средне | Много, хотя RTK снижает | Минимум |
| Размер бандла в килобайтах, minified и gzip | ~18 | ~17 | ~1 |
| TypeScript | Хорошо; с `flow` хуже | Хорошо | Отлично |
| DevTools | MobX DevTools, ограниченно | Redux DevTools: time-travel, export/import | Redux DevTools через devtools-middleware |
| Тестирование | Классы без React; нужен вызов dispose | Редьюсер — чистая функция, идеально; санки сложнее | `getState` / `setState` без React |
| Кривая освоения | 1-2 дня | 2-4 дня с RTK | 30 минут |
| Серверное состояние | Своя реализация, встроенной нет | RTK Query, встроен | React Query или SWR (stale-while-revalidate), внешние |
| Связь между сторами | Через RootStore, внедрённый в корне | `getState()` внутри санка | `get()` внутри методов стора |
| Серверный рендеринг (SSR) | Да, с нюансами | Да | Да, из коробки |
| Строгость | `enforceActions`, опционально | Навязана самим паттерном редьюсера | Встроенной нет |
| Подходит для | Сложных реактивных вычислений, команд с ООП-опытом | Больших команд, журнала изменений, legacy-проектов | Новых проектов, быстрого старта, малых команд |
...
- **Говорить "Zustand маленький, значит примитивный"** — размер бандла не коррелирует со зрелостью или функциональностью. Zustand — это обдуманный минимализм, а не ограниченность. Сложность в Zustand перемещается из инфраструктуры (boilerplate Redux) в явные структуры (slices, селекторы в хуках).

- **Не знать про Server Components и будущее state management** — React Server Components (RSC) меняют картину. Большая часть server-state вообще не нужна на клиенте. Данные с сервера приходят как props серверного компонента, без useEffect и без store. Сильный кандидат понимает, что RSC меняет распределение: "состояние" сервера — это URL + серверные компоненты, client-state — это взаимодействие пользователя: формы, фильтры, состояние интерфейса. Zustand и MobX лучше вписываются в этот мир (нет провайдера, нет serialization проблем), Redux требует адаптации.
