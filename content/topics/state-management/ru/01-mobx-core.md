...
**Данные с сервера в этой теме мы грузим в стор руками, чтобы была видна механика реактивности.** В продакшене эту работу берёт на себя готовая библиотека серверного состояния: React Query (теперь TanStack Query) или RTK Query из Redux Toolkit (RTK). Она отвечает за кэш, дедупликацию запросов и фоновое обновление, а сравнение подходов есть в статье [Сравнение инструментов управления состоянием](./05-comparison.md).
