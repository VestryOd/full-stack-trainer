...
**Загрузку с сервера здесь мы собираем руками, чтобы была видна механика MobX.** В продакшене эту работу берёт на себя готовая библиотека серверного состояния: React Query (теперь TanStack Query) или RTK Query из Redux Toolkit (RTK). Она сама кэширует ответы, дедуплицирует запросы и обновляет их в фоне, а сравнение подходов есть в статье [Сравнение инструментов управления состоянием](./05-comparison.md).
...
- **`runInAction`** — когда нужно быстро исправить существующий
  async-метод или логика простая: один запрос, один блок мутаций.
- **`flow`** — идиоматический способ MobX. Предпочтителен для:
  - сложных асинхронных сценариев с несколькими `await`;
  - случаев, когда нужна отмена;
  - новых сторов — так яснее, что это "MobX async action".
...
1. **Стандарт**: декораторы в JavaScript оставались в стадии предложения (proposal) годами, и разные реализации (Babel, TypeScript) отличались поведением. Proxy — стабильная часть языка начиная с ES2015 (редакция стандарта ECMAScript 2015 года).
...
- **"flow — это просто синтаксический сахар над async/await"** — не совсем. `flow` использует генераторы, что даёт возможность отмены (`cancel()`). С async/await отмену нельзя сделать нативно — нужны AbortController или флаги. Это реальное техническое отличие, важное для интерфейса: оно снимает гонки запросов при быстром вводе.

- **"Можно просто писать async methods без flow/runInAction"** — работает без strict mode, но нарушает батчинг. Каждое присвоение после `await` — отдельное синхронное уведомление. Поэтому компонент может ре-рендериться между `this.isLoading = false` и `this.user = user`, показывая промежуточное состояние.
...
- **Не освобождать реакции в компонентах** — это утечка памяти. Создайте `autorun` или `reaction` внутри `useEffect` и ничего не верните — и MobX продолжит держать ссылку на компонент после размонтирования. Компонент остаётся в памяти, а MobX продолжает вызывать на нём ре-рендеры. React strict mode выбрасывает про это предупреждение:
  ```ts
  useEffect(() => {
    const dispose = autorun(() => {
