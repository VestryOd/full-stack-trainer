...
В Kafka (и в распределённых системах в целом) говорят о трёх уровнях гарантий доставки. Важно понимать, что каждая из них описывает не только продюсера или только потребителя (consumer), а **весь путь сообщения**: запись → хранение → чтение → обработка.
...
Продюсер        Брокер         Потребитель
   ...
   │               │               │
   │               │               │── перезапуск
   │               │               │   читает офсет после msg
   │               │               │   → msg потеряно навсегда
```

Когда возникает в Kafka:
- **Producer**: `acks: 0` — не ждёт подтверждения от брокера
- **Потребитель**: `autoCommit: true` + коммит произошёл **до** завершения обработки
- Потребитель прочитал сообщение, сдвинул офсет, затем упал. Сообщение не обработано, а офсет уже сохранён

Когда уместно: метрики, аналитика кликов, логи большого объёма — там, где небольшая потеря данных допустима, а задержка на повторные попытки неприемлема.
...
Когда возникает в Kafka:
- **Продюсер**: `acks: 1` или `acks: -1` без идемпотентности — при повторной отправке продюсер может записать дубликат
- **Потребитель**: `autoCommit: false` + ручной коммит после обработки. При сбое до коммита сообщение перечитывается

Когда уместно: большинство реальных систем работают на at-least-once, компенсируя дубликаты идемпотентностью на стороне потребителя (см. ниже).
...
**Idempotent producer (идемпотентный продюсер)** — это продюсер, который может безопасно повторять запись, не создавая дубликатов.
...
t=4  Брокер видит PID=42, seq=5 — уже записано.
     Он отбрасывает сообщение и отправляет ACK.
     → дубликата нет
```
...
**Что даёт idempotent producer**: запись ровно один раз **в одну партицию одного топика**. Дубликаты при повторных попытках устранены.

**Что не даёт**: не защищает от дубликатов при обработке на стороне потребителя — это только половина пути.
...
Kafka Transactions — концептуально:
  ...
  beginTransaction()
    ...
    │     ← создаём запрос на оплату
    └── commitTransaction()
          ← атомарно фиксируем всё
...
**Честная оговорка**: транзакции Kafka работают только внутри Kafka, от Kafka к Kafka. Гарантия держится потому, что офсет потребителя коммитится в *ту же* транзакцию, что и выходные топики.

Запишите результат в базу данных или во внешний сервис — и гарантии больше нет. Эта система не участвует в транзакции Kafka, а двухфазный коммит поддерживает далеко не всякое внешнее хранилище. Документированный выход — хранить офсет там же, где и результат. В большинстве реальных систем вместо транзакций используют идемпотентных потребителей.

## Idempotent Consumer — практическое решение

**Idempotent consumer (идемпотентный потребитель)** — это потребитель, который при повторной обработке одного и того же сообщения даёт тот же результат, что и при первой.
...
**Poison message (ядовитое сообщение)** — сообщение, которое потребитель не может успешно обработать. Например: невалидный JSON, несовместимая схема, вызывает exception в бизнес-логике, зависит от недоступного сервиса.

**Проблема специфична для Kafka**: в отличие от RabbitMQ, Kafka не убирает сообщение из партиции. Если потребитель падает при обработке и не коммитит офсет, после перезапуска он снова получит то же сообщение. Бесконечный цикл. При этом отставание группы (lag) растёт без предела — это число сообщений между закоммиченным офсетом и концом партиции.
...
Без обработки poison message:
  ...
  → Потребитель завис на offset=42. Вся партиция заморожена.
...
### Потребитель DLT — мониторинг и ручной разбор
...
| Семантика | Потеря данных | Дубликаты | Как получить в Kafka |
|---|---|---|---|
| At-most-once | Возможна | Нет | `acks: 0` или автокоммит до обработки |
| At-least-once | Нет | Возможны | `acks: -1` плюс ручной коммит после обработки. Это стандарт |
| Exactly-once | Нет | Нет | Идемпотентный продюсер плюс транзакции Kafka, и только от Kafka к Kafka. Либо at-least-once плюс идемпотентный потребитель |
...
Нет. At-least-once — стандарт в большинстве продакшн-систем. При правильно реализованном идемпотентном потребителе (`ON CONFLICT DO NOTHING`, версионирование, таблица `processed_events`) дубликаты не создают проблем. Kafka Transactions добавляют сложность, которая оправдана только в конкретных сценариях (обработка финансовых транзакций, стриминговая обработка Kafka Streams).

**"Poison message просто вызовет исключение, и Kafka сама разберётся"**

Нет. Kafka не имеет встроенного механизма dead-letter на уровне брокера (в отличие от RabbitMQ). Без явной обработки потребитель застрянет на одном сообщении навсегда. Паттерн DLT — ответственность разработчика, не брокера.

**"Повторные попытки с экспоненциальной задержкой решают проблему poison message"**

Частично. Повторные попытки помогают при временных сбоях: сеть, недоступный сервис. Но если сообщение содержит невалидные данные — никакое количество retry не исправит ситуацию. DLT нужен именно для таких случаев: обнаружить, изолировать, разобраться вручную, не блокируя поток.

**"Idempotent producer устраняет все дубликаты"**

Нет. Idempotent producer устраняет дубликаты на уровне записи в одну партицию при повторной отправке. Он не защищает от дубликатов при обработке на стороне потребителя: потребитель прочитал, обработал и упал до коммита, а после перезапуска перечитает сообщение снова. Идемпотентность потребителя — отдельная задача.
