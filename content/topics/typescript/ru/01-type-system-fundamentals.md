...
1. Избыточные поля допустимы при присваивании:
   ...
   (отношение подтипа: больший тип совместим с меньшим)
...
3. Функции проверяются структурно по параметрам:
   (x: number) => void совместима с
     (x: number, y: string) => void
   в некоторых контекстах — это намеренно,
   см. статью 07 про вариантность
```
...
Используйте type когда:
  ...
  - Нужен tuple
  - Пишете conditional или mapped types
  - Даёте алиас примитиву или функции

В большинстве команд: interface для объектов и классов,
type — для всего остального. Нарушить одно из правил
не катастрофа, но важно понимать, почему разница есть.
```
...
const result = identity("hello"); // T выведен как "hello", result: "hello"
const result2 = identity(42);    // T выведен как 42

// Вывод из нескольких аргументов — один T может быть только одним типом:
...
pair(1, 2);          // T: number ✅
pair("a", "b");      // T: string ✅
pair(1, "hello");    // ❌ TS2345: T зафиксирован как number по первому аргументу
pair<string | number>(1, "hello"); // ✅ назовите union явно — компилируется
```

Литерал здесь выживает потому, что `result` — это `const`. Неограниченный `T` даёт *расширяющийся* литеральный тип, и расширяется он только в изменяемой позиции: в `let`, в литерале массива или в свойстве объекта.

---

## Типичные ошибки на интервью

- **"TypeScript — номинальная типизация"** — нет, TypeScript структурно-типизированный. Два класса с одинаковой структурой совместимы, даже если у них разные имена. (Исключение: можно *симулировать* номинальную типизацию через брендирование — см. [Продвинутые паттерны TypeScript](./09-advanced-patterns.md).)
