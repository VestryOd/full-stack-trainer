...
Вариантность описывает, как отношение подтипирования между простыми типами переносится на сложные типы, которые их содержат. Возьмём `Cat extends Animal`. Вариантность отвечает, что из этого следует для `Box<Cat>` и `Box<Animal>` или для `(x: Cat) => void` и `(x: Animal) => void`.
...
Мнемоника: "принимает больше — подставляется везде, где принимают меньше". Функция, которая умеет работать с любым `Animal`, точно справится с `Cat`.
