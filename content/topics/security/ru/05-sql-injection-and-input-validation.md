...
SQL Injection возникает, когда пользовательский ввод конкатенируется в запрос к базе данных без экранирования. SQL — Structured Query Language, язык, на котором приложение разговаривает с реляционной базой, и внедрённый текст становится частью команды. Злоумышленник может изменить логику запроса, обойти аутентификацию, прочитать или удалить данные.
...
В параметризованном запросе в тексте остаётся плейсхолдер вроде `$1`, а значение передаётся рядом с запросом. Драйвер никогда не собирает запрос конкатенацией, и все примеры ниже следуют этому правилу.
...
**Принцип**: данные никогда не становятся частью SQL-текста. База данных получает шаблон запроса и данные отдельно — нет возможности "выйти" из строкового контекста.

## Input Validation — почему серверная валидация обязательна

Валидация на фронтенде — это про удобство пользователя, а не про безопасность. Любой может отправить запрос напрямую через curl/Postman, минуя браузер и JS.
...
Эти два шага отвечают на разные вопросы. Валидация спрашивает, корректны ли данные, санитизация — безопасны ли они.

| | Validation (валидация) | Sanitization (санитизация) |
|---|---|---|
| Какой вопрос задаёт | Данные корректны? | Данные безопасны? |
| Что делает | Проверяет структуру, тип, формат | Преобразует и очищает данные для безопасного использования |
| При плохом вводе | Отклоняет запрос (400 Bad Request) | Не отклоняет — трансформирует |
| Пример | Email валиден? Длина пароля достаточна? | Экранирование HTML для отображения, нормализация пробелов |
...
В примере ниже перечислены пять атак, которые должна пережить точка загрузки файлов, и следом четыре серверные проверки, которые их останавливают.
...
- **"ORM полностью защищает от SQL Injection"** — ORM (объектно-реляционный маппер) защищает при использовании стандартных методов (`findBy`, `where: {}`). Но `$queryRawUnsafe` (Prisma), `query()` с конкатенацией (TypeORM), `createQueryBuilder` с `getMany()` при небезопасной интерполяции — уязвимы. Всегда проверяйте raw queries.
...
- **"Экранирование строк достаточно для защиты от SQL Injection"** — ручное экранирование зависит от кодировки, локали и версии базы данных, и его легко нарушить. Единственная надёжная защита — parameterized queries/prepared statements.

- **"Проверки заголовка Content-Type достаточно для загрузки файлов"** — Content-Type устанавливает клиент, злоумышленник ставит туда что угодно. Всегда проверяйте magic bytes файла (реальное содержимое), а не заголовок запроса.
