...
Server Component (App Router):
  ...
  → гидратации для этого компонента нет — ему нечего "оживлять"
```
...
выполнять "тяжёлые" зависимости (markdown-парсеры,
  image processing), которые не должны попадать в клиентский бандл
```
...
## Client Component — включается явно

Директива `'use client'` — это не "сделай этот компонент клиентским", а маркер **границы модуля**. Всё, что импортируется из файла с этой директивой, попадает в клиентский граф зависимостей — как и всё, что импортирует *этот* модуль.
...
Важный, часто упускаемый нюанс: **директива действует на весь модуль и на всё, что он импортирует**. Если `Counter.tsx` импортирует утилиту из `utils/date.ts`, эта утилита тоже попадёт в клиентский бандл. Неважно, что сама она не содержит браузерных API: она находится "ниже по графу" от `'use client'`-границы.
...
Импортировать Server Component внутрь Client Component **напрямую** нельзя. В момент рендера на клиенте у Client Component нет доступа к серверным ресурсам, которые нужны импортированному компоненту. Но Server Component можно передать как `children` или проп *ещё на сервере*, до пересечения границы:
...
`ServerOnlyContent` рендерится на сервере, как часть родительского Server Component. Только после этого его результат передаётся в `ClientWrapper` как `children`. И этот результат — RSC (React Server Component) payload, а не исходный код.

С точки зрения `ClientWrapper` это просто непрозрачный React-узел. Он не "знает", что внутри был серверный код, и не может на него повлиять. Например, не может обернуть узел в условие от клиентского состояния и заставить его перерендериться на сервере.
...
Это частая причина runtime-ошибки `Functions cannot be passed directly to Client Components`. Обычно она возникает, когда разработчик по привычке передаёт callback из Server Component — так, как сделал бы это в обычном React.

## `server-only` и защита от случайных импортов

Граница `'use client'` определяется по *графу импортов*. Поэтому легко случайно "протащить" в клиентский бандл серверный код — с секретами или прямыми запросами к базе данных. Простой пример: утилитарный файл обращается к `process.env.DB_PASSWORD`, а импортируют его и Server, и Client компонент.
...
1. Меньше JS в бандле
   Client Component  → HTML + JS-код компонента + зависимости
                       (всё это попадает в бандл)
   Server Component  → только результат рендера, 0 байт JS
                       (HTML / RSC payload)

2. Меньше hydration-работы
   Каждый Client Component при гидратации требует, чтобы React
   сопоставил серверный HTML с виртуальным DOM и навесил
   обработчики событий.
   Server Component — гидратации нет вообще, нет затрат CPU
   на клиенте.

3. Прямой доступ к данным
   Server Component может обратиться к БД напрямую — нет лишнего
   HTTP round-trip "браузер → API → БД", который был бы нужен
   Client Component.

4. Тяжёлые зависимости не попадают на клиент
   Например, markdown-парсер (remark/rehype) или библиотека
   форматирования используется только на сервере — клиент
   не платит за её вес.
```
 ...
 └─ ProductList (Server)        — рендерит список, фетчит данные
     └─ ProductCard (Server)     — статическая карточка
         └─ AddToCartButton (Client)
             — нужен onClick → useState/useTransition
```
...
React Context (через `useContext`/`createContext` с состоянием) работает только в браузере. Поэтому провайдеры темы, React Query, состояния авторизации и т.п. обязаны быть Client Components. Их обычно выносят в отдельный "тонкий" слой в корне дерева:
