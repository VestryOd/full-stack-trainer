...
# Модели рендеринга: CSR, SSR, SSG, ISR

Четыре модели из заголовка: CSR — рендеринг в браузере, SSR — рендеринг на сервере, SSG — статическая генерация при сборке, ISR — статика с фоновым обновлением.
...
В App Router (Next.js 13+) выбор модели больше не делается "на уровне всего приложения". Он делается *гранулярно*, на уровне отдельного route segment: через `fetch`-опции и экспортируемые конфигурации (`export const dynamic`, `export const revalidate`). Это важное отличие от Pages Router, где модель выбиралась на уровне всей страницы — экспортом функции `getServerSideProps`, `getStaticProps` или отсутствием обеих.
...
**Плюсы**: минимальная нагрузка на сервер — он отдаёт статику. И отличный пользовательский опыт *после* загрузки: переходы между состояниями мгновенные, без похода на сервер.

**Минусы**: пустой HTML до выполнения JS. Это плохо для поисковой оптимизации (SEO) и для метрик отрисовки — FCP (first contentful paint) и LCP (largest contentful paint). А фетчинг внутри `useEffect` создаёт *водопады* запросов (request waterfalls): компонент сначала монтируется, потом запрашивает данные, потом рендерит детей, которые тоже могут что-то запрашивать.
...
```txt
Request → Server рендерит React в HTML
        → HTML отдаётся браузеру → Hydration
```
...
**Минусы**: нагрузка на сервер пропорциональна трафику. Выше TTFB (time to first byte): нужно подождать рендер и fetch, прежде чем уйдёт первый байт. И сложнее кешировать на CDN (content delivery network) — хотя Next умеет кешировать SSR-ответы через заголовки `Cache-Control` и Data Cache.
...
```txt
Build time → HTML для каждой страницы → деплой на CDN
           → пользователи получают статику
```
...
```txt
Request 1 (в пределах TTL)
  → мгновенно отдаётся закешированная страница
Request после истечения TTL
  → отдаётся СТАРАЯ страница, в фоне запускается regeneration
Следующий request
  → отдаётся уже новая страница
```
...
После SSR/SSG браузер получает готовый HTML — контент виден сразу, но он **не интерактивен**: обработчики событий ещё не подключены. Hydration — это процесс, в котором React проходит по уже существующему дереву DOM (Document Object Model — дерево узлов страницы в браузере). React "подключает" к этой разметке свой виртуальный DOM и обработчики событий, *без* пересоздания с нуля.
...
Каталог товаров               → ISR (revalidate по TTL плюс
                                 on-demand при изменении товара)
Личный дашборд пользователя   → SSR или CSR
                                 (данные привязаны к сессии)
Лендинг / маркетинг           → SSG
Страница товара в e-commerce  → ISR + on-demand revalidation
                                 после обновления цены/наличия
Real-time данные (биржа, чат) → CSR + WebSocket/polling,
                                 либо Server Components + streaming
```

Важно: в App Router модели не выбирают "одну на всё приложение", их композируют. Layout может быть статическим (SSG), а вложенный в него Server Component с динамическими данными — рендериться по SSR-модели. Обёрните этот компонент в `<Suspense>`, и он уйдёт отдельным чанком через стриминг. Partial Prerendering — экспериментальная возможность, развивающая эту идею ещё дальше.

## Типичные ошибки на интервью

- **"SSG — это когда сервер рендерит HTML"** — нет. SSG означает, что HTML создаётся *во время билда*. На момент запроса сервер вообще не участвует в рендере: он просто отдаёт готовый файл.
...
- **Путают `revalidate: 0` и `cache: 'no-store'`** — оба "отключают" статический кеш, но семантически это разные вещи. Опция `revalidate: 0` всё ещё часть модели ISR и кеша Data Cache: эквивалент "не кешировать вообще". А `no-store` — это явный SSR на каждый запрос, без участия Data Cache. На практике результат близкий, но при ответе стоит показать, что вы понимаете разницу между Full Route Cache и Data Cache.

- **Думают, что hydration mismatch — это просто "warning в консоли, можно игнорировать"** — в продакшене это может привести к видимому "перерисовыванию" контента и потере состояния. Типичный пример: сбрасывается значение инпута, в который пользователь уже начал печатать.

- **Не могут объяснить, почему `Date.now()` или `Math.random()` напрямую в JSX — антипаттерн** — результат вычисляется дважды. Один раз на сервере (при SSR или SSG) и один раз на клиенте, при первом рендере до гидратации. Эти два значения почти гарантированно разойдутся.

- **Считают, что выбор модели делается один раз для всего приложения** — в App Router модель выбирается per-segment. Модели комбинируются и внутри одной страницы: статический layout, динамический контент, стриминг через Suspense.
