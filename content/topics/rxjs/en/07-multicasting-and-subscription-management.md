...
The laziness from [Reactive Model and Observables] means the subscriber function runs **per subscription**. For HTTP that is literal: two subscriptions, two requests.
...
A **cold Observable** creates its own source for every subscriber: `http.get()`, `new Observable(...)`, `defer(...)`, `interval(...)`. A **hot Observable** has a source that exists independently of subscribers and hands them the same values: a `Subject`, `fromEvent` (the event happens whether you listen or not), the result of `share()`.
...
```
                                       share and shareReplay: which to pick
┌────────────────────────────────┬────────────────────────────────────────┬─────────────────────────────────────┐
...
`share()` inserts a `Subject` between the source and the subscribers: the first subscription starts the source, the rest join the same result. When no subscribers remain, the source is unsubscribed — and the next subscription starts the work again.
...
The key detail the RxJS documentation warns about verbatim: with `refCount: false` (which is **the default** for the `shareReplay(1)` form) the source is **not unsubscribed** when the subscriber count drops to zero, and the inner `ReplaySubject` may "run for ever".
...
- **`refCount: false`** (the default) — sensible for one-shot expensive computations and HTTP requests you want cached for the application's lifetime: a reference list, config, the user profile. Here "the source is not unsubscribed" is not a leak but the desired behaviour, because the source has already completed.

The second nuance is error behaviour. `share()` has the flags `resetOnError` (default `true`), `resetOnComplete` and `resetOnRefCountZero`. If a `shareReplay` stream fails, all current subscribers receive the error — but whether the *next* subscriber gets a fresh attempt or the cached error depends on configuration. A caching reference list usually wants "reset on error" so the next subscription retries.
...
> **Legacy.** Multicasting used to be assembled by hand: `multicast(() => new Subject()), refCount()` or `publishReplay(1), refCount()`. In RxJS 7 the whole `multicast`/`publish`/`publishReplay`/`publishBehavior`/`refCount` family is deprecated and will be removed in v8. The replacements: `share()`/`shareReplay()` for ordinary cases, and `connectable(source, { connector })` when you specifically need a stream you connect manually via `connect()`. Seeing `publish().refCount()` means you are looking at RxJS 5–6 era code.
...
The second level of the same rule: if a value is only needed for display, hand the subscription to the framework — the `async` pipe in a template, or `toSignal` in Angular. Then there is nothing to close at all.

```
                                           How to close subscriptions
┌───────────────────────┬────────────────────────────────────────┬────────────────────────────────────────────┐
...
The order is not a style choice. A `takeUntil` placed **not last** leaves the operators after it alive: if a `switchMap` follows `takeUntil`, its inner subscriptions keep running, because `takeUntil` only completed the part of the chain before it. This is one of the rare situations where operator order affects leaks rather than results.

> **Legacy.** Manual management through `new Subscription()` with `add()` and an `unsubscribe()` in `ngOnDestroy` is the previous generation of solutions. It works but demands discipline at every new subscription. Today components use `takeUntil(destroy$)`, and in Angular `takeUntilDestroyed()`, which picks up the destruction context automatically.
...
If the log shows ten `SUB` entries against two `UNSUB`, the leak is found. Note that `tap` with an observer object sees both subscribe and unsubscribe ([Transformation and Filtering Operators]), while `finalize` fires on any ending ([Error Handling and Retries]).
...
```txt
[Reactive Model and Observables]  — the laziness behind "N subscriptions =
                                     N units of work", and teardown on unsubscribe
[Creating Streams and Subjects]   — the Subject that sits inside
                                     share/shareReplay
[Transformation and Filtering
 Operators]                        — take/takeWhile/takeUntil as ways to complete
                                     a stream, and tap for diagnostics
[Flattening Operators]             — why nested subscriptions are not closed
                                     by an outer unsubscribe
[Error Handling and Retries]       — finalize as guaranteed cleanup, and how
                                     share behaves on an error
```

## Common interview traps

- **"Why did the same Observable make three HTTP requests?"** — because it is cold: every subscription restarts the work, and three `| async` in a template are three subscriptions. What is expected is not only the diagnosis but the fix: `shareReplay({ bufferSize: 1, refCount: true })`, or a single subscription with an `as` variable in the template.
...
- **`subscribe` inside a service** — the service becomes the owner of the subscription and of state, exposes an array instead of a stream, and a request error terminates it forever. What is expected is the "subscribe close to the edge" rule: the service returns a stream, the consumer subscribes.
