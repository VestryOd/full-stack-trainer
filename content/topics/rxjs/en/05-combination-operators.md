...
The three answers land on one of these eight operators.
...
One shape note before the details: the operator forms of `combineLatest`, `merge` and `zip` inside `pipe()` are deprecated. Inside a pipe, use `combineLatestWith`, `mergeWith` or `zipWith`.

> **Legacy.** `combineLatest(a$, b$)`, `merge(a$, b$)` and `zip(a$, b$)` as **static functions** are the norm. The same names as **operators inside `pipe()`** are deprecated in RxJS 7 and will be removed in v8. The reason was confusion: `a$.pipe(combineLatest(b$))` and `combineLatest(a$, b$)` looked alike but produced a different argument order.
...
combineLatest   ------A-B---C----D--|
                      ^ A=(1,x) — the first emit waits for
                        query$ to produce a value;
                        B=(2,x) C=(2,y) D=(3,y): either source

withLatestFrom  --------A--------B--|
                        ^ A=(2,x): value 1 is lost — query$ had no
                          value yet; emits only on the leading
                          status$
   combineLatest gives 4 values (any change), withLatestFrom 2:
    the leader picks the moment, the follower adds a snapshot
```
...
The second thing to keep in mind is that `combineLatest` emits on **every** change. Update three filters programmatically one after another and you get three emits and three requests. The cure is `debounceTime(0)`, which collapses a synchronous burst, or `distinctUntilChanged` with a comparator.
...
Using `combineLatest` here would simply be a bug: it would save on every form change rather than on the button press. And conversely, `withLatestFrom` is wrong for filters, because changing a "follower" filter would not trigger a reload.
...
The second trap of `withLatestFrom` is visible in the diagram. A leading value that arrives before the follower's first value is **lost silently**. A click made before the user profile loaded does nothing — no error, no log. If that is unacceptable, give the follower a `startWith` or restructure with `switchMap`, which will wait for a value.
...
forkJoin  -------------------(2,y)|
                             ^ one value — the last of each,
                               and only after every source
                               has completed

zip       ----A---------B----|
              ^ A=(1,x): pairs by index — first with first,
                B=(2,y): second with second;
                extra values are buffered
   forkJoin over an infinite source never emits anything;
  zip buffers when sources differ in speed — a memory risk
```
...
- **An error in any source terminates the result.** Like `Promise.all`, `forkJoin` fails as a whole. To get a partial result, swallow errors inside each source with `catchError` — see [Error Handling and Retries](./06-error-handling-and-retries.md).
...
- **`race`** picks the winner by **first emit**, not by completion, and unsubscribes from the losers. The classic use is a timeout: `race(api.load(), timer(5000).pipe(switchMap(() => throwError(() => new Error('timeout')))))`. Although for timeouts there is a dedicated `timeout` operator — see [Error Handling and Retries](./06-error-handling-and-retries.md).
...
**`combineLatest` instead of `withLatestFrom` for an action.** A "save" button written as `combineLatest([clicks$, form$])` saves on every form change — that is, on every keystroke. The sign of it: the action fires without the user acting.

**`withLatestFrom` instead of `combineLatest` for filters.** The list reloads only when the status changes, while editing the search box "does nothing". The sign of it: some filters work, others do not.
...
- [Reactive Model and Observables](./01-reactive-model-and-observables.md) — the subscription contract every combination rests on.
- [Creating Streams and Subjects](./02-creating-streams-and-subjects.md) — `BehaviorSubject` as a way to guarantee a first value for `combineLatest`.
- [Transformation and Filtering Operators](./03-transformation-and-filtering-operators.md) — `startWith`, `debounceTime` and `distinctUntilChanged` alongside `combineLatest`.
- [Flattening Operators](./04-flattening-operators.md) — `switchMap` after `combineLatest`: the standard "filters to request" pairing.
- [Error Handling and Retries](./06-error-handling-and-retries.md) — why one failed source breaks `forkJoin`, and how to get a partial result.
- [Multicasting and Subscription Management](./07-multicasting-and-subscription-management.md) — how many subscriptions `combineLatest` creates on a shared source.

## Common interview traps

- **Not knowing about `combineLatest`'s silence** — it emits nothing until every source has produced a value. The expected answer includes the symptom ("the screen is empty although the data loaded") and three fixes: `startWith`, a `BehaviorSubject`, or rethinking the construct. Then comes the follow-up: what if one source never emits at all? The result never appears, and there is no error.

- **Confusing `combineLatest` with `withLatestFrom`** — the most common pair after `switchMap`/`mergeMap`. What is expected is a criterion rather than a definition: what should cause a recomputation? One more probing question: what happens on a click if the follower has not emitted yet? The value is lost silently.
