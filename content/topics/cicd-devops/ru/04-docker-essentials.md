...
**Docker** — платформа для сборки, доставки и запуска приложений в изолированных, воспроизводимых средах, называемых **контейнерами**. Ключевая проблема, которую он решает, — это классическое "работает у меня на машине".

Одно и то же приложение ведёт себя по-разному в dev, в CI (непрерывной интеграции), в staging и в production. В каждом окружении своя версия ОС (операционной системы), свой runtime, свой набор библиотек и своя системная конфигурация.
...
Виртуальная машина (VM):
  ...
  │ └─────────────────────────────────┘ │
  └─────────────────────────────────────┘
  У каждой VM — своё полное ядро ОС; тяжело, запуск за минуты
...
Контейнеры используют две функции ядра Linux, чтобы создать иллюзию отдельной машины без накладных расходов полной ОС. **Namespaces** изолируют процессы: у каждого контейнера своё дерево процессов, свой сетевой стек и своя видимость файловой системы. **Cgroups** (control groups — контрольные группы) ограничивают, сколько CPU (процессорного времени) и памяти достанется контейнеру.
...
```txt
Image (образ)  = схема: послойный снимок файловой системы
                 только для чтения (read-only) со всем,
                 что нужно приложению для запуска
                 (runtime, зависимости, конфиги, готовый код)

Container      = запущенный экземпляр образа
(контейнер)      = образ + записываемый слой сверху
...
node:20-alpine   — образ на базе Alpine Linux (~50 МБ)
                   меньше и быстрее качается, но его C-библиотека —
                   musl, а не привычная glibc (нативные аддоны,
                   рассчитанные на glibc, могут ломаться)
node:20-slim     — на базе Debian, без многих инструментов (~80 МБ)
...
Финальный `rm -rf /var/lib/apt/lists/*` удаляет кэш пакетного менеджера. Если бы он был в отдельном `RUN`, кэш всё равно остался бы запечённым в предыдущем слое, и образ стал бы больше.
...
docker run my-app:1.0 node dist/migrate.js      # CMD заменён на node dist/migrate.js
...
**Всегда используйте exec form для `CMD` и `ENTRYPOINT`.** Shell form оборачивает команду в `/bin/sh -c "..."`, поэтому ваше приложение запускается как дочерний процесс `sh`. Это создаёт две проблемы:

1. Процесс не получает сигналы ОС вроде `SIGTERM` напрямую. Их получает `sh` и может не передать дальше. Это ломает graceful shutdown (корректное завершение).
2. Проблема PID 1. В контейнере у процесса с PID (идентификатором процесса) 1 есть особые обязанности: он поглощает зомби-процессы. Если ваше приложение — дочерний процесс `sh`, то PID 1 у `sh`, а он зомби-процессы корректно не обрабатывает.

## Слои образа и кэш сборки

Каждая инструкция `RUN`, `COPY`, `ADD` и `FROM` в Dockerfile создаёт новый **слой** (layer) в образе. Слои накладываются друг на друга: каждый записывает только разницу (diff) со слоем под ним.

```txt
Образ "my-app:1.0"
  Слой 5: COPY . . + RUN npm run build  ← меняется часто
  Слой 4: RUN npm ci --only=production  ← меняется с lockfile
  Слой 3: COPY package.json, lockfile   ← меняется с зависимостями
...
Если вы изменили исходный файл (src/server.ts):
  ...
  Слой 3 (COPY package.json)  — cache HIT (файл не менялся)
  Слой 4 (RUN npm ci)         — cache HIT (lockfile не менялся)
  Слой 5 (COPY . .)           — cache MISS (исходники изменились)
                                → пересборка отсюда
```
...
```dockerfile
# ✅ Правильный порядок — установка зависимостей кэшируется,
# пока не меняется package-lock.json
FROM node:20-alpine
WORKDIR /app
...
COPY package.json package-lock.json ./
RUN npm ci                              # ВСЕ зависимости, включая devDependencies
...
С multi-stage:       ~80 МБ (runtime + prod-зависимости + вывод)
```
...
```txt
# .dockerignore
node_modules/          # не копировать: ставим заново внутри образа
.git/                  # история git не нужна в образе
.env                   # никогда: внутри могут быть секреты
...
1. **Скорость**: без `.dockerignore` вся директория `node_modules/` — а это потенциально сотни мегабайт — передаётся демону Docker при каждом `docker build`. Это медленно и тратит трафик в CI.
2. **Безопасность**: файлы `.env`, локальные учётные данные и приватные ключи никогда не должны попадать в Docker-образ. Даже если за `COPY . .` следует `RUN rm .env`, файл `.env` остаётся в промежуточном слое. Его можно извлечь из образа командой `docker history`.
3. **Корректность**: `COPY . .` без `.dockerignore` копирует `node_modules/` с хоста в образ. А они могли быть собраны под другую ОС: нативные аддоны, собранные на macOS, не запустятся на Linux.

## Non-root пользователь и rootless-контейнеры

По умолчанию процессы внутри Docker-контейнера работают от имени `root` — это пользователь с UID (идентификатором пользователя) 0. Это риск безопасности: если злоумышленник использует уязвимость в приложении и выходит за пределы контейнера, он делает это с правами root — получая максимальные привилегии на хосте.
...
# переключиться на встроенного пользователя node
USER node
CMD ["node", "dist/server.js"]
```

**Rootless containers** (бескорневые контейнеры) — более широкая концепция: весь демон Docker работает от имени непривилегированного пользователя на хосте. Тогда, даже если контейнер выйдет за свои пределы, он попадёт в пространство этого непривилегированного пользователя, а не в root. Это отдельная конфигурация установки Docker, не Dockerfile.

```txt
На практике для fullstack-разработчика:
  → Добавляйте USER <non-root> во все production Dockerfile
  → Используйте --chown в COPY, чтобы задать владельца файлов
  → Не используйте --privileged в docker run (полный доступ к хосту)
...
**Docker Compose** — инструмент для определения и запуска многоконтейнерных приложений. Вместо нескольких команд `docker run` с десятками флагов вы описываете весь стек приложения в файле `docker-compose.yml`. Дальше его запускает одна команда.
...
services:
  ...
  db:
    ...
    environment:
      ...
      POSTGRES_DB: myapp
    volumes:
      - postgres-data:/var/lib/postgresql/data   # named volume: переживёт перезапуск
    healthcheck:
...
```txt
services    → каждый ключ — именованный контейнер с конфигурацией
...
depends_on  → порядок запуска сервисов относительно друг друга
networks    → по умолчанию Compose создаёт одну сеть, и все сервисы
              к ней подключаются; поэтому "db" в DATABASE_URL
              разрешается в IP контейнера сервиса db
```

### Senior-нюанс: `depends_on` не означает "готово"
