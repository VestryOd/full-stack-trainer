...
`+` — единственный оператор с двойным поведением: либо числовое сложение, либо строковая конкатенация.
...
Это баг, не фича. Его хотели исправить в ECMAScript 2015, но предложение отклонили из-за совместимости с миллиардами строк существующего кода.
...
typeof 42n         // 'bigint'
typeof function(){} // 'function' (тоже особый случай: функция —
                    //  объект, но typeof даёт 'function')
typeof {}          // 'object'
...
`NaN` (Not-a-Number) — единственное значение в JS, которое не равно самому себе. Так требует стандарт 754 организации IEEE (Institute of Electrical and Electronics Engineers), который описывает арифметику с плавающей точкой.
...
[Современный JS]     — Object.hasOwn, Number.isNaN, Number.isFinite
                       — строгие версии старых функций без неявного
                       ToNumber
[Управление памятью] — typeof используется для проверки типов, но
                       typeof null = 'object' требует отдельной
                       проверки
```
