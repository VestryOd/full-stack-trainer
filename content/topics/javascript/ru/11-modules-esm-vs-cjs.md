...
CommonJS (CJS) — собственная система модулей Node.js, появившаяся до стандартных ESM (ECMAScript Modules). Понимание её механики важно для работы с legacy-кодом и для объяснения ключевых отличий от ESM.
...
2. Linking (Связывание)
   ...
   — экспортируемые привязки (let/const/function) создаются,
     но не инициализируются
...
Пусть в CJS модуль A требует B, а B требует A — это цикл. Тогда B получает **текущее состояние `module.exports` A**: то, что было присвоено до кругового `require`.
...
**Ключевое правило для ESM-циклов**: если нужны данные из кругового импорта, берите их через функцию. Функция захватывает привязку, а не значение на момент создания. Второй вариант — убедиться, что нужный модуль инициализирован к моменту обращения.
...
Нюанс: `const snapshot = value` копирует **текущее значение** примитива (2) в локальную переменную. Само имя `value` — это live binding на привязку в lib.mjs. Но при присваивании копируется значение примитива, а не привязка, поэтому `snapshot` не обновляется при последующих `increment()`.
...
// import() в CJS грузит любой ESM, даже с top-level await:
// main.cjs
async function main() {
...
```txt
CJS → ESM: require() грузит ESM с Node.js 22.12 (и 20.19)
           Условие: в графе модуля нет top-level await,
                    иначе бросается ERR_REQUIRE_ASYNC_MODULE
           На версиях старше или с top-level await: import()
...
[Современный JS]       — import() динамический, AbortSignal для
                         отмены fetch внутри top-level await
```
...
- **"`import()` и `require()` — одинаковые"** — нет. `import()` возвращает `Promise<namespace object>` с `default` и именованными экспортами. Вызов `require()` возвращает `module.exports` синхронно. С Node.js 22.12 `require()` тоже грузит ESM, но только если в графе модуля нет top-level `await`. У `import()` такого ограничения нет.
