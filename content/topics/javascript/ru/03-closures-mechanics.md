...
Function Object {
  ...
  [[Environment]]   → ссылка на Environment Record,
...
### Что сборщик мусора (GC) считает "живым"

Сборщик мусора V8 (поколенческий mark-and-sweep) удерживает объект, пока на него есть **reachable ссылка**. Reachable — значит есть путь от GC roots: стек, глобальные переменные, живые замыкания.

Замыкание удерживает весь Environment Record, на который ссылается его `[[Environment]]`. Это означает, что одна живая функция, замкнутая на ER, держит в памяти весь ER. Он остаётся там, даже если функция не обращается к большей части его переменных.
...
**2. Отсоединённые узлы DOM (Document Object Model):**
```js
function setup() {
...
До ESM (ECMAScript modules) единственным способом создать "приватные" переменные было замыкание внутри immediately invoked function expression (IIFE):
...
```txt
[Контексты выполнения] — замыкание = [[Environment]] → ER,
                         созданный во время execution context
[this-binding]         — стрелочная функция захватывает this тем
                         же механизмом: своего ThisBinding нет,
                         разрешение идёт по Scope Chain
[Управление памятью]   — замыкания как основная причина утечек;
                         WeakMap и WeakRef — GC-безопасные ссылки
[Генераторы]           — генератор хранит состояние в своём ER и
                         приостанавливает выполнение: замыкание
                         плюс control flow
```
