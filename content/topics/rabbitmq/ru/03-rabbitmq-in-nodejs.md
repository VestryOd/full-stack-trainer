...
Пакет `amqplib` — стандартный Node.js-клиент для AMQP 0-9-1 (Advanced Message Queuing Protocol). Библиотека предоставляет два API: на колбэках (`amqplib/callbacks`) и на Promise (`amqplib`). Всегда используйте Promise-based API с `async/await`.
...
Вызов `nack` с `requeue: true` опасен без задержки. Сообщение сразу возвращается в начало очереди и доставляется снова — потенциально тысячи раз в секунду. Правильный паттерн повторных попыток с backoff разбирается в статье 04.
...
Без prefetch RabbitMQ доставляет все сообщения из очереди потребителю так быстро, как может. Допустим, в очереди 50 000 сообщений, а потребитель обрабатывает медленно. Все 50 000 окажутся в его памяти разом. Вы фактически переместили очередь из управляемого хранилища RabbitMQ в heap вашего приложения.
...
```txt
Prefetch = 1:
  ✓ Идеально честно: каждый потребитель обрабатывает одно сообщение
  ✗ Высокая задержка: нужен ack перед получением следующего
  ✗ Низкая пропускная способность: нет конвейерного параллелизма
  Подходит для: тяжёлых медленных задач со строгим порядком
...
Prefetch = 100+:
  ...
  ✗ При краше потребителя надо переслать больше сообщений
  Подходит для: быстрых, лёгких по I/O потребителей
```

## Dead Letter Queues (DLQ, очереди мёртвых писем)

**Dead Letter Queue (DLQ)** — это очередь мёртвых писем, куда попадают сообщения, которые невозможно обработать. Сообщение становится "мёртвым" в трёх случаях:
...
channel.consume('orders.dead-letters', (msg) => {
  ...
  // 2. Сохранить в базу данных для анализа
  // 3. Повторить с экспоненциальным backoff
  //    (перепубликовать в исходную очередь с задержкой)
  channel.ack(msg);
});
```
