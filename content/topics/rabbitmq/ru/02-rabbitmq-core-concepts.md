...
RabbitMQ реализует **AMQP** (Advanced Message Queuing Protocol, Расширенный протокол очередей сообщений) — открытый wire-level протокол для middleware, ориентированного на сообщения. "Wire-level" означает, что протокол определяет точные байты, передаваемые по сети. Поэтому любая клиентская библиотека на любом языке, реализующая AMQP, может работать с RabbitMQ без привязки к поставщику.

RabbitMQ использует AMQP 0-9-1. Существует также AMQP 1.0 — другой и несовместимый стандарт: RabbitMQ поддерживает его через плагин, но нативный именно 0-9-1. Когда вы используете `amqplib` в Node.js, вы говорите именно на AMQP 0-9-1.
...
Exchange принимает сообщения от продюсеров и маршрутизирует их в очереди по заданным правилам. Exchange **не хранит** сообщения — только маршрутизирует. Если сообщение пришло в exchange, но ни одна очередь не подходит, оно удаляется. С выставленным флагом `mandatory` оно вместо этого возвращается продюсеру.
...
await channel.assertQueue('my-queue', {
  ...
  exclusive: false,   // другие соединения могут использовать
                      // (true = удалить при отключении)
...
Binding — это связь между exchange и очередью с опциональным **binding key** (ключом привязки). Он говорит exchange отправлять подходящие сообщения в эту очередь. Одна очередь может иметь несколько привязок от разных exchange, а один exchange — несколько привязок к разным очередям.
...
Привязки:
  'events' ──[order.*]───► queue: 'order-service'
      совпадает: order.placed, order.cancelled
  'events' ──[*.placed]──► queue: 'analytics'
      совпадает: order.placed, payment.placed
  'events' ──[payment.#]─► queue: 'billing-service'
      совпадает: payment.processed, payment.refund.initiated
...
// Привязываем все очереди: '' как routing key — это соглашение
// для fanout (всё равно игнорируется)
...
**Важно:** Fanout реализует Pub/Sub. У каждого сервиса своя очередь, он получает собственную копию и обрабатывает её независимо. Exchange доставляет во все эти очереди.
...
Default exchange (пустая строка `""`) — это предварительно объявленный direct exchange с одним особым правилом. **Каждая очередь автоматически привязана к нему, а её имя служит routing key.**
...
Вызов `sendToQueue` — это синтаксический сахар для `publish('', queueName, ...)`. Он публикует в default exchange, а routing key становится имя очереди. Для простых случаев этого достаточно. При росте системы явные exchange дают больше гибкости: можно добавить нового потребителя в topic exchange, не трогая продюсер.
...
Соединение с RabbitMQ — это соединение TCP (Transmission Control Protocol), и открывать его дорого. **Channel** (канал) — лёгкое виртуальное соединение, мультиплексированное поверх одного TCP-соединения. На практике: одно соединение на процесс, каналы — по необходимости (на поток, на операцию или на потребителя).
...
Частая ошибка с amqplib: переиспользование одного канала и для публикации, и для потребления в высоконагруженном приложении. Правильнее держать отдельные каналы для публикации и для потребления и никогда не вызывать методы канала конкурентно из двух async-операций на одном канале.
