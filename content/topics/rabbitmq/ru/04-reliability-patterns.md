...
```txt
Producer ──► Broker ──► Consumer (успешно обработал) ──► ack
                                    │
                          Broker удаляет сообщение
...
**RabbitMQ не предоставляет exactly-once "из коробки".** Kafka transactions и EOS (Exactly-Once Semantics) приближают к этому. Но и там exactly-once достигается на уровне приложения, через идемпотентных потребителей. Это не гарантия самого брокера.
...
### Идемпотентность через upsert-семантику в базе данных
...
`nack` с `requeue: true` повторяет мгновенно — сообщение возвращается в начало очереди и потребитель получает его снова через миллисекунды. При временных ошибках (нижестоящий сервис временно недоступен, сетевой сбой) нужно ждать перед повтором.

В RabbitMQ нет встроенной задержки повторов. Её строят через паттерн **delayed queue** из TTL (time to live, время жизни сообщения) и DLQ (dead letter queue, очередь мёртвых писем):

```txt
Основная очередь ──(ошибка)──► Retry Exchange
                                     │
                                     ▼
                        Retry Queue (TTL: 30с)
                                     │
...
По умолчанию `channel.publish()` — это fire-and-forget на уровне TCP (Transmission Control Protocol). Если соединение оборвалось до сброса буфера, брокер мог сообщение и не получить. **Publisher confirms** (режим подтверждений) заставляют канал ждать явного ack от брокера для каждого опубликованного сообщения.
...
Самый распространённый пробел в надёжности event-driven систем: вы сохраняете данные в базу и публикуете сообщение двумя отдельными операциями. Если процесс упал между ними — либо запись в базе есть без сообщения, либо сообщение опубликовано без записи.
...
**Transactional Outbox** (транзакционный журнал исходящих) решает это: сообщение записывается в таблицу `outbox` в той же транзакции базы данных, что и бизнес-данные. Отдельный relay-процесс читает outbox и публикует в RabbitMQ.
...
async function relayOutboxMessages(
  ...
  // SELECT FOR UPDATE SKIP LOCKED не даёт двум relay-процессам
  // взять одну и ту же строку
...
Детекция: заголовки `x-death` накапливаются при каждой повторной доставке. Когда `count` превышает порог — считайте сообщение отравленным.
...
async function quarantineMessage(msg: ConsumeMessage): Promise<void> {
  // Сохраняем в базу данных для ручной проверки
  await db.query(
...
- **"Transactional Outbox переусложнён — достаточно try/catch вокруг публикации"** — try/catch здесь не поможет. Если процесс упал после записи в базу и до запуска публикации, блок catch тоже не выполнится. Outbox — единственный способ гарантировать атомарность записи в базу и публикации сообщения без распределённых транзакций.
