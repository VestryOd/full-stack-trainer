...
Что вы получаете:
  + схема как единая точка правды о форме документа — та самая
    схема, которая в MongoDB «живёт в коде» (см. [Document Model
    and Use Cases])
...
Чем вы платите:
  - результат запроса — не документ БД, а Document-обёртка:
    геттеры/сеттеры, отслеживание изменений, валидация. Это память
    и CPU на каждый объект (лечится lean() — см. [Mongoose Queries,
...
3. Схема НЕ создаёт ничего в базе, кроме индексов. Никакого DDL
   нет: коллекция появится при первой записи.
```

## SchemaTypes и приведение типов
...
осознанно (см. [Mongoose Queries, populate, and Pitfalls]).
```
...
1. unique — это НЕ валидатор. Mongoose лишь создаёт уникальный
   ...
   unique: true (см. [Mongoose Queries, populate, and Pitfalls]).
...
const userSchema = new Schema<User>({
  ...
  username: {
    type: String,
    required: true,
    // асинхронный валидатор: обращение к БД внутри валидации
    validate: {
...
Асинхронный валидатор «занят ли username» полезен для понятного сообщения об ошибке, но он **не является гарантией уникальности**: между проверкой и записью проходит время (см. race condition в [CRUD and Query Operators]). Гарантию даёт только уникальный индекс.
...
Практический вывод: если инварианты важны, есть два honest-подхода:
...
1. Домен пишет только через документ:
   ...
   атомарности «read-modify-write» (см. [CRUD and Query Operators]).
...
```typescript
// Virtual — вычисляемое поле, в БД не хранится
...
// он превращается в строку (Extended JSON — см. [Document Model
// and Use Cases]), и это ДВА разных типа, которые нельзя путать
// в DTO
```
...
JSON — и первый же lean() ломает типизацию (см. [Mongoose Queries,
populate, and Pitfalls]).
```
...
```txt
[Document Model and Use Cases]    — схема, которая «живёт в коде»;
                                    ObjectId и Extended JSON на
                                    границе API
[CRUD and Query Operators]        — что реально делают updateOne и
                                    findOneAndUpdate, на которых
                                    не срабатывают document-хуки
[Schema Design: Embedding vs      — что описывать вложенной схемой,
 Referencing]                       а что ссылкой с ref
[Indexes and Query Performance]   — unique как индекс, а не валидатор;
                                    индексы, объявленные в схеме
[Mongoose Queries, populate,      — lean(), populate, autoIndex,
 and Pitfalls]                      __v, маппинг E11000 в ошибку API
```

## Типичные ошибки на интервью

- **«Валидация Mongoose работает всегда»** — она работает на `save()`/`create()`, а на `updateOne`/`findOneAndUpdate` — только с `runValidators: true`, и даже тогда `required` не проверяется для полей, которых нет в обновлении. `bulkWrite` не валидируется вовсе.
...
- **«`unique: true` в схеме — это валидация»** — это объявление уникального индекса. Нарушение приходит как ошибка БД `E11000`, а не `ValidationError`; а если индекс не создан (`autoIndex: false` в проде), уникальности нет вообще.
...
- **«Mongoose — это ORM, он абстрагирует MongoDB»** — он не убирает необходимость понимать документную модель, индексы и атомарность: `populate` останется дополнительными запросами, а плохая схема останется плохой (см. [Schema Design: Embedding vs Referencing]).
