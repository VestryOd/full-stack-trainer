...
Из этого следует важное практическое ограничение, которое ломает половину старых тестов: **`fakeAsync`/`tick` с Vitest не работают**. Ранер Vitest не патчит zone.js, а документация прямо говорит, что `fakeAsync` больше не рекомендуется. Вместо него — обычный `async`/`await`, `await fixture.whenStable()` и фейковые таймеры самого Vitest (`vi.useFakeTimers()`).

### Что чем тестировать

Инструмент выбирается по тому, что вы тестируете. Большей части строк ниже `TestBed` не нужен вовсе.
...
Чем меньше `TestBed`, тем быстрее и стабильнее набор тестов. Логику выносят в сервисы именно поэтому (глава 05).
...
`TestBed` — это конфигурируемый инжектор плюс возможность создать компонент. Angular-тесты стоит любить именно за подмену через DI (внедрение зависимостей): модули мокать не нужно, достаточно дать другой провайдер (глава 04).
...
│ экземпляр создан, шаблон ещё не проверялся                 │
└────────────────────────────────────────────────────────────┘
                               │  входы
                               ▼
┌────────────────────────────────────────────────────────────┐
...
└────────────────────────────────────────────────────────────┘
   fakeAsync/tick не работают с ранером Vitest: zone.js там
 не патчится, и документация больше не рекомендует fakeAsync
```

Два места, где тесты v22 отличаются от тестов трёхлетней давности:

1. **`await fixture.whenStable()` вместо `fixture.detectChanges()`.** В zoneless-тестах ручной `detectChanges()` — это принудительная синхронизация, которая может скрыть настоящую ошибку. Компонент, забывший уведомить фреймворк, в тесте «работает», а в приложении нет. А `whenStable()` даёт Angular запланировать проверку самому — то есть тест проверяет ровно то поведение, которое будет в продакшене.
2. **Входы задаются через `fixture.componentRef.setInput(...)`.** Прямая запись `component.ticket = x` не помечает компонент изменённым (глава 11). А с сигнальными входами она ещё и не скомпилируется: `input()` возвращает сигнал только для чтения.

Полезные приёмы:

- `TestBed.inject(Token)` — достать зависимость.
- `TestBed.overrideProvider(...)` — подменить её после конфигурации.
- `TestBed.runInInjectionContext(fn)` — вызвать гард, резолвер или функцию с `inject()` внутри.
- `DeferBlockBehavior.Manual` — пошагово проверять состояния `@defer`-блока (глава 12).
...
API контроллера: `expectOne`, `expectNone`, `match` (несколько запросов), `verify`. У запроса: `flush(body, opts)` для успеха, `error(new ProgressEvent('error'), { status: 500 })` для ошибки. Лучше вынести `verify()` в `afterEach` — тогда каждый тест заодно проверяет, что лишних запросов не было.

### Harness: тест, не привязанный к разметке

`cdk/testing` даёт слой абстракции между тестом и DOM (объектной моделью документа). Harness описывает компонент через его *поведение*: нажать, прочитать текст, выбрать опцию. Сам тест не знает ни про классы, ни про вложенность.

Загружается через `TestbedHarnessEnvironment.loader(fixture)`, дальше `getHarness(Predicate)` / `getAllHarnesses(...)`. Свой harness — класс, наследующий `ComponentHarness`, с локаторами (`this.locatorFor('.selector')`) и методами домена.
...
Юнит- и компонентные тесты не проверяют то, что интересует пользователя. Работает ли путь целиком: открыл список, отфильтровал, создал тикет, увидел его в списке? Это и проверяет сквозной тест (end-to-end, e2e), и для него нужен настоящий браузер: **Playwright** (де-факто стандарт сейчас) или Cypress.

Правила разумного e2e:

- Их мало — единицы ключевых сценариев.
- Они не заменяют юнит-тесты.
- Элементы ищут по роли, тексту или `data-testid`, а не по CSS-классам.
- Бэкенд без причины не мокают, иначе смысл теста теряется.

Protractor, который поставлялся с Angular исторически, давно удалён.

## Параллели с React

Задачи совпадают одна в одну, инструменты — нет.

| задача | React: Jest/Vitest + React Testing Library | Angular: `TestBed` |
|---|---|---|
...
- **Подмена зависимостей — главное преимущество Angular.** С `jest.mock('./api')` вы перехватываете импорт на уровне сборщика: путь надо знать, hoisting надо помнить, а типизация мока — на вашей совести. В Angular зависимость запрашивается по токену, поэтому подмена — одна обычная запись: `{ provide: TicketApi, useValue: fake }`. Она типизирована и не зависит от путей файлов. Именно это делает DI не «лишним слоем», а инструментом (глава 04).
- **Философия React Testing Library против доступа к экземпляру.** React Testing Library (RTL) сознательно не даёт добраться до состояния компонента: проверяйте то, что видит пользователь. В Angular у вас есть `fixture.componentInstance`, а с сигналами — прямое чтение состояния. Это удобно, но легко скатиться в тесты, проверяющие реализацию. Разумная граница: состояние проверять в тестах сервисов, а компоненты тестировать через DOM или harness.
...
Три набора тестов, плюс один e2e-сценарий как пример:

- Юнит-тесты `TicketStore`, без `TestBed`.
- Тест HTTP-слоя с `HttpTestingController`, включая проверку интерсептора.
- Тест компонента списка с подменой API через провайдер и harness для карточки тикета.
...
1. Юнит-тесты `TicketStore` без `TestBed`. Проверяйте сигналы прямым чтением:
   - `add` добавляет в начало.
   - `update` сохраняет порядок и не трогает остальные объекты.
   - `remove` по несуществующему id не бросает исключение.
   - `computed`-счётчики пересчитываются.
...
4. Тест компонента `TicketList`. Подмените `TicketApi` заглушкой через провайдер и дождитесь `whenStable()`. Затем проверьте, что отрисовалось нужное число карточек, и что при пустом ответе показано пустое состояние. Никаких `fixture.detectChanges()` и никакого `fakeAsync`.
...
describe('TicketStore', () => {
  // TestBed нужен только чтобы получить экземпляр с его зависимостями;
...
- Компонент **не уведомляет** Angular об изменении. Ручной `detectChanges()` принудительно синхронизирует и скрывает это, а `whenStable()` ждёт запланированной проверки, которую никто не запланировал. Причина обычно в мутации вместо `set` или в записи в обычное поле из асинхронного колбэка (глава 03). В продакшене такой компонент тоже не обновится, то есть тест обнаружил реальный баг, а не «неудобство API».
- Ничего особенного не нужно: `httpResource` подписывается сам. Добавьте `provideHttpClientTesting()`, затем `expectOne` и `flush()`, а затем `await fixture.whenStable()`, чтобы шаблон увидел новое значение. Важно помнить, что ресурс отправляет запрос при создании компонента, так что `expectOne` идёт **после** `createComponent`.
- Три причины встречаются чаще всего:

  - Периодический опрос (polling) или `interval` в компоненте отправил ещё один запрос, о котором тест не знает.
  - Ресурс `httpResource` перезапросил данные, потому что изменился читаемый им сигнал.
  - Вы ждали `whenStable()` после `flush()`, и за это время ушёл следующий запрос цепочки — например, `reload()` после мутации.

  Диагностика — `httpTesting.match(() => true)` перед `verify()`, чтобы увидеть, что осталось.
- Скорее всего провайдер объявлен не там, где ищется зависимость. Проверьте три места:

  - Компонент или его дочерний компонент объявил `providers: [TicketApi]` у себя, и локальный провайдер перекрыл тестовый (глава 04).
  - Сервис запрашивает не `TicketApi`, а `HttpClient` напрямую, и подменять надо было бэкенд (`provideHttpClientTesting`).
  - Вы сконфигурировали `TestBed` дважды, и второй вызов не применился, потому что компонент уже создан.
...
1. `detectChanges()` — это приказ «проверь шаблон сейчас», выполняемый независимо от того, уведомлял ли кто-нибудь фреймворк об изменении. А `whenStable()` дожидается **запланированной** синхронизации, то есть проверяет ту же цепочку, что произойдёт в приложении. Скрываемый класс ошибок — «состояние изменилось, но уведомления не было». Типичные причины: мутация массива вместо `set` или запись в обычное поле из `setTimeout` либо колбэка сторонней библиотеки. Ещё одна — `setValue` формы без отражения в сигналы (главы 03 и 10). С `detectChanges()` такой компонент проходит тесты и ломается в продакшене. С `whenStable()` тест падает там, где есть реальный дефект.
2. `fakeAsync`/`tick` построены на патчах zone.js, а ранер Vitest zone.js не патчит. Vitest стоит по умолчанию с v21, поэтому эти помощники там просто не работают, и документация больше не рекомендует `fakeAsync` в принципе. Замена прямая: обычные `async`/`await` для промисов и `await fixture.whenStable()` для ожидания синхронизации Angular. Таймерами управляют через `vi.useFakeTimers()`/`vi.advanceTimersByTime()`, а потоки двигают прямым контролем источника (`Subject.next()`) вместо виртуального времени.
3. `jest.mock('./api')` работает на уровне модульной системы. Нужно указать путь, а он ломается при переносе файла, и помнить про hoisting. Сам мок типизируется вручную и легко расходится с реальным API. Провайдер в `TestBed` работает на уровне токена: `{ provide: TicketApi, useValue: fake }`. Из этого три следствия. Тест не зависит от расположения файлов и не сломается от рефакторинга структуры. Заглушка проверяется компилятором на совместимость с токеном, поэтому изменение сигнатуры сервиса ломает компиляцию теста, а не даёт ложно зелёный прогон. И тем же механизмом подменяется что угодно: конфиг через `InjectionToken`, `Router`, HTTP-бэкенд.
4. Потому что сигнальный стор — обычный класс без шаблона. Состояние читается вызовом сигнала, команды — вызовом методов, а `computed` пересчитывается сам при чтении. Ни компилятора шаблонов, ни change detection здесь не нужно: `TestBed` пригодится только чтобы собрать зависимости, иначе их передают вручную. Набору тестов это даёт скорость, потому что компоненты не создаются. И устойчивость: такие тесты не ломаются от правок вёрстки и проверяют поведение, а не разметку. Это и есть практическая причина выносить логику из компонентов в сервисы (глава 05).
5. Потому что `querySelector('.ticket-card__title > span')` привязывает тест к структуре разметки. Любое переименование класса или добавление обёртки ломает тесты, не имеющие отношения к изменению. Harness — это API компонента для тестов: `getTitle()`, `isSelected()`, `click()`. Селекторы живут в одном месте, внутри harness, поэтому правка вёрстки правится один раз. Дополнительно harness скрывает асинхронность: все его методы возвращают промисы и сами дожидаются стабилизации. И он переиспользуется между юнит- и интеграционными тестами. А библиотечные компоненты часто поставляют свои harness, так что знать их внутреннюю разметку вам не нужно вовсе.

</details>

## Частая ошибка

Первая — расставить `fixture.detectChanges()` после каждой строки, по образцу тестов из старых статей. В zone-эпоху это было необходимостью. Сейчас это активно вредно: принудительная проверка скрывает именно те дефекты, которые тест должен ловить. Компонент, забывший перевести состояние в сигнал, с `detectChanges()` проходит тест и ломается в браузере.

Хуже того, привычка приводит к тестам, которые «зелёные» ровно потому, что синхронизацию вызывали руками в нужных местах. Правильная форма — `await fixture.whenStable()` там, где нужно дождаться обновления, и ничего между действиями. Если тест без `detectChanges()` не проходит — это диагноз коду, а не тесту.

Вторая — присваивание входа напрямую: `fixture.componentInstance.ticket = ticket`. С сигнальными входами это не скомпилируется: `input()` возвращает сигнал только для чтения. А с оставшимися `@Input()`-полями пройдёт молча и оставит компонент непомеченным. Шаблон не обновится, и `expect` упадёт на пустом DOM без внятной причины.

React-опыт подсказывает, что пропсы — это просто значения. Но в Angular вход — часть контракта, за которым стоит уведомление фреймворка. Правильно: `fixture.componentRef.setInput('ticket', ticket)`, и затем `await fixture.whenStable()`. Тот же принцип, что с динамическими компонентами из главы 11.
