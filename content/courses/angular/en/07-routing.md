...
- Order matters: routes are matched top-down, and `'**'` always comes last.
- `path: ':id'` comes **after** `path: 'new'`, otherwise `/tickets/new` matches the parameter.
- `providers` on a route creates its own injector, and it lives as long as the route does.
...
for role-gated lazy sections the difference is megabytes
```
...
Four guards and one resolver, and the difference between them is when they run:

| hook | when it runs | what `false` means | typical use |
|---|---|---|---|
...
The key distinction, and a common interview question: **`canMatch` runs while matching, `canActivate` runs after**. There are two practical consequences.

First, `canMatch: false` does not cancel navigation. It makes the router keep looking, so one path can render different components for different roles.

Second, `canMatch` rejects a route **before** the lazy chunk loads, and `canActivate` runs after it. For an admin section that is the difference between "the user never downloaded the admin code" and "downloaded it but did not see it".

The older `canLoad` is deprecated in favour of `canMatch`; so are class-based guards (`implements CanActivate`).

### Reading route state

Four ways to read route state:
...
A snapshot does not update when `:id` changes and the component is reused. That is the source of the "opened another ticket, same data" bug.
...
A word on `snapshot`: it is valid as of the moment the component was created. Navigating from `/tickets/1` to `/tickets/2` makes the router reuse the same instance, because only the parameter changed. The `snapshot` then stays as it was, and `ngOnInit` never runs again. This is the single most common trap in Angular routing.
...
Preloading is configured as a feature. `withPreloading(PreloadAllModules)` pulls every lazy chunk after startup. A custom strategy can be narrower, loading only routes marked `data: { preload: true }`.

`provideRouter` takes optional features. Two of them are switched on in this chapter's project:

| feature | what it does |
|---|---|
| `withComponentInputBinding()` | feeds route params, query params and `data` into component inputs |
| `withInMemoryScrolling({ … })` | restores the scroll position and enables anchor scrolling |

The rest are situational, one line of configuration each:

| feature | when you reach for it |
|---|---|
| `withViewTransitions()` | animated transitions through the View Transitions API |
| `withRouterConfig({ onSameUrlNavigation: 'reload' })` | navigating to the same URL again must re-run the route |
| `withHashLocation()` | the host cannot rewrite every path to `index.html`, so URLs become hash-based |
| `withNavigationErrorHandler(fn)` | one place to log or redirect when a navigation fails |
| `withDebugTracing()` | printing every navigation step to the console while debugging |

Angular v22 adds two experimental features. `withExperimentalPlatformNavigation()` integrates with the browser Navigation API, and `withExperimentalAutoCleanupInjectors` destroys unused route injectors automatically.

### Are resolvers worth it

`ResolveFn` delays navigation until the data is ready. The upside: the component opens with data and never flashes empty. There are three downsides:

- The user stares at the old page with no idea what is happening.
- An error in the resolver breaks navigation entirely.
- The data arrives in `data` rather than in signals, so there is nothing to refresh it with later.

The practical position: a resolver fits things that genuinely must exist before rendering. Examples: checking that an entity exists and redirecting to 404, supplying a `title`, loading a section's configuration. The screen's own data is better fetched in the component with `httpResource` (chapter 08). There the loading and error states become part of the UI (user interface) rather than part of navigation.
...
- **A resolver versus a `loader` in React Router's data API.** Same idea — data before render. But in React Router a `loader` is the primary way to fetch data. In Angular a resolver competes with `httpResource`, which exposes `loading`/`error` as signals. So resolvers deserve to be used more sparingly in Angular than loaders are in React.
- **Route parameters.** `useParams()` returns values on every render, and a React component usually remounts when its key changes. In Angular the component is reused and the inputs update instead. No `key` equivalent is needed. But you must write code that reacts to an input change: a `computed`, not a one-off read in `ngOnInit`.
...
- **`route.params.subscribe(...)` in `ngOnInit`** with manual unsubscription. And `route.snapshot.paramMap.get('id')` with no reaction to changes. These are two generations of solving one problem that `withComponentInputBinding` now covers.
- **`data: { roles: ['admin'] }` plus one generic guard** reading `route.data`. The technique works. With functional guards people more often write a factory instead: `canActivate: [hasRole('admin')]`.
- **A custom `RouteReuseStrategy`** with `shouldDetach`/`shouldAttach` for "tabs that keep their state". Angular v22 added an official `destroyDetachedRouteHandle()` for this, because detached views used to leak surprisingly often.

## What we add to the project

Support Desk gets real navigation. That means the ticket list, a detail page by `:id`, a creation form and a lazy admin section gated by role through `canMatch`. Parameters arrive as component inputs, and leaving a half-filled form is intercepted by `canDeactivate`.
...
Note the shape: these routes are flat siblings, not the nested `children` array from the theory section. The detail page is a screen of its own, so it is not a child route of the list. The order rule holds in both forms — the static `tickets/new` is declared before `tickets/:id`.
...
- `/tickets/new` would match `/tickets/:id`, and `id` would receive the string `"new"`. With `transform: numberAttribute` you get `NaN`, so no ticket is found and the screen shows "not found". The bug surfaces as "the create form does not open". The rule: static segments are declared before parameterized ones.
- The component's fields survive: the instance is the same, and neither the constructor nor `ngOnInit` runs again. The `input()` updates, and everything derived from it through `computed` recomputes. Hence the rule: screen state that depends on a parameter must be derived from the input rather than written into a field at initialization.
...
- With a repeated key you get a single value, not an array: the last one in the query string (`'notes'`). The feature reads from a paramMap-like structure, and such a structure keeps one value per key. If you need the array, read `queryParamMap.getAll('tab')` through `ActivatedRoute`.
...
1. Explain in your own words the difference between `canMatch` and `canActivate`. Then give a case where the wrong choice hurts not security but the amount of code downloaded.
...
1. `canMatch` runs while the URL is being matched against routes. Returning `false` marks the route as not applicable, and the router keeps looking, so one path can render different components for different roles. The guard `canActivate` runs after the route has been chosen, and `false` there cancels navigation entirely. The code-size case is the lazy admin section. Guarding it with `canActivate` means the router picks the route and **downloads the chunk** first, then checks access. An ordinary user fetches code they will never see. With `canMatch` the route is rejected before loading, and the chunk is never requested.
2. The router reuses the component instance when the route is the same and only the parameter changed. The component is neither destroyed nor recreated, so the constructor and `ngOnInit` do not run. The reaction must be a dependency rather than a hook. Take the parameter as an `input()` (with `withComponentInputBinding`) and derive everything else from it with `computed`. Then a change of `:id` updates the input and dependent computations refresh automatically. The snapshot approach — "read it in `ngOnInit` and store it in a field" — keeps showing stale data instead.
...
4. A resolver is justified when something genuinely must be ready **before** rendering and affects navigation itself. Examples: verifying an entity exists and redirecting to 404, supplying a title, loading a section's configuration. There are two downsides. First, navigation "hangs". The user stays on the old page with no indication until the resolver finishes, and the slower the network the more it feels frozen. Second, an error in the resolver breaks navigation completely. Loading or error state cannot be shown inside the screen's UI, because it lives outside the component. The screen's own data is better fetched in the component (`httpResource`, chapter 08), where `loading` and `error` are part of the UI.
5. The router continues matching and, finding nothing, reaches `'**'`. The user sees a "not found" page while the URL stays as requested. With `canActivate: false` navigation is cancelled. The URL does not change at all and the user remains on the current screen, unless the guard returned an explicit redirect. Hence the practical choice. "This section does not exist for you" is `canMatch`. "The section exists but you must log in" is `canActivate` with a redirect to the login page.

</details>

## Common mistake

The first one is reading the parameter from `snapshot` inside `ngOnInit`. The code looks impeccable and works on first open: `const id = this.route.snapshot.paramMap.get('id')`, then fetch. The problem appears when the user moves from one ticket to another from inside the app — through a "next" link, say.

The router reuses the component and `ngOnInit` never runs. The `snapshot` stays as it was, so the screen keeps showing the previous ticket while the address bar shows the new URL. The bug feels mysterious because a page reload "fixes" it.

The right approach is not a one-off read but a dependency. Use `input()` with `withComponentInputBinding` plus a `computed` derived from it, and the update arrives on its own.

The second is using `canActivate` where `canMatch` belongs, for lazy sections. Access-wise both work: a user without the role never sees the admin screen. But with `canActivate` the router picks the route and **downloads the admin chunk** before it asks the guard. Code the user is not allowed to run still ends up in their browser.

Besides the wasted traffic this sends the wrong signal: the contents of the admin section are visible to anyone who opens the Network tab. Changing one line (`canActivate` → `canMatch`) removes both the download and the leak of the section's structure. Neither guard is a security boundary, though: the server must authorise every request on its own.

The trade-off is that `canMatch: false` leads to a 404 rather than "access denied". So when the user deserves an explanation, guards are combined: `canMatch` for feature flags, `canActivate` with a redirect for authentication.
