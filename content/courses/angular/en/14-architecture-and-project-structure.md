...
- **No `utils.ts`, `helpers.ts` or `common.ts`** — the guide explicitly warns against such names, because they describe nothing and turn into junk drawers.
...
```
                        Why core/shared turns into a dumping ground
┌───────────────────┬─────────────────────────────────────┬────────────────────────────────┐
...
`CoreModule` + `SharedModule` is a pattern from the NgModules era, where it had a technical justification: `CoreModule` was imported once into `AppModule` (singletons), `SharedModule` into every feature module (components and pipes). With standalone components the justification is gone: providers are configured by `provide*` functions and components are imported individually.

The problem is not the names but the fact that "shared" is an invitation to put anything there. A working replacement: **`core/` holds only what the application cannot start without** (config, auth, interceptors, error handling), **`ui/` holds components that know nothing about the domain** (a table, a modal, a button), and everything else lives inside its feature. The membership test for `ui/`: if a component imports a domain model, it is not `ui/`.
...
In plain Angular these boundaries rest on discipline and code review; they can be automated with the ESLint rule `import/no-restricted-paths` or `eslint-plugin-boundaries`. In a monorepo it is done systematically — with tags and `@nx/enforce-module-boundaries`, where the linter knows the dependency graph; see the Nx course, the chapter on library architecture and module boundaries. The connection matters: **folders in one application and libraries in a monorepo solve the same problem**, except a monorepo gives the boundary a mechanical check.
...
But there is a nuance React does not have: DI makes "smartness" cheap — any component can inject a store in one line. So the boundary erodes on its own and has to be maintained deliberately. A pragmatic compromise: presentational components in `ui/` stay strictly on inputs and outputs; components inside a feature may inject their own feature's store (that does not make them unreusable, because they are not needed outside the feature anyway).

### Barrel files and circular imports

An `index.ts` re-exporting a folder's contents looks tidy and creates two problems. First, **cycles**: `tickets/index.ts` exports `ticket-store.ts`, which imports `ticket-api.ts`, which imports a type from `tickets/index.ts` — a cycle that at runtime yields `undefined` in the most unexpected places (the classic symptom is `NG0201: No provider for undefined` from chapter 04). Second, **extra code in the bundle**: importing one symbol drags in the whole barrel, and if tree-shaking cannot help (side effects in a module), everything ships in the chunk.

Practice: import by direct paths inside the application and introduce a barrel only at a genuine boundary — when a feature really is a library with a public API (in a monorepo that is the library's entry point).

### Legacy: NgModules and the migration path

An NgModules project is not rewritten in one go — there is an official schematic with three modes to be run in sequence, verifying the application between steps:
...
The order matters, and after every step you must confirm the app still works. A mixed state is normal during the migration: a standalone component can be imported into an NgModule (through `imports`), and an NgModule can be used by a standalone component (also through `imports`). That is what makes feature-by-feature progress possible.

On the order of work in a real project: standalone first (it unblocks everything else), then control flow (`ng generate @angular/core:control-flow`), then signal inputs and queries, and only then zoneless — because the last one requires state to already live in signals (chapter 03).

### A checklist for reading an unfamiliar project

```
                              Checklist: how to read an unfamiliar Angular project
┌──────────────────────────────────┬────────────────────────────────────────┬──────────────────────────────────┐
...
- **Structure is a convention, not a freedom.** In React the structure is decided by the team and the metaframework (`app/` in Next.js dictates routing). In Angular routing is not tied to folders, so the discipline is entirely yours — but Angular has an official style guide with concrete rules, a rare case where "the right way" is documented.
- **`core/` means providers, not "context providers".** In React shared dependencies usually take the shape of a tree of contexts around the app. In Angular their equivalent is `provide*` functions in `app.config.ts` (chapter 04), so the "global layer" is a flat list of providers rather than wrapper components.
...
- **Barrel files: the same problem, different consequences.** An `index.ts` creates cycles and hurts tree-shaking in React projects too. But in Angular a cycle additionally breaks DI: a provider receives `undefined`, and the error reads "No provider for undefined" instead of a sensible message about imports.
...
- **`CoreModule` and `SharedModule`** with a comment saying "import CoreModule only in AppModule", sometimes with a constructor guard (`if (parentModule) throw new Error(...)`). With standalone components that is an archaism.
- **Organization by type:** `app/components/`, `app/services/`, `app/models/`, `app/pipes/` — the directories the style guide explicitly discourages. The tell: understanding one feature requires opening four folders.
- **A `SharedModule` dragging in half the application:** it imports `CommonModule`, `FormsModule`, Material and 30 components, exports everything, and every lazy module pulls it whole — a classic cause of bloated chunks (chapter 12).
...
- **Barrels on every folder** with `import { X } from '../..'` — fertile ground for cycles; sometimes accompanied by `// eslint-disable-next-line import/no-cycle`, which means "we know and did not fix it".
- **`environment.ts`/`environment.prod.ts` with `fileReplacements`** as the only configuration mechanism (chapter 04 offers a token), and `assets/` instead of `public/` (renamed in new projects).

## What we add to the project

Support Desk is reorganized by feature: `core/` shrinks to what is genuinely shared, `ui/` is cleared of domain knowledge, everything ticket-related gathers in `tickets/`, an ESLint rule forbids importing one feature's internals from another, and a project checklist is written down.
...
├── core/
│   ├── app-config.ts            # InjectionToken + provideAppConfig (chapter 04)
...
│   └── navigation-progress.ts   # the navigation indicator (chapter 09)
├── ui/
│   ├── data-table/              # generic, knows no domain (chapter 11)
│   ├── modal/
│   └── priority-picker/         # CAREFUL: it knows TicketPriority → see below
└── tickets/
    ├── ticket.ts                # the model: the feature's public part
    ├── ticket-api.ts            # public: other features go through it
    ├── ticket-store.ts          # public
    ├── ticket-rules.ts          # the multi token of rules (chapter 04)
    ├── sla-remaining-pipe.ts
    ├── overdue-directive.ts
    ├── ticket-list/             # a screen: ts + html + css + spec + harness
    ├── ticket-detail/
    └── ticket-form/
```
      ...
      patterns: [
        {
          ...
          group: ['**/tickets/**', '!**/tickets/ticket.ts', '!**/tickets/ticket-api.ts', '!**/tickets/ticket-store.ts'],
          message:
...
The rule is cheap, but it is what turns an agreement into a checkable constraint. In a monorepo the same role belongs to `@nx/enforce-module-boundaries` with tags — there the boundary is a library rather than a folder, and the linter reasons over the dependency graph (the Nx course, the chapter on library architecture and module boundaries).
...
- `ui/` — reusable components with NO domain knowledge. Importing from features is forbidden.
...
- `TicketCard` knows about `Ticket`, so it cannot live in `ui/`. Its proper place is `tickets/`, and the admin section may use it **only** if `ticket-card` is declared part of the feature's public contract (that is, it sits at the top level of `tickets/` rather than inside a screen folder). The alternative, when a card is needed across domains: split it into a presentational `ui/entity-card` (title, badge, slots through `ng-content` — chapter 11) and a domain `tickets/ticket-card` that configures it.
- If the function formats a **date**, it belongs in `ui/` or `core/` (a shared presentation utility) but in a file with a descriptive name (`format-relative-time.ts`, not `utils.ts`). If it formats a **ticket's SLA**, it belongs in `tickets/`, and the second feature that wants it should either go through the feature's public contract or have its own: identical code does not always mean identical meaning.
...
- It manifests as `undefined` where a class was expected: during module loading one of them is not initialized yet, so the provider receives `undefined` instead of `TicketApi` — and the error reads `NG0201: No provider for undefined` or "Cannot read properties of undefined (reading 'ɵprov')", in a file unrelated to the cycle. The fix is replacing barrel imports with direct ones.
- In a monorepo `Ticket` becomes a contracts library (`libs/shared/api-types`) that both the frontend and the backend depend on: a contract change becomes visible in the graph, and the linter prevents the frontend from importing server code. That is precisely the scenario monorepos are adopted for — see the Nx course (the chapters on library architecture and the fullstack link).
...
1. Because code cohesion follows the domain, not the technical type. With type-based organization one feature is smeared across four folders: adding a field to a ticket means opening `models/`, `services/`, `components/` and `pipes/` — and it becomes impossible to see what belongs to the feature and what does not. Three things break: deleting a feature (its remains live in every folder), boundaries (importing "your" service and someone else's looks identical, and no rule can tell them apart) and lazy loading (one feature's code is interleaved with another's, making chunk splitting harder). Feature-based organization solves all three: a feature is a folder you can read whole, extract into a library, or delete.
2. `CoreModule` existed because in the module era providers were registered in modules: to keep singletons single, the module holding them was imported only into `AppModule` (hence the guard against a second import). `SharedModule` existed because components and pipes had to be declared and exported by a module and imported by every feature module. Standalone components removed both reasons: providers are configured by `provide*` functions in `app.config.ts` or on a route, and components are imported individually into the `imports` of whoever uses them. What remains is organizational separation — and that is better expressed by `core/` and `ui/` folders with explicit rules than by modules.
3. The test is mechanical: a component in `ui/` **imports nothing from features** — no model, no service, no type. It is parameterized by generics, inputs and slots (`ng-content`). The borderline case from the project is `PriorityPicker`: it imports `TicketPriority`, so formally it is not `ui/`. Two options: keep it in `tickets/` (it is about the ticket domain and meaningless outside it) or generalize it into `OptionPicker<T extends string>` receiving the options as an input — then it lives in `ui/` legitimately. The wrong third option is keeping it in `ui/` with the domain import: `ui/` then stops being a reusable layer and turns into yet another `shared/`.
4. Two ways. First, cycles: a barrel re-exports files that import each other through the same barrel, and the cycle closes unnoticed. In Angular that is especially nasty because DI suffers: at the moment the provider is evaluated the class is still `undefined`, and the error reads `NG0201: No provider for undefined` or a crash mentioning `ɵprov` — in a file unrelated to the cycle, which makes diagnosis expensive. Second, the bundle: importing one symbol pulls the whole barrel, and if any module has side effects, tree-shaking cannot save you — extra code ships in the chunk. Hence the practice: direct paths inside the application, barrels only at genuine public boundaries.
5. The order: (1) the standalone migration with `ng generate @angular/core:standalone` in three passes (`convert-to-standalone` → `prune-ng-modules` → `standalone-bootstrap`), verifying the app between steps; (2) the new control flow (`ng generate @angular/core:control-flow`); (3) signal inputs and queries, moving state onto signals; (4) only then zoneless. The reason for that order: standalone unblocks everything else (functional providers, lazy `loadComponent`, dropping `SharedModule`), while zoneless comes last because it requires changes to be announced explicitly — that is, state must already be in signals, or the code must call `markForCheck`. Turning zoneless on over code full of plain fields and mutations gives you an application that stops updating in unpredictable places (chapter 03).

</details>

## Common mistake

The first is "architecture first". A developer arriving from React with large-app experience starts with scaffolding: `core/`, `shared/`, `features/`, `components/services/models` inside every feature, barrels at every level, abstract base classes "for later". In a three-screen application that produces a structure with more plumbing than code, where any change requires edits in four places. It is cheaper to start flat — a feature is a folder, files sit next to each other — and introduce a layer once the second or third feature and real duplication appear. The official style guide backs exactly that path: group by feature, one concept per file, no folders by code type.

The second is a `shared/` growing faster than the application. It starts innocently: a component is needed in two places, so it moves to `shared/components`. Then `utils.ts` moves in, then `models/`, then a service that "seems shared". Six months later `shared/` knows every domain, every feature depends on it wholesale, and lazy chunks drag in half the application (chapter 12). The sign to check in review is simple: **a file in the shared layer imports something from a feature**. If it does, it is not shared, and it belongs either in the feature or, generalized, in `ui/`. And a useful habit from the style guide: never create files named `utils.ts`/`helpers.ts`/`common.ts` — a name that describes nothing guarantees that everything ends up inside.
