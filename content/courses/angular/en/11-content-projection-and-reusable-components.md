...
Markup is handed into someone else's component in three ways. They differ in when the content is created and in whether you can pass data into it.
...
The docs explicitly forbid wrapping `ng-content` in `@if`/`@for`/`@switch`: the content is created regardless, and the condition merely hides it.

This is the first fork people get wrong. Projection through `ng-content` looks like the universal answer, but it has two hard limits. The content is **always created**, and it cannot receive data from the host component. As soon as you need a row per ticket, or content created only when the dialog opens, `ng-content` no longer fits. You need an `ng-template` or dynamic creation.

### ng-content and slots

The consumer writes the markup once, and `select` decides which slot each piece lands in.
...
└─────────────────────────────────────┘
 ...
 without reshaping it for the selector
    ...
    where it lands in the panel template
┌──────────────────────────────────────────┐
│ <header>                                 │
...
- Projected content belongs to the **consumer**. Its expressions are evaluated in the parent's context, and its styles are the parent's (chapter 01). It sees the host's `providers` but not its `viewProviders` (chapter 04).
- The documentation states the limitation outright. DOM (document object model — the page's live tree of elements) nodes for projected content are always created, even when the slot is hidden. Word for word: "Angular always instantiates and creates DOM nodes for content rendered to a `<ng-content>` placeholder, even if that `<ng-content>` placeholder is hidden". A second rule follows it: "You should not conditionally include `<ng-content>` with `@if`, `@for`, or `@switch`". Hiding it is possible; not creating it is not.
...
Destruction is `ref.destroy()` or `viewContainerRef.clear()`. There is also the standalone `createComponent()` function from `@angular/core`, which needs an `EnvironmentInjector`. Use it when there is no convenient `ViewContainerRef` — when rendering into an arbitrary DOM node, say.

Note two important details. A component created in a `ViewContainerRef` joins that container's **logical tree** — its change detection and its DI (dependency injection, chapter 04). Physically it is inserted as a sibling of the anchor element, not inside it. And `setInput()` is the only correct way to pass inputs to a dynamic component.

### Angular CDK and @angular/aria

The CDK (Component Dev Kit) is Angular Material without the styles: the behaviour and primitives Material itself is built on. Here is what it holds.

| the task | the CDK primitive |
|---|---|
| a floating layer with positioning | `overlay` |
| render a template or component elsewhere | `portal` |
...
The `@angular/aria` package (stable since v22) goes further and gives ready behaviour directives: `accordion`, `combobox`, `grid`, `listbox`, `menu`, `tabs`, `toolbar`, `tree`.

The practical rule: **writing your own is justified for markup and styles, but not for behaviour that is easy to get wrong**. That means positioning floating layers within the viewport, trapping focus in a dialog, announcing changes to screen readers, and virtual scrolling. In those tasks a home-made version is almost always worse, and `@angular/aria` also covers keyboard navigation for the common patterns.
...
- **A dynamic component.** In React `const C = map[type]; return <C/>` is enough — a component is a value. In Angular you need `ViewContainerRef.createComponent()` and `setInput()`. A component is not a function but a description with metadata, and creating it goes through DI and change detection.
- **Where the habit breaks:** expecting content passed into a component to "belong" to it. It belongs to the parent. Its expressions evaluate in the parent's context, and its styles are scoped by the parent. It resolves services from the host's `providers` but not from its `viewProviders`. That is why "my content cannot see the component's service" is not a bug but the `providers`/`viewProviders` distinction.
...
Two reusable components. The `DataTable` describes its columns through templates supplied by the consumer — the render-props analogue. The `Modal` is created dynamically, with a focus trap and Escape handling. Both must work with any data, not only with tickets.
...
What the CDK would have covered here:

- `cdk/overlay` — creating the layer, positioning and the backdrop;
- `cdk/a11y` (`FocusTrap`, `cdkTrapFocus`) — keeping focus inside the dialog, which this solution lacks;
- `cdk/portal` — inserting a template or component into the layer;
- `cdk/dialog` — all of the above plus roles and focus restoration.

A home-made version makes sense here as an exercise. In a project with more than one dialog, `cdk/dialog` is cheaper.
...
- It depends on which `createComponent()` you used. A component created through a `ViewContainerRef` is destroyed together with the view that holds the container, so a route change does clean it up. The standalone `createComponent()` from `@angular/core` belongs to nobody, and there `destroy()` is mandatory. The leak the walkthrough guards against is a different one. A `close()` that only hides the modal leaves the instance alive while the modal itself lives on, and its `effect`s, subscriptions and `httpResource` keep running. Hence `destroy()` in `close()` plus the `DestroyRef` safety net in the constructor.
...
1. Because `<ng-content>` is not an element you can create or skip but a compiler instruction about where to place content that already exists. The documentation says it plainly. Nodes for projected content are always created, even when the placeholder is hidden. And `ng-content` should not be wrapped in `@if`/`@for`/`@switch`. The condition merely hides the result, while constructors, `effect`s and requests have already run. For genuine laziness the content is passed as an `ng-template` and inserted through `ngTemplateOutlet` at the right moment. A `ViewContainerRef` does the same job. And when the goal is deferring the code itself, `@defer` is the tool (chapter 12).
2. `ng-content` relocates ready-made markup with no way to pass anything into it: its expressions evaluate in the parent's context. But `ng-template` passes a **description** of markup that the component inserts itself. Along with it goes a context: an object whose fields land in `let` variables, with `$implicit` going to the first unnamed one. This is the direct analogue of render props (`renderRow={(row) => …}`). The consumer describes how to draw a row, and the component decides how many times and with what data. The difference from React is that what is passed is a compiled template rather than a function, so the context is typed through `ngTemplateContextGuard`.
3. The parent. The content is declared in the parent's template, and that has three consequences. First: expressions inside evaluate in the parent's context and are checked when **its** template is checked. The host cannot inject its own values there except through an `ng-template` with a context. Second: styles are scoped by the parent (the parent component's `_ngcontent` attribute), so the host's CSS does not apply. Third: in DI the content sees the host's `providers` but **not** its `viewProviders`. It also follows that the content's lifecycle is tied to the parent, and the host's `ngAfterContentInit` fires after it has been created.
4. The `setInput()` call does two things: it writes the value, and it marks the component as changed and schedules a check. That is exactly what a `[x]="…"` binding in a template would do. A direct `ref.instance.title = 'x'` changes the field, but the framework never learns about it. With an `OnPush` component — the default since v22 — the template is not re-checked, and the old value stays on screen. Writing to the instance also bypasses input `transform`s and, in older code, `ngOnChanges`. Practically: `setInput()` is the only correct way; a manual write requires `markForCheck`/`detectChanges` and still breaks the input contract.
5. Delegate the behaviour that is easy to get wrong. The `overlay` package gives the layer, positioning relative to the trigger with viewport edges and scrolling accounted for, and the backdrop. The `a11y` package gives a focus trap, `FocusMonitor` and screen-reader announcements. The `listbox` and `menu` packages give keyboard navigation, `aria-activedescendant` and typed selection. If you like, `@angular/aria` provides ready `combobox`/`listbox` directives. Write yourself the markup, styles, animations and domain logic. That means what an option looks like, how the list is filtered, where the data comes from and what counts as selected. The boundary is simple. Anything describable as correct behaviour per the standard comes from a library. Anything describable as how this looks and what it means in our product you write.

</details>

## Common mistake

The first is trying to get lazy content through `@if` around `<ng-content>`. The React model suggests `{isOpen && children}` does not create the children while the condition is false. In Angular the content is created regardless, and the condition only hides the DOM.

The symptom is distinctive. The modal is closed, yet the form inside has already requested its reference data and its `effect`s are running. The Network tab shows calls the user never triggered. Sometimes it is caught even earlier, by a console error from a component that should not exist yet.

The right path: content that must appear on demand is passed as an `ng-template` and inserted with `ngTemplateOutlet`/`ViewContainerRef` at open time. Deferring the *code* itself is `@defer`'s job (chapter 12).

The second is a dynamic component without `destroy()`. You created it with `createComponent()`, showed it, closed the modal by "hiding" it — and left the instance alive. It remains part of change detection, its subscriptions and `httpResource` keep working, and its polling keeps hitting the network. Reopening puts a second copy next to it.

The leak accumulates quietly: the application simply grows heavier. The rule: every `createComponent()` has a matching `destroy()`, or a `clear()` on the container. And it is worth adding a `DestroyRef.onDestroy()` safety net, as in the walkthrough.

The same principle explains why `@if` beats "hiding with CSS": removing a view destroys the components inside it along with all their state.
