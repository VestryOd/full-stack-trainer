...
Change detection (CD) answers one question. When should the template's `update` function run and sync the DOM (document object model — the page's live tree of elements)? What gets checked is not "a component" but a **view**: a component's template plus the embedded views created by `@if`, `@for` and `@defer`. Class instances are never recreated in the process — only the binding update code runs.
...
```
App                             traversed: leads to a dirty view
├── AppHeader                   skipped: OnPush, inputs unchanged
├── TicketList                  checked: a signal in it changed
│   ├── TicketFilters           skipped: inputs unchanged
│   ├── TicketCard #101         checked: the selected input changed
│   └── TicketCard #102 … #106  skipped: same inputs
└── AppFooter                   skipped
```

### The `OnPush` and `Eager` strategies
...
- at least one of its inputs changed — Angular compares the previous binding value with the new one. The comparison is shallow, so for objects it is a reference comparison;
...
One case people routinely forget. Write to a child component's fields directly through `viewChild`, and no inputs changed and no event happened. That component needs an explicit `markForCheck()`.

### How it used to work: zone.js

Historically Angular had no explicit notifications and solved the problem differently. `zone.js` patched every asynchronous browser API — `setTimeout`, `addEventListener`, `Promise`, `XHR`. Then it told the framework: an async operation finished, application state *might* have changed. When the microtask queue drained, Angular ran `tick()` — a walk of the entire tree.
...
| an `emit` picked up by `AsyncPipe` | the zone plus `markForCheck` in the pipe | `markForCheck` in the pipe |
...
- Updating state from an unpatched source does not update the DOM. Examples: a `setTimeout` writing a plain field, a third-party library callback, a manual subscription without `AsyncPipe`.
...
- For SSR (server-side rendering) and tests, stability is now expressed through `PendingTasks`. Use `inject(PendingTasks).run(async () => …)` or the `pendingUntilEvent()` helper, which keeps the application unstable until the Observable emits or completes.

### Diagnosis

Every symptom below has one cause and one instrument that shows it:

| symptom | cause | tool |
|---|---|---|
| fresh data, stale DOM | the source never notified Angular | `provideCheckNoChangesConfig` |
| a click anywhere fixes the view | same cause; the click forces a walk | the same, plus the DevTools profiler |
| more checks than changes | an `Eager` component, a method call | the DevTools profiler |
...
`NG0100` exists in dev only. Production runs no second check, so the inconsistency stays invisible.

Three tools worth knowing:

1. **`provideCheckNoChangesConfig({ interval, exhaustive: true })`** — a periodic dev check. Angular re-verifies bindings and throws `ExpressionChangedAfterItHasBeenCheckedError` when it finds a value that changed without a notification. With `exhaustive: true` it treats all views as `Eager`. It is the way to catch exactly "the data changed but nobody told Angular" — the class of bug that zoneless makes silent.
...
`afterRenderEffect` is stable and has four phases. They run in the order below, and only when the effect is dirty:

- **`earlyRead`** — read the layout that a later `write` needs. The escape hatch for "I have to measure before I can decide what to write".
- **`write`** — mutate the DOM: styles, classes, sizes, scroll position.
- **`mixedReadWrite`** — read and write in one phase. This cancels the guarantee below, so treat it as a last resort.
- **`read`** — read the final layout, once every writer has finished.

The return value of one phase is passed into the next, so `earlyRead` can hand a measured rectangle to `write`.

The split exists to prevent layout thrashing. Reading a geometric property such as `offsetHeight` or `getBoundingClientRect` forces the browser to apply pending style changes and recompute the layout. Interleaving reads and writes across several components makes it recompute on every switch.

Grouping all reads before all writes gives at most one layout pass per render, however many components take part. That is also why the documentation advises avoiding `mixedReadWrite` and `earlyRead`: both weaken the grouping.
...
- **`memo` versus `OnPush`.** `React.memo` is a wrapper that compares props and may skip a re-render. `OnPush` is a property of the component itself, and it affects the entire subtree. Both comparisons are shallow, hence the identical symptom: pass a mutated object and no update happens.
...
- **Where the habit breaks:** `await fetch(...)` → assign a field → "in React this would update". In zoneless Angular nothing happens, because there was no notification. And in the other direction. JSX is React's markup syntax embedded in JavaScript, and computation inside it is cheap — so a React developer brings methods into the template. There they run on every check, and the number of checks is decided by CD, not by the developer.
...
- **`ChangeDetectorRef` in the constructor:** `private cdr = inject(ChangeDetectorRef)` with a `cdr.markForCheck()` after every async operation. Sometimes it is `cdr.detectChanges()`, meaning check right now, or `detach()`/`reattach()` for manual control. With signals this stays only for integrating non-reactive code.
- **`setTimeout(() => …, 0)` as a way to make it update**, plus `ApplicationRef.tick()` called from a service. Both patterns mean the author never found the real cause of the missed check.
...
**Output:** a working counter fed by an external source, dev diagnostics enabled, auto-scroll to the selected card. Plus an understanding of why each piece is built the way it is.
...
6. Add a component with `changeDetection: ChangeDetectionStrategy.Eager` — say, an `AppFooter` showing the current time. Then type in the search field and use the Angular DevTools Profiler to compare its check count with the `OnPush` components. Finally move it to `OnPush` and achieve the same behaviour correctly.
...
The difference is fundamental. `markForCheck()` marks **one specific component** — the one whose `ChangeDetectorRef` was injected — plus the path to the root.

That is why it is the wrong tool for a service. The service does not know who reads it, and a `ChangeDetectorRef` in a root service does not point where you would want.

A signal marks **all of its readers**, wherever they are: one source, any number of consumers, no references to components at all.
...
In the Profiler you can clearly see this component joining every cycle while you type in the search box. Its data depends on nothing. The correct version is `OnPush` (that is, simply dropping the `changeDetection` field) plus a signal if the time genuinely needs to update:
...
- Mutating a field of the object leaves the object reference unchanged. So the input counts as unchanged, and the `OnPush` child is not checked. That lasts until an event happens in its subtree, or until something marks it another way. If the object holds a **signal** and the child reads it in its template, the update does arrive. The notification then comes from the signal, not from the input comparison. That is in fact a usable technique: pass an object of signals rather than an object you mutate.
- `NG0100` is a dev-mode mechanism: only there is the template checked twice and the results compared. Production has no second check, so the error never fires. The inconsistency remains, though: the user sees a value that is one cycle behind, or no update at all. Production is not "working better", it is just quiet.
...
1. `markForCheck()` marks the view its injected `ChangeDetectorRef` belongs to as dirty. It also marks every ancestor of that view as needing traversal, building a path from the root to the component. At the next synchronization Angular walks down the marked path and checks the marked component; branches the marking never reached are not visited. That is why it is a cheap operation: it does not trigger a check of the whole tree, it only adds one route.
2. `zone.js` patched the browser's async APIs and told Angular that an async operation had finished and state *might* have changed. A full tree walk followed. Two properties pushed the framework away from it.

   - **Redundancy.** The zone cannot know whether anything really changed, so checks ran far more often than needed.
   - **Cost and opacity.** An extra bundle package, patches around platform APIs, bloated stacks, and `async/await` behaving differently depending on the build target.

   The move became possible together with signals because the zone was an *automatic* notification source. Removing it and keeping only manual `markForCheck` would have pushed that work into every callback. A signal notifies by itself and knows its readers. It replaced the zone as the automatic mechanism, but with precision: only what actually depends on the changed value is touched.
3. Three ways:

   - **Replace the mutation with a new object.** The input comparison then sees a new reference, and the child is checked.
   - **Keep the state in a signal** and read it in the child's template. The signal notifies regardless of inputs.
   - **Force the check from outside.** Call `markForCheck()` on the child, reaching it through `viewChild` for example, or give the child the `Eager` strategy.

   The signal is the correct answer, and the new object is the minimal patch: both remove the cause rather than the symptom. Forcing the check is a crutch. It treats the manifestation and leaves mutable state the framework knows nothing about, so the same bug will reappear elsewhere.

4. `NG0100` fires because in dev mode Angular checks a template and then re-checks it, comparing the results. They must match: one synchronization must produce one consistent state. If a value changed between the checks, the code is mutating state *during* the check. The typical sources: writing fields from hooks that run after the check, a getter with a side effect, a state change from `afterNextRender`. The message says your data model is updated in the wrong place. Production runs no second check, so the error disappears — the inconsistency, a value one cycle behind, does not.
5. `effect` runs as part of synchronization, **before** the result reaches the DOM. Measurements inside it therefore observe the previous layout. You get the size of a node that does not exist yet, or a position from before the new styles applied. By contrast, `afterRenderEffect` runs after render and is split into phases: `earlyRead`, `write`, `mixedReadWrite`, `read`. All layout reads then happen separately from all writes, so the browser does not recompute the layout repeatedly. Measuring inside a regular `effect` gives you either stale numbers or a forced layout at an unfortunate moment. At worst it gives a loop: measured → wrote a signal → another check.

</details>

## Common mistake

The first mistake comes straight from React: got the data, assigned the field, the DOM will update. In the zone model that really did work, which is why the internet is full of code shaped exactly like that.

In a zoneless application, assigning a plain field from `setTimeout`, `fetch().then()` or a third-party callback achieves nothing. There was no notification, so the template is never checked.

The insidious part is that the bug looks intermittent. The next click anywhere triggers a traversal and the value catches up. So the symptom gets reported as "it updates, but with a delay" or "it only updates if I click something".

The right reaction is not to hunt for a place to put `markForCheck`. Ask instead why this state is not a signal. A signal notifies all of its readers itself, and the question of who should trigger the check disappears.

The second mistake is the opposite in sign: fighting "too many checks" somewhere other than the cause. The classic culprits are methods and getters in templates (`{{ getTotal() }}`, `{{ formatDate(x) }}`) and impure pipes. They run on every check of the template, and how many checks there are is decided by CD.

React experience suggests the wrong intuition here. In JSX a call inside the markup runs once per render, and its cost is visible. In Angular the same code runs as many times as the component is checked — and before you open the Profiler, that is invisible.

The rule is simple: templates contain signal reads, `computed` values and pure pipes; anything that computes, computes in the class. And since `OnPush` is now the default, an explicit `Eager` in a decorator deserves to be read as a red flag. Nearly always it was left there by a migration rather than chosen deliberately.
