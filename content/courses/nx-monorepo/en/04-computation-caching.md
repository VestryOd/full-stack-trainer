...
- **`default`** — all project files + `sharedGlobals`. Those are global files that affect everything: the root `tsconfig.base.json`, or a lock file of the CI environment. CI is continuous integration: the automatic run of builds and tests on a server.
...
**Outputs** declare *where the task writes*: `["{projectRoot}/dist"]`, `["{workspaceRoot}/dist/apps/shell"]`, `["{options.outputPath}"]`. On execution Nx saves those paths into `.nx/cache` along with the terminal output. On a hit it restores the files into place and replays stdout, colors included.

Tasks without file artifacts (lint, typecheck) need no outputs: only the output and exit code are cached — which is all they need.
...
The general diagnostic algorithm has three steps. Reproduce with `--skip-nx-cache`: if the result differs from the hit, the contract is broken. Then figure out which input isn't accounted for — `nx show project X` shows the target's final inputs. Then declare it. Turning the cache off for good is capitulation, not a fix.
...
- `grep -rn "process.env" apps/*/vite.config.* webpack.config.*` (and equivalents) — find every variable the build reads. Each one must either be in the matching target's inputs or provably not affect the artifact. This is the highest-yield audit for cache "lies".
...
We set up an honest cache for mini-shop. First a `production` set in namedInputs, so that specs don't invalidate builds. Then a banner in shell driven by the `SHOP_BANNER` env variable. We use it to reproduce the classic false cache hit, then repair the contract.
...
3. Render a banner in the shell interface from an environment variable. Pipe `process.env.SHOP_BANNER` through `define` in `vite.config.ts`, then render it in app.tsx.
4. Reproduce the cache lie. Build with `SHOP_BANNER="Sale -20%"`, then build with `SHOP_BANNER="Black Friday"`. Show with a grep over dist that the artifact still contains the old banner on a cache hit.
...
No mystery here. File contents didn't change, the target configuration didn't either, and env variables don't enter the hash by default. From Nx's point of view it's the same task, and it faithfully replayed its result.
...
- Delete `dist/apps/shell` and run build. On a hit Nx restores the artifact from `.nx/cache`, because outputs are stored in the cache in full. The dist folder is not a source of truth but a projection of the cache.
...
2. Specs don't participate in the production artifact: editing them cannot change the build result, so the build hash must not change. The mechanism is namedInputs. There `production` is `default` minus `!*.spec.*`, so `build.inputs = ["production", "^production"]` while `test.inputs = ["default", ...]`. Specs are in default, so test invalidates and build doesn't.
...
4. Something in the hash changes between runs. Code generation may write into projectRoot before the task, and then the input content mutates. A file with a timestamp or another unique value may have got into inputs. A declared env or runtime input may be unstable, e.g. `{"runtime": "date"}`. Or, most prosaically, the target has `cache: false`. Diagnosis: first confirm caching is on with `nx show project`. Then hunt the mutating input: compare the projectRoot state before and after the first run. Uncommitted changes produced by the build will show up in git status.
...
A developer from the single-app world is bitten by a false hit for the first time. The reaction is familiar: the cache can't be trusted. Soon three things happen:

- `--skip-nx-cache` is baked into CI;
- a local alias runs `rm -rf dist .nx`;
- "problematic" targets get `cache: false`.

This is clean-build cargo cult: the problem wasn't the cache but one undeclared input, fixable with one line. A disabled cache deceives no one — but it also saves no one a second, and turns the flagship Nx feature into dead weight.

The correct reflex: a false hit = a hole in the inputs contract. Find the input, declare it, and the cache is honest again.

The symmetric mistake is caching what must not be cached. A classic: the team adds a `deploy` or `db:migrate` target and forgets `"cache": false`. One day the deploy "passes" in 40 milliseconds with a green checkmark, because the hash matched yesterday's.

For side-effect tasks the result is neither files nor stdout but a change in the outside world, and memoization is meaningless there by definition. The rule is simple. Pure functions get cached: build, test, lint, typecheck. Anything that touches the network, a database or production gets an explicit, permanent `cache: false`.
