...
- **`plugins`** — the list of registered plugins with their options. This is not just "installed packages": every entry here means "this plugin scans the repo and *infers* targets from tool configs". The `@nx/vite/plugin` entry is the reason a project with a `vite.config.ts` has `build` and `serve` even though its project.json is empty.
...
You'll also meet `generators` (default generator options — so that every `nx g @nx/react:lib` in this repo gets the same bundler and linter by default), `defaultBase` (for affected) and `release` (versioning/publishing).

### What makes a folder a "project"

Nx doesn't scan "everything": graph nodes come from three sources. A folder becomes a project if it contains a **`project.json`**, or a **`package.json`** (the package-based classic), or if it was "adopted" by a plugin from `nx.json` whose `createNodes` matched a tool config (that's how `@nx/playwright` creates an e2e project from `playwright.config.ts`). Knowing this list removes the main confusion in an unfamiliar repo: "why does `nx show projects` list a project that has no project.json?"
...
The tool that makes these layers visible is **`nx show project shell --web`**: an interactive page with ALL of the project's targets, where every option is annotated with its source (which plugin or file contributed it). Without `--web`, the same command prints the final JSON to the terminal. It answers "what will actually run on `nx build shell`" — and we'll use it in every chapter.

> **Versions.** The names of inferred targets are not Nx constants but plugin options in `nx.json`: `"serveTargetName": "serve"`. In some repos the dev server is `nx serve app`, in others `nx dev app`, because someone (or a different generator version) named the target differently. Before complaining that "the command from the tutorial doesn't work", check the `plugins` block of your repo's nx.json.

### apps/ vs libs/ — why this split

The Nx convention: **thin apps, fat libs** (a rule of thumb: 20% of the code in apps, 80% in libs). An application is a build-and-deploy point: routing, composition, configuration. All the logic, UI components and data access live in libs — even when there's only one app. The reasons are pragmatic: only libs get boundaries (`tags` + module boundaries, chapter 06), only small libs give precise affected results (change `catalog/data-access` — `checkout` tests don't run), and only code extracted into a lib can be reused by a future second app — and this course will have four. The folder names themselves are a convention: `workspaceLayout` in nx.json can change them, and newer presets use `packages/`.
...
- `find . -name project.json -not -path "*/node_modules/*" -exec wc -l {} +` — project.json weight across the whole repo in a second: 10-line files mean crystal, 100+ lines mean explicit executors.
...
4. Run `nx serve shell` (check it in the browser) and `nx build shell`; find the build artifact on disk and the file where its path is configured.
...
> **Versions.** Since Nx 18 the project path is taken as provided (`--directory=apps/shell` — literally that folder); in older versions (`projectNameAndRootFormat=derived`) Nx constructed the path itself from the name and nesting options, and the same command could put the project somewhere else. `nx add` appeared in Nx 17 — before that you installed plugins by hand, and the main risk was installing a version that didn't match the core (chapter 00).
...
│   ├── vite.config.ts        # the source of truth for build/serve/test
...
│       └── app/
│           ├── app.tsx
│           └── app.spec.tsx  # vitest, driven by the same vite.config.ts
...
All five targets are inferred: they were contributed by the plugins that `nx add` registered in nx.json. Note that crystal targets run **the tool's own CLI** (`vite build` — the same command you'd type by hand); Nx wraps it with the graph, the cache and the pipeline — how exactly is chapter 03.
...
- The answer to the renaming edge case: change `"serveTargetName": "serve"` to `"dev"` in this block — and **every** vite project in the repo gets an `nx dev <project>` target. Not a single project file is touched — that's what "targets belong to the plugin, not the project" means.
...
The artifact path is configured not "somewhere in Nx" but in `apps/shell/vite.config.ts` → `build.outDir` — the plugin read it from there and reported it to Nx as the target's output (which matters for caching, chapter 04). Re-running the generator over an existing project fails on a name conflict — generators don't promise idempotency; checking "what would change" is exactly what `--dry-run` is for.
...
1. A plugin from `nx.json` (e.g. `@nx/vite/plugin`), while the graph is being built, scans the repo with its `createNodes`, finds `vite.config.ts`, reads it and *infers* the targets (`build`, `serve`, ...) with the options already filled in. The target exists only in the in-memory graph — it's in none of the project's files; the tool config is the source of truth. That's why editing `vite.config.ts` changes the behaviour of `nx build` without a single change to project.json.
2. Bottom to top: plugin-inferred targets → `targetDefaults` from nx.json on top of them → targeted overrides from project.json (or the `"nx"` key of the project's package.json) on top of that. project.json wins. The merged result is shown by `nx show project <name>` (with `--web` — including the source of every option).
...
4. A `project.json` in the folder, a `package.json` in the folder, or a plugin from nx.json whose `createNodes` matched a tool config in that folder. The third source explains the "extra" projects: e.g. an e2e project created by `@nx/playwright` from a single `playwright.config.ts` — it may have no project.json at all.
5. In nx.json, in the plugin options: `"serveTargetName": "serve"` or `"dev"`. Target names are not constants baked into Nx but configurable options of a specific repo; so the first reflex in someone else's workspace is not to google the "right" command but to check `plugins` in nx.json or run `nx show project <name>`.

</details>

## Common mistake

A developer from the single-app world opens `apps/shell/project.json`, sees five lines with no targets — and concludes that "the configuration is hidden somewhere, Nx is magic". Two bad scenarios follow. First: they find a 2022 article (the pre-crystal world) and write a full `build` target with executor and options into project.json — now the repo has **two sources of truth**: their manual target silently shadows the inferred one, and edits to vite.config.ts partially stop affecting the build. Second: they edit `vite.config.ts` without knowing that the specific option is overridden in `targetDefaults`, and can't understand why nothing changes. The cure is the same: `nx show project shell --web` — see the final configuration and the *source* of every option before touching anything.

A related reflex is creating a new application by copying an existing one's folder ("I'll copy shell and rename it — faster than figuring out generators"). The copied project.json keeps the old name (a conflict when the graph is built), tags and aliases go stale, and half the files the generator would have adapted stay as they were in the donor. A generator run with `--dry-run` prints the full list of what should be created and modified — that's the honest answer to "what makes up a new project".
