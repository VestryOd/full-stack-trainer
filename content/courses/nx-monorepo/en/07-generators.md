...
│ every change goes to a VIRTUAL FS (Tree)          │
└───────────────────────────────────────────────────┘
                          │
                          ▼
┌───────────────────────────────────────────────────┐
...
Two things follow from this architecture for free. First, `--dry-run` doesn't "emulate" — it executes **the same code** and prints the virtual FS diff, just without the flush; a divergence between dry-run and a real run is impossible by construction. Second, generators are unit-testable without a disk: create a Tree in memory, run the function, assert on the contents.
...
The invocation syntax is `nx g <package>:<name>`. What a given generator can do is described by its **schema.json**: option types, required flags, `x-prompt` (the question asked interactively when an option is missing), defaults. To see it without reading JSON: `nx g @nx/react:lib --help`. The third layer of defaults is `generators` in nx.json (chapter 01): options recorded there are applied silently.
...
Chapter 06 ended with conventions: the scope/type structure, tags, non-buildable, vitest, a templated importPath. A convention living in a wiki degrades with every new person: someone forgets the tags, someone picks a different bundler, someone puts the lib in the wrong place. **A local generator is a convention compiled into code**: the correct structure emerges not because everyone read the wiki, but because any other path is longer.

The key technique when writing one is **composition**: don't create the project by hand via `tree.write` — programmatically call the built-in generator (`libraryGenerator` from `@nx/react`) with pinned options, then post-process the result. The built-in generator updates tsconfig.base.json, creates project.json, the eslint and vite configs — and keeps doing it correctly after every Nx upgrade. Your code is responsible only for the delta: domain templates, validations, naming conventions.
...
Everything you need is exported from `@nx/devkit`: `Tree` (read/write/exists/children/delete), `names()` (name canonicalization: `shoppingCart` → fileName `shopping-cart`, className `ShoppingCart`), `generateFiles()` (renders a template folder with `__fileName__` substitutions in paths and `<%= className %>` in contents), `formatFiles()` (prettier over what changed), `readProjectConfiguration`/`updateProjectConfiguration` (work with project.json as an object).
...
- Before moving/renaming/deleting a project by hand — `nx g @nx/workspace:move --project=X --destination=...` and `@nx/workspace:remove`: they update tsconfig paths, imports and configs for you.
...
2. Generate a `feature-lib` generator stub inside it and implement the contract:
   ...
   - **Output:** a lib in `libs/<scope>/feature-<name>` with importPath `@mini-shop/<scope>-feature-<name>`, tags `scope:<scope>,type:feature`, bundler none, vitest — plus a `<Name>Page` component from a template and a re-export in index.ts;
   - **Implementation:** composition with `libraryGenerator` from `@nx/react`; your own files via `generateFiles`.
3. Run with `--dry-run`, then create a real lib: `feature-cart` in the `checkout` scope (create the scope folder beforehand... or think about how the generator should behave with a brand-new scope — decide and justify).
...
> **Versions.** The signatures of the `@nx/plugin` generators themselves changed between majors (name/path positional or as a flag) — check `nx g @nx/plugin:generator --help` for your version. The essence is stable: you get a `generators.json` at the plugin root and a generator folder with four files.
...
export async function featureLibGenerator(tree: Tree, options: FeatureLibGeneratorSchema) {
  ...
  if (!scopes.includes(options.scope)) {
    throw new Error(`Unknown scope "${options.scope}". Existing: ${scopes.join(', ')}`);
  }
...
`__fileName__` in the file name and `<%= className %>` in the contents are substitutions from the object passed to `generateFiles`; the `.template` suffix is stripped (and protects the file from being compiled/linted as part of the plugin itself).

Steps 3–4 — running it. The new-scope decision: **the generator does not create domains silently** — a new domain means new rules in the boundaries matrix (chapter 06); that's a deliberate architectural act, not a scaffolding side effect. So first `mkdir libs/checkout` (and a line in depConstraints), then:
...
it('canonicalizes the feature name', async () => {
  ...
  expect(tree.exists('libs/catalog/feature-shopping-cart/src/lib/shopping-cart-page.tsx')).toBe(true);
  expect(tree.read('libs/catalog/feature-shopping-cart/src/index.ts', 'utf-8')).toContain('ShoppingCartPage');
});
```

Answers to the remaining edge cases:

- A re-run fails inside `libraryGenerator` on the project-name conflict — correct behaviour; generators promise no idempotency (chapter 01), and "updating" an existing lib is a refactoring task, not scaffolding.
...
1. Dry-run and the real run execute the same implementation code: all writes go to the virtual FS (Tree), and the only difference is the final step — print the diff or flush to disk. Divergence is impossible by construction: there's no separate "emulation" that could fall behind reality.
2. We get: a correctly structured project.json, the alias in tsconfig.base.json, eslint/vite/vitest configs, proper project registration — and all of it keeps matching the current Nx version after every upgrade, because the Nx team maintains it. We pay: a dependency on the `libraryGenerator` signature, which can change between majors — after `nx migrate` our generator needs its test run and possibly its options adjusted.
3. `node_modules/<package>/generators.json` — find the generator's name, take the `schema` and `factory` paths; `schema.json` — check the options and their defaults (often "the unexpected" is a silent default, or one from nx.json); the `factory` file — read the implementation and see the actual logic. The same chain as for executors in chapter 03, only the registry is called generators.json.
4. The user will type anything: `shoppingCart`, `Shopping-Cart`, `shopping cart`. `names()` produces canonical forms (fileName `shopping-cart`, className `ShoppingCart`, propertyName `shoppingCart`) — so the file structure and class names stay uniform regardless of who invoked the generator and how. Without it, the naming convention dies with the second user.
5. For the generator: (a) it executes rather than gets read — a convention can't be "forgotten"; (b) validations (our scope check) catch mistakes at creation time, not in review; (c) updating the convention = updating code in one place plus a test, not a "please re-read the wiki" broadcast. The honest argument for the wiki: a generator is code that must be maintained (especially across major Nx upgrades); for a convention applied twice a year, that price may not pay off.

</details>

## Common mistake

A developer writing their first custom generator builds it "from scratch": hand-creates project.json and the vite config via `tree.write`, edits tsconfig.base.json — effectively copying what `@nx/react:lib` does into their own code. It works for a few months; then `nx migrate` changes the config structure (say, the repo moves to inferred targets or a new eslint format), the built-in Nx generator updates itself — while the homemade one keeps generating last year's structure. The right instinct: your generator is a thin wrapper (validations + templates + pinned options) around the built-in one; all "infrastructure" generation happens only through composition.

The second mistake is the orphaned generator. Written with enthusiasm, used for three months; then the repo's structure moves on (a new tag axis, a different test runner), the generator isn't updated — and now it creates libs that immediately fail lint. People conclude "generators don't work" and go back to copy-paste. The cure is disciplinary: the generator has a unit test (so it fails in CI when it falls behind), and any PR that changes the repo's conventions must change the generator too — it's a contract, same as types in an API.
