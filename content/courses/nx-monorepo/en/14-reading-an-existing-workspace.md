...
The final chapter targets the very task this course was built for: you land in an existing Nx monorepo (your work project) and, within an hour or two, build an accurate picture of it. Not "as intended" per the wiki — "as is" per the derived data.
...
The route isn't arbitrary: each layer builds on the previous one. Until you know the version, you don't know if it's crystal or fat configs; until you've seen the graph, the boundary rules won't make sense; until you've read the targets, there's nothing to look for in CI.
...
2. **nx.json.** A `plugins` block — the repo runs on crystal; in parallel, check project.json weight (`find . -name project.json -not -path '*/node_modules/*' -exec wc -l {} + | sort -n | tail`). Read `targetDefaults` (what pipeline is declared) and `namedInputs` (is there a `production` set — chapter 04). Peek at `tasksRunnerOptions` if present — a custom runner means a legacy remote-cache layer.
3. **The graph.** `npx nx graph`: how many projects, do domains read as groups, do cross-domain edges bypass shared. `nx graph --file=graph.json` + the incoming-edge count (the chapter 02 one-liner) — who sits at the repo's center. Also `grep -rn implicitDependencies --include=project.json .` — how many manual edges, and why.
...
6. **Federation** (if present). `find . -name "module-federation.config.*"`: who's the host, who are the remotes, what do they expose (pages, or a scatter of components?), are there shared overrides and why (git blame). remotes.d.ts, or MF 2.0 types?
...
- **Decorative boundaries.** Tags assigned, the rule a placeholder, or lint not a required PR check. The illusion of boundaries from chapter 06; treatment — warn → backlog → error.
- **Microfrontends without the benefit.** module-federation.config exists, but CI builds and deploys everything in one pipeline. The team pays the full MF price (runtime risks, shared negotiation, complexity) and never receives the only benefit — independent deploys. The most expensive find in the catalogue; the honest options: finish building independent deploys, or roll back to build-time composition.
...
- Give every find a marker: 🔴 losing money/safety right now (a write token in the repo, a disabled CI cache), 🟡 growing debt (placeholder boundaries, a frozen version), 🟢 a conscious trade-off (document and close).
...
The mini-shop code doesn't change — it becomes the reference: we run the checklist against it (all the answers are known from chapters 00–13) and calibrate the format before walking into a production repo.
...
4. **Graph:** apps + libs + api; the catalog / checkout / shared / api domains read as folders; the center — shared-ui and shared-api-types (expected for a design system and a contract lib). 🟢
...
7. **Federation:** host shell, remotes catalog/checkout, page-granularity exposes; shared — the Nx default + strictVersion for react (chapter 11); the workspace-lib strategy is written down (compatibility + coordinated deploys via affected). 🟢
...
- Without accusations or "you're doing it all wrong": show the CI config and ask the question in terms of price — "we pay for federation with runtime risks and shared negotiation, yet we deploy everything together; do we want to finish independent deploys or simplify to build-time?" Both answers are legitimate (chapter 09) — only the current "price without the benefit" isn't, and that's a team-level decision, not the auditor's.
- A too-green picture usually means the auditor checked declarations, not facts: the rule exists — but does it run in CI? production is declared — but does a spec edit produce a hit? remotes deploy separately — and roll back separately? Every "yes" from a config should be confirmed by at least one observed run.
...
1. The principle of "derived data over intentions": the README describes what was planned; the tools describe what is. `nx report`, the graph and show project are computed from the repo's actual state and cannot go stale. The wiki is useful afterwards — as a source of "why is it like this?" questions, not as a source of facts; a divergence between the wiki and tool output is itself a find.
2. Because the team carries every MF cost — runtime integration, shared negotiation, version management, N-artifact infrastructure — while the single benefit (independent team release cycles) never materializes: the release is still collective. The ways out: (a) finish building independent deploys (affected -t deploy, canaries, the chapter 11 shared-lib strategy) — if the organizational need is real; (b) roll back to build-time composition — the federation goes away, the apps stay in the monorepo. The choice follows chapter 09's "are there independent teams?", not fashion.
3. `nx show project <name> --web`: the final configuration with every option's source annotated — you can see whether project.json shadowed an inferred target. The hybrid's danger: edits to the tool config (vite.config) get partially ignored because the manual executor wins; behaviour diverges between projects; and during upgrades the migrations update one layer while leaving the other, widening the gap.
...
5. By ROI and dependencies: (1) CI on affected with a correct base — changes the pipeline-time asymptotics and requires nothing; (2) the production set in namedInputs — amplifies both the local cache and the freshly enabled affected CI; (3) boundaries warn→error — stops the *growth* of architectural debt (cheap to start, long to burn down — hence early); (4) the Nx upgrade one major per PR — unlocks modern mechanisms and de-risks every following step; (5) the buildable forest — the most labour-intensive with the least effect, dismantled as touched. The general principle: first what's cheap and changes the slope of the curve, then what's expensive and removes a level.

</details>

## Common mistake

The auditor's first mistake is conducting the audit through interviews and documentation. People sincerely retell the state of two years ago, the wiki describes a target picture that never shipped — and the audit records mythology instead of reality. Anything you can't obtain with a command or read from a config with git blame is a hypothesis, not a fact. The correct route is the reverse: tools first (report → nx.json → graph → show project → eslint → MF configs → CI), people second — now with specific "why is it like this here?" questions, which get answered willingly and precisely, because the question shows you know the context.

The second mistake is opening an epic "clean up the monorepo" PR after the audit: flip boundaries to error, rewrite CI, dismantle the buildable libs and upgrade two majors in one go. Such a PR spends weeks in review, conflicts with all ongoing work and eventually dies — discrediting the very ideas it carried. Finds are fixed one at a time, each in its own PR with a measurable effect ("CI time on a typical PR: 24 min → 9 min"), and disruptive switches are turned on gradually (warn → error, one major at a time). An audit is a map of debt, not a mandate for revolution; the winner is whoever converts it into a dozen small, boring victories.
