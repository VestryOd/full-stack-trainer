...
Three words with a strict meaning in Nx. A **target** is a named *capability* of a project ("shell can build, serve, lint"). A **task** is a concrete run: `project:target`, optionally with a configuration: `shell:build:production`. The full run form is `nx run shell:build`; `nx build shell` is its shorthand — they are identical.

**Configurations** are named option presets inside a target (`production`/`development`), selected with `--configuration=production`. The `--prod` shorthand still works but is considered legacy.
...
- **`nx:run-commands`** — the universal one: runs an arbitrary shell command. So common that it has syntactic sugar — a shorter spelling of the same thing. Writing `"command": "tsc --noEmit"` in a target's configuration is the short form of `{"executor": "nx:run-commands", "options": {"command": "tsc --noEmit"}}`.

Crystal plugins (chapter 01) infer targets in exactly this command form. Shell's inferred `build` is literally `vite build` with `cwd: apps/shell`, where cwd is the directory the command runs in. The philosophy is simple. The tool runs through its own native CLI (command-line interface), just as you would run it by hand. Nx wraps it with the graph, the cache and the pipeline.
...
The `dependsOn` rules are declared in one of two places. Either in `targetDefaults` in nx.json, for all same-named targets in the repo — that is the usual place. Or in a specific project's project.json, surgically. A `targetDefaults` key can be a target name (`"build"`) or an executor (`"@nx/vite:build"`). The executor form is more precise when same-named targets do different things across the repo.
...
The key point: **the task graph is a separate structure**, derived from the project graph. Project graph nodes are projects; task graph nodes are tasks. One project graph yields different task graphs depending on what was requested: `nx build shell` has one task graph, `nx run-many -t test` another.

You can inspect it without running anything: `nx build shell --graph`. A cycle in the task graph — A waits for B, B waits for A — is a run-time error. That is one more reason the `shared-ui ⇄ shell` cycle from chapter 02 is not viable.

Parallelism: Nx runs independent tasks concurrently (`--parallel`, default 3) while respecting topological order — a task starts once everything it depends on has finished.

There is a practical consequence for CI (continuous integration — build and test on every push). The command `nx run-many -t lint,test,build` replaces three sequential stages. Lint and test across projects don't wait for each other, and each build starts as soon as its dependencies are ready.

> **Versions.** Long-running tasks (dev servers) historically broke the pipeline: the task "never finishes", so everything depending on it waits forever. Nx 21 introduced **continuous tasks** (`"continuous": true`), and the pipeline no longer waits for them to complete.

> **Versions.** The microfrontend dev mode is built on continuous tasks (chapter 10), where serving the host pulls up the serve of every remote. In older versions this was worked around with hacks like `run-many` over a manual list of serve tasks.
...
- `npx nx show project <project> --web` — for each target you can see three things. Its command form or executor, its `dependsOn`, and where it was inherited from (targetDefaults / plugin / project.json).
...
- In the repo's CI config, look at how tasks are launched. Using `nx run-many` or `nx affected` is the norm. A sequence of separate `npm run ...` calls per project is a red flag (chapter 13).
...
1. Find out what `nx build shell` physically runs. Locate the target in `nx show project shell`, determine its form (command or executor), then find the `nx:run-commands` code on disk.
...
So even the "magic" inferred build is `nx:run-commands`. Its impl is a plain Node file in `node_modules/nx/src/executors/run-commands/`, and it spawns `vite build` in the project folder. The whole chain is readable with your own eyes.
...
- The `cache: true` went into `targetDefaults`, not project.json. When the libs get their own typecheck targets in chapter 06, they become cacheable automatically, with no copy-paste across projects. The rule: properties shared by all same-named targets live in nx.json; project.json keeps only project specifics.
...
- Declaring `dependsOn: ["typecheck"]` on itself is a cycle in the task graph. Nx refuses to build the plan and fails with an error. The form `["^typecheck"]` is legal: dependencies' typecheck runs first. While the libs have no such target it is a no-op, meaning nothing gets run.
...
1. The project graph is the static map of "who depends on whom" (nodes are projects, edges are imports). The task graph is the plan of a specific run, where nodes are `project:target` tasks. It is built from the project graph by expanding `dependsOn` rules from the requested tasks. Different commands produce different task graphs over the same project graph. For `nx build shell` Nx takes `shell:build`, and the `^build` rule adds the build tasks of its dependencies. It walks recursively down the graph, skipping any dependency without that target.
...
3. Sugar: the configuration normalizes to `{"executor": "nx:run-commands", "options": {"command": "vite build", ...}}`. At run time Nx looks up the run-commands implementation path in `node_modules/nx/executors.json` and loads the impl module. That module spawns a `vite build` child process with the project's `cwd`. It relays the output and the exit code back to Nx, which caches them.
4. Stages are artificial barriers: the whole test stage waits for the slowest lint, even though project X's tests don't depend on project Y's lint. A `run-many` with several targets hands Nx all the tasks at once. Nx executes them with maximum parallelism, honouring only the real dependencies from the task graph. The same work finishes faster, and the ordering is guaranteed by the graph, not by a rigid sequence.
5. The general rule has two halves. What holds for *all* same-named targets in the repo — cacheability, `^build`, shared inputs — goes into `targetDefaults`. What's project-specific — the command itself, special options, a targeted override — goes into project.json. New projects then get correct behaviour automatically, and project.json files stay thin.

</details>

## Common mistake

A developer from the single-app world reproduces the familiar CI in the monorepo. Sequential stages `lint → test → build`, each looping over projects or calling `npm run`. It works, but throws away the main thing: Nx already knows both the order and the allowed parallelism, from the task graph.

Manual stages add artificial barriers: build waits for *all* tests to finish even though its dependencies are ready. Worse, they duplicate the dependency knowledge. When a new lib appears, someone must remember to slot it into the handcrafted order — while `dependsOn` would have picked it up automatically.

The second classic misstep is putting a long-running task into a regular pipeline. You make `e2e` (end-to-end — tests that drive the whole running app) depend on `serve`, and then wonder why the pipeline hangs. A `serve` never finishes, and the pipeline waits precisely for *completion*.

Before Nx 21 this was solved with workarounds: starting the server outside Nx, or wait-on scripts. Newer versions have `"continuous": true` for it, and the Module Federation dev mode is built on it too (chapter 10). If your repo is older — know that a hung `dependsOn: ["serve"]` is not a bug but the wrong tool for the job.
