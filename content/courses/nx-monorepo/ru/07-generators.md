...
Из этой архитектуры бесплатно следуют две вещи. Во-первых, `--dry-run` не "эмулирует" — он выполняет **тот же код** и печатает diff виртуальной ФС, просто без flush; расхождение между dry-run и реальным запуском невозможно по построению. Во-вторых, генераторы тестируются юнитами без диска: создали Tree в памяти, прогнали функцию, проверили содержимое.
...
Синтаксис вызова — `nx g <пакет>:<имя>`. Что умеет конкретный генератор, описано в его **schema.json**: типы опций, обязательность, `x-prompt` (вопрос, который зададут интерактивно, если опция не передана), дефолты. Посмотреть без чтения JSON: `nx g @nx/react:lib --help`. Третий слой дефолтов — `generators` в nx.json (глава 01): записанные там опции подставляются молча.
...
Глава 06 закончилась конвенциями: scope/type-структура, теги, non-buildable, vitest, importPath по шаблону. Конвенция, живущая в вики, деградирует с каждым новым человеком: кто-то забудет теги, кто-то выберет другой bundler, кто-то положит либу не туда. **Локальный генератор — это конвенция, скомпилированная в код**: правильная структура получается не потому, что все прочитали вики, а потому что другой путь длиннее.

Ключевой приём при написании — **композиция**: не создавать проект руками через `tree.write`, а программно вызвать встроенный генератор (`libraryGenerator` из `@nx/react`) с зафиксированными опциями и доработать результат. Встроенный генератор обновит tsconfig.base.json, создаст project.json, eslint- и vite-конфиги — и продолжит это делать правильно после каждого обновления Nx. Ваш код отвечает только за прибавку: доменные шаблоны, валидации, конвенции имён.
...
Всё нужное экспортируется из `@nx/devkit`: `Tree` (read/write/exists/children/delete), `names()` (каноникализация имени: `shoppingCart` → fileName `shopping-cart`, className `ShoppingCart`), `generateFiles()` (рендер папки шаблонов с подстановками `__fileName__` в путях и `<%= className %>` в содержимом), `formatFiles()` (prettier по изменённому), `readProjectConfiguration`/`updateProjectConfiguration` (работа с project.json как с объектом).
...
- Прежде чем руками двигать/переименовывать/удалять проект — `nx g @nx/workspace:move --project=X --destination=...` и `@nx/workspace:remove`: они обновят tsconfig paths, импорты и конфиги за вас.
...
2. Сгенерировать в нём заготовку генератора `feature-lib` и реализовать контракт:
   ...
   - **Выход:** либа в `libs/<scope>/feature-<name>` с importPath `@mini-shop/<scope>-feature-<name>`, тегами `scope:<scope>,type:feature`, bundler none, vitest — плюс компонент `<Name>Page` из шаблона и реэкспорт в index.ts;
   - **Реализация:** композиция с `libraryGenerator` из `@nx/react`, свои файлы — через `generateFiles`.
3. Прогнать `--dry-run`, затем создать боевую либу: `feature-cart` в scope `checkout` (папку scope создать заранее... или подумать, как генератор должен себя вести с новым scope — решите и обоснуйте).
...
> **Версии.** Сигнатуры самих `@nx/plugin`-генераторов менялись между мажорами (имя/путь позиционно или флагом) — сверьтесь с `nx g @nx/plugin:generator --help` своей версии. Суть неизменна: получаем `generators.json` в корне плагина и папку генератора с четырьмя файлами.
...
export async function featureLibGenerator(tree: Tree, options: FeatureLibGeneratorSchema) {
  ...
  if (!scopes.includes(options.scope)) {
    throw new Error(`Неизвестный scope "${options.scope}". Существующие: ${scopes.join(', ')}`);
  }
...
`__fileName__` в имени файла и `<%= className %>` в содержимом — подстановки из объекта, переданного в `generateFiles`; суффикс `.template` отрезается (и защищает файл от компиляции/линта в составе самого плагина).

Шаги 3–4 — запуск. Решение по новому scope: **генератор не создаёт домены молча** — новый домен означает новые правила в boundaries-матрице (глава 06), это осознанное архитектурное действие, а не побочный эффект скаффолдинга. Поэтому сначала `mkdir libs/checkout` (и строка в depConstraints), потом:
...
- Повторный запуск упадёт внутри `libraryGenerator` на конфликте имени проекта — это правильное поведение; идемпотентность генераторам не обещана (глава 01), а "обновление" существующей либы — задача рефакторинга, не скаффолдинга.
...
1. Dry-run и реальный запуск исполняют один и тот же код implementation: все записи идут в виртуальную ФС (Tree), и различие только в последнем шаге — печатать diff или делать flush на диск. Расхождение невозможно по построению: нет отдельной "эмуляции", которая могла бы отстать от реальности.
2. Получаем: project.json с правильной структурой, алиас в tsconfig.base.json, eslint/vite/vitest-конфиги, корректную регистрацию проекта — и всё это продолжает соответствовать текущей версии Nx после каждого обновления, потому что поддерживается командой Nx. Платим: зависимость от сигнатуры `libraryGenerator`, которая может меняться между мажорами, — после `nx migrate` наш генератор нужно прогнать тестом и, возможно, поправить опции.
3. `node_modules/<пакет>/generators.json` — найти имя генератора, взять пути `schema` и `factory`; `schema.json` — сверить опции и их дефолты (часто "не то" — это молчаливый дефолт или дефолт из nx.json); файл из `factory` — прочитать implementation и увидеть фактическую логику. Та же цепочка, что для executors в главе 03, только реестр называется generators.json.
4. Пользователь введёт что угодно: `shoppingCart`, `Shopping-Cart`, `shopping cart`. `names()` даёт канонические формы (fileName `shopping-cart`, className `ShoppingCart`, propertyName `shoppingCart`) — и структура файлов с именами классов остаётся единообразной независимо от того, кто и как вызвал генератор. Без этого конвенция имён умирает на втором пользователе.
5. За генератор: (а) выполняется, а не читается — конвенцию нельзя "забыть"; (б) валидации (наш scope-чек) ловят ошибку в момент создания, а не на ревью; (в) обновление конвенции = обновление кода в одном месте + тест, а не рассылка "перечитайте вики". Честный аргумент за вики: генератор — код, который надо поддерживать (особенно после мажорных обновлений Nx); для конвенции, применяемой дважды в год, эта цена может не окупиться.

</details>

## Частая ошибка

Разработчик, впервые взявшийся за кастомный генератор, пишет его "с нуля": руками через `tree.write` создаёт project.json, vite-конфиг, правит tsconfig.base.json — фактически копирует то, что делает `@nx/react:lib`, в собственный код. Первые месяцы работает; потом `nx migrate` меняет структуру конфигов (скажем, репа переезжает на inferred targets или новый формат eslint), штатный генератор Nx обновляется сам — а самодельный продолжает генерить прошлогоднюю структуру. Правильный инстинкт: ваш генератор — тонкая обёртка (валидации + шаблоны + фиксированные опции) вокруг штатного, вся "инфраструктурная" генерация — только композицией.

Вторая ошибка — генератор-сирота. Его написали на энтузиазме, три месяца им пользовались, потом структура репы уехала (новая ось тегов, другой test-runner), а генератор не обновили — и теперь он создаёт либы, которые тут же падают на lint. Люди делают вывод "генераторы не работают" и возвращаются к копипасте. Лечение дисциплинарное: у генератора есть юнит-тест (значит, он падает в CI, когда отстал), а любой PR, меняющий конвенции репы, обязан менять и генератор — это такой же контракт, как типы в API.
