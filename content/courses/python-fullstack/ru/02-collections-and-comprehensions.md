...
- `set` — неупорядоченный набор **уникальных** значений: `{1, 2, 3}`. Аналог `Set` в JS. Ловушка: `{}` — это **пустой dict**, а не пустой set (синтаксис `{}` был занят словарём раньше исторически); пустой set создаётся только через `set()`.
...
Это мощнее, чем `.slice()` в JS: третий параметр (шаг) и работа с `str` "из коробки" (`"hello"[::-1]` → `"olleh"`) — в JS для разворота строки нужно идти через массив и обратно.
...
**List comprehension vs generator expression — eager vs lazy.** `[x for x in items]` (квадратные скобки) строит весь список в памяти немедленно. `(x for x in items)` (круглые скобки, без отдельного вызова функции) создаёт **генератор** — объект, который производит значения по одному, лениво, по мере итерации, и который можно пройти только один раз:

```python
squares_list = [x * x for x in range(1_000_000)]   # сразу занимает память под весь список
squares_gen = (x * x for x in range(1_000_000))    # почти не занимает память, считает по требованию

sum(x for x in range(1_000_000) if x % 2 == 0)     # генератор передан прямо в sum(), без промежуточного списка
```
...
- List comprehension = `.map()`/`.filter()` в одном выражении; generator expression — как ручной `function*`, но встроен в синтаксис как основная идиома, а не редкий инструмент.
...
Добавляем `priority` каждой задаче (пока просто строка `"low"/"medium"/"high"`, без Enum — тот появится в главе 04 вместе с dataclass) и расширяем `list` двумя флагами: `--status {all,done,pending}` для фильтрации и `--sort {id,priority}` для сортировки. Именно тут естественно применяются list comprehension (фильтрация) и dict comprehension (таблица приоритет → ранг для сортировки).
...
- `PRIORITY_RANK = {name: rank for rank, name in enumerate(PRIORITY_ORDER)}` — dict comprehension строит `{"low": 0, "medium": 1, "high": 2}` из списка один раз при импорте модуля, а не переписывается вручную и не рассинхронизируется с `PRIORITY_ORDER`.
...
- `key=lambda t: (-PRIORITY_RANK[t["priority"]], t["id"])` — сортировка по кортежу решает задачу "два критерия, разное направление" одним вызовом `sorted()`: `sorted()` в Python **устойчив** (stable sort) и сравнивает кортежи поэлементно, а минус перед рангом переворачивает направление только для первого критерия (высокий приоритет — раньше), оставляя `id` по возрастанию.
- Валидация `priority`/`status`/`sort` полностью на стороне `choices=...` в argparse — внутри бизнес-логики нет `if priority not in {...}: raise ...`, потому что невалидное значение сюда физически не может дойти (argparse оборвёт процесс раньше с понятным сообщением).

## Проверь себя

1. Почему список comprehension `[x for x in range(3)]` не "утекает" переменную `x` в окружающую область видимости, а обычный `for x in range(3): pass` — утекает? Ведь оба используют одно и то же ключевое слово `for`.
...
3. В чём разница между `[x for x in items]` и `(x for x in items)` в терминах того, когда вычисляются значения и можно ли пройти результат больше одного раза?
...
1. Comprehension в Python 3 — это фактически неявная функция: у него собственная область видимости, и переменная цикла существует только внутри вычисления comprehension. Обычный оператор `for` — не функция и не создаёт новый scope (см. главу 01: только `def`/`class`/comprehension создают новую область видимости), поэтому его переменная цикла остаётся в области видимости, где `for` был написан, и после цикла видна снаружи.
2. Синтаксис `{}` исторически закреплён за `dict` — словари появились в Python раньше, чем set как отдельный тип с собственным литералом, и `{}` уже означало "пустой словарь" к моменту, когда set получил литеральный синтаксис `{1, 2, 3}`. Менять смысл `{}` задним числом означало бы сломать весь существующий код, поэтому для пустого set пришлось оставить только явный вызов `set()`. Конфликта с непустыми `{1, 2, 3}` нет, потому что наличие `:` внутри (`{"a": 1}`) однозначно отличает dict-литерал от set-литерала на этапе разбора синтаксиса.
3. `[x for x in items]` вычисляет **все** значения сразу и кладёт их в список в памяти — результат можно проходить сколько угодно раз, у него есть длина и индексация. `(x for x in items)` создаёт объект-генератор, который вычисляет следующее значение только когда его действительно запрашивают (например, следующий шаг `for`-цикла или вызов `next()`); после одного полного прохода генератор исчерпан, и повторная итерация не даст ничего — это его фундаментальное отличие от списка.
4. Ключ `dict` должен быть **hashable** — то есть у него должен быть стабильный `__hash__`, не меняющийся, пока объект существует, и это на практике означает "неизменяемый" (или хотя бы не изменяемый теми полями, что участвуют в хеше). `tuple` неизменяем и hashable (если все его элементы тоже hashable). `list` изменяем: если бы список был ключом, а потом кто-то вызвал `.append()` на нём, хеш ключа "поменялся бы прямо в словаре", что сломало бы внутреннюю хеш-таблицу — поэтому Python явно запрещает списки как ключи, бросая `TypeError: unhashable type`.
5. `a = 1`, `b = [2, 3, 4]`, `c = 5`. Механика: интерпретатор сначала резервирует по одному элементу под каждую "обычную" (не звёздную) цель слева и справа от звезды — здесь это `a` (первый элемент) и `c` (последний), — а всё, что остаётся между ними после этого резервирования, целиком уходит в список, привязанный к цели со звёздочкой (`b`). Именно поэтому звезда может стоять где угодно в цели: остаток вычисляется как "то, что не забрали фиксированные позиции по краям".

</details>

## Частая ошибка

Самая частая и самая незаметная ошибка — попытка создать пустой set через `{}`, скопировав привычку из JS, где `{}` — просто "пустой объект", а для набора уникальных значений там и так пришлось бы явно писать `new Set()`. В Python `x = {}` молча создаёт **dict**, а не set, и код падает не сразу, а позже, в неожиданном месте — например, при попытке вызвать `x.add(1)`, потому что у `dict` нет метода `add` (`AttributeError: 'dict' object has no attribute 'add'`). Ошибка проявляется далеко от места, где `{}` был написан, и трассировка не указывает на реальную причину — то, что тип был выбран неверно ещё на этапе создания переменной.

Вторая типичная ошибка — путать eager и lazy при передаче comprehension/generator в функции, которые проходят коллекцию несколько раз. Например, если написать `gen = (t for t in tasks if t["done"])` и затем вызвать `len(gen)` или пройтись по `gen` дважды (сначала для подсчёта, потом для вывода) — первое упадёт с `TypeError: object of type 'generator' has no len()`, второе на второй итерации просто ничего не выведет, без единой ошибки. Если результат нужно проходить больше одного раза или знать его длину — нужен список (`[...]`), а не генератор (`(...)`); генератор оправдан только тогда, когда данные проходятся строго один раз, обычно сразу же и по одному значению за раз.
