...
**Где живёт аутентификация: в зависимости, а не в middleware.** В главе 14 появился middleware для логирования запросов, и он тоже видит каждый входящий запрос. Аутентификация всё равно относится к зависимости. Причина — в том, где каждый из двух стоит в стеке.

Starlette собирает приложение вложенными слоями, и ваш собственный middleware оборачивает роутер. Поэтому middleware выполняется **до** сопоставления маршрута. В этот момент нет ни найденного роута, ни параметров пути, ни разрешённых зависимостей. Middleware пришлось бы заново выводить из сырого пути, каким запросам нужен токен, а потом передавать пользователя дальше через `request.state`.

Тот же стек, снаружи внутрь:

```txt
┌────────────────────────────────────────────────────────────┐
│ ваш middleware (глава 14): видит финальный статус          │
│ ┌────────────────────────────────────────────────────────┐ │
│ │ Starlette: превращает HTTPException в ответ            │ │
│ │ ┌────────────────────────────────────────────────────┐ │ │
│ │ │ роутер: сопоставляет запрос с маршрутом            │ │ │
│ │ │ ┌────────────────────────────────────────────────┐ │ │ │
│ │ │ │ Depends(get_current_user) -> User              │ │ │ │
│ │ │ │ обработчик роута: current_user уже типизирован │ │ │ │
│ │ │ └────────────────────────────────────────────────┘ │ │ │
│ │ └────────────────────────────────────────────────────┘ │ │
│ └────────────────────────────────────────────────────────┘ │
└────────────────────────────────────────────────────────────┘
...
Зависимость же выполняется внутри уже найденного роута. Она возвращает типизированный `User` прямо в сигнатуру обработчика. А `OAuth2PasswordBearer` вдобавок регистрирует схему в OpenAPI, и именно это даёт Swagger UI кнопку "Authorize". Middleware в эту схему не добавляет ничего.

Даже "защитить всё приложение" в FastAPI делается зависимостью, а не middleware. Её передают один раз наверху: `FastAPI(dependencies=[Depends(get_current_user)])`. Для одной группы роутов это `APIRouter(dependencies=[...])`. В этом проекте `Depends(get_current_user)` остаётся на каждом task-эндпоинте, потому что `/auth/register` и `/auth/token` обязаны быть открытыми.

Middleware из главы 14 остаётся ровно таким, каким был, а аутентификация меняет то, что он пишет в лог. Запрос без токена или с невалидным токеном до роута не доходит: `get_current_user` бросает `HTTPException`.

Слой Starlette, который превращает это в ответ, стоит **внутри** вашего middleware. Поэтому в логе окажется обычный `401`, в той же форме, что и в главе 14. Статус уже сконвертирован к моменту, когда ответ идёт обратно через ваш код.
...
# голый API -- ещё не полный: про лимит в 72 байта сразу ниже
...
Одно ограничение этого API легко пропустить, и это настоящая дыра в авторизации. Алгоритм `bcrypt` читает не больше **72 байт** пароля. Всё, что идёт после 72-го байта, игнорируется. Поэтому два разных длинных пароля с одинаковыми первыми 72 байтами оказываются взаимозаменяемыми. Каждый из них проходит проверку против хеша другого.

Обратите внимание: 72 — это счёт **байтов**, а не символов. Пароль из кириллицы или эмодзи упирается в лимит гораздо раньше латинского. В `utf-8` такой символ занимает от двух до четырёх байтов.

Версии библиотеки говорят об этом по-разному. До версии `bcrypt` 4.3.0 включительно пароль обрезался молча — так же, как в оригинальной реализации OpenBSD. Версия 5.0.0, вышедшая в сентябре 2025 года, вместо этого бросает `ValueError`. Опасен именно первый вариант, поэтому не полагайтесь на то, что установленная версия поймает эту ошибку за вас.

Решение, описанное в документации самой библиотеки, — предварительно хешировать пароль, прежде чем его увидит `bcrypt`. Сначала `sha256`, затем base64 от полученного дайджеста. Шаг с base64 важен, потому что он убирает нулевые байты. На нулевом байте `bcrypt` перестал бы читать пароль раньше времени. Дайджест `sha256` — это 32 байта, его base64-форма — 44 байта, поэтому результат всегда укладывается в лимит:
...
Предварительное хеширование обязано применяться с обеих сторон. Забудьте про него в `verify_password` — и ни один длинный пароль больше не пройдёт проверку.
...
- В Express роуты защищают middleware — `app.use(requireAuth)` или обработчиком на отдельном роуте — и вешают пользователя на `req.user`. В FastAPI вместо этого зависимость, и пользователь приходит типизированным аргументом. Nest.js здесь ближе к FastAPI: аутентификация живёт в guard, а не в middleware.
...
1. Добавьте `pyjwt`, `bcrypt` и `python-multipart>=0.0.18` в `dependencies`. Нижняя граница здесь не косметическая. У версий ниже 0.0.18 есть уязвимость к отказу в обслуживании, `CVE-2024-53981`, ровно в разборе multipart, на который опирается `/auth/token`.
...
6. Создайте `auth/security.py` (`hash_password`, `verify_password` через `bcrypt` напрямую — без `passlib`; `create_access_token`, `decode_access_token` через `PyJWT`) и `auth/dependencies.py` (`OAuth2PasswordBearer(tokenUrl="auth/token")`, `get_current_user`). В обеих функциях хеширования предварительно хешируйте пароль через `sha256` и base64, чтобы лимит `bcrypt` в 72 байта не мог его обрезать. Добавьте и `auth/__init__.py`, ре-экспортирующий через `__all__` пять публичных имён пакета.
...
`src/taskman/auth/__init__.py` (новый файл):
...
- Пароль предварительно хешируется через `sha256` и base64, прежде чем его увидит `bcrypt`. Без этого шага `bcrypt` читает только первые 72 байта пароля. Тогда два разных длинных пароля с одинаковым началом принимали бы хеш друг друга. Новый `bcrypt` бросает здесь `ValueError`, старый 4.x обрезал молча, а один и тот же код остаётся безопасным на обеих ветках.
...
- Пакет `auth/` получает `__init__.py`, который ре-экспортирует его пять публичных имён. Именно поэтому дальше в проекте написано `from ..auth import get_current_user`, а не `from ..auth.dependencies import get_current_user`. Вызывающему коду не нужно знать, в каком подмодуле лежит имя, а `security.py` можно будет разделить позже, не трогая эти импорты. Список `__all__` вдобавок делает ре-экспорт явным для `mypy --strict`, как описано в главе 05. Объект `oauth2_scheme` в `__all__` не попал намеренно: за пределами `auth/` он никому не нужен.
...
6. Middleware тоже видит каждый входящий запрос, но `get_current_user` сделан зависимостью. К чему у зависимости есть доступ, которого нет у middleware?
...
6. Зависимость выполняется внутри найденного роута, а middleware — снаружи. Starlette оборачивает роутер в ваш собственный middleware, поэтому middleware исполняется до сопоставления маршрута. Значит, у него нет ни найденного роута, ни параметров пути, ни разрешённых зависимостей. У зависимости есть всё три. Она вдобавок возвращает типизированный `User` в сигнатуру обработчика и наполняет схему OpenAPI, а именно из неё берётся кнопка "Authorize" в Swagger UI. Middleware, конечно, тоже может прочитать заголовок `Authorization` и разобрать токен руками. Заново изобретать ему придётся другое: сопоставление сырого пути с ответом на вопрос "нужен ли этому запросу токен", плюс способ передать объект пользователя дальше. Ровно это сопоставление `Depends` уже умеет.
