...
`__all__` — отдельная тема, и он ничего не прячет. Если написать имя явно, приватное имя всё равно импортируется: `from module import _private_name` сработает. В рантайме `__all__` ограничивает только то, что попадёт при `from module import *`. Ещё он служит подсказкой инструментам документации и IDE (среда разработки) о том, что считается официальным публичным API модуля.

А вот для инструмента, проверяющего типы, `__all__` в `__init__.py` значит гораздо больше. Строка `from .task import Task` внутри `__init__.py` — это **неявный** ре-экспорт. При включённом `no_implicit_reexport`, а его включает `mypy --strict`, такой импорт из пакета уже не проходит проверку. Сообщение будет `Module "taskman.models" does not explicitly export attribute "Task"`, код ошибки — `attr-defined`.

Сделать ре-экспорт явным можно двумя способами. Первый — перечислить имя в `__all__`, как в примере выше. Второй — написать выглядящее избыточным `from .task import Task as Task`. В этом курсе `__all__` есть в каждом `__init__.py`, поэтому строгому прогону mypy из главы 10 к этим импортам сказать нечего.
...
- Явный `export` в JS/TS против "всё публично по умолчанию" в Python. Список `__all__` и подчёркивание перед именем — это соглашение для читателя и для `import *`, а не приватность, которую обеспечивает язык. Под `mypy --strict` тот же `__all__` вдобавок решает, что пакет ре-экспортирует, и это уже ближе к настоящему списку `export`.
