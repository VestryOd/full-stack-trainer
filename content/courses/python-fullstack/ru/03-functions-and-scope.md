...
Ключевое отличие от JS — что происходит при **присваивании** имени внутри вложенной функции. В Python интерпретатор на этапе разбора функции целиком решает: если имени есть присваивание где-либо в теле функции — это имя **локальное для всей функции**, с самой первой строки, даже если присваивание физически ниже:
...
`counter += 1` — это присваивание, значит `counter` считается локальной переменной во всей `increment()`. Но `+=` сначала **читает** текущее значение `counter`, чтобы прибавить 1 — а локальная `counter` на этот момент ещё не существует. Отсюда `UnboundLocalError: local variable 'counter' referenced before assignment`. Чтобы явно сказать "это не новая локальная переменная, а изменение внешней" — нужны ключевые слова `global` (для переменной модуля) или `nonlocal` (для переменной внешней функции при замыкании):
...
В JS такой проблемы вообще не возникает: `let`/`const` объявляют переменную явно один раз при декларации, и любое последующее `counter += 1` внутри вложенной функции однозначно понимается как присваивание уже существующей внешней переменной — двусмысленности "новая локальная или изменение внешней" в JS нет, потому что в JS всегда есть явное ключевое слово объявления (`let`/`const`/`var`), а в Python объявления как отдельного действия нет вообще — есть только присваивание, и compiler вынужден определять "локальность" имени эвристикой по всему телу функции.
...
`@shout` над `def greet` — это ровно то же самое, что написать `greet = shout(greet)` после определения функции; никакого отдельного языкового механизма, кроме сахара для этой строчки, нет. Побочный эффект: без дополнительных мер `greet.__name__` после декорирования будет `"wrapper"`, а не `"greet"` — это чинится декоратором `functools.wraps(func)`, применённым к `wrapper`, который копирует `__name__`, `__doc__` и другие метаданные с исходной функции. `functools` подробно разберём в главе про itertools/functools — здесь это первое, минимально необходимое знакомство.
...
- Присваивание переменной из внешней области видимости внутри вложенной функции требует явного `nonlocal`/`global` — в JS переменные `let`/`const` из внешнего замыкания переприсваиваются без специального синтаксиса, потому что JS всегда явно объявляет переменную один раз через `let`/`const`/`var`.

## Что добавляем в проект

Выносим три ветки `if/elif` в `main()` в отдельные функции-обработчики (`handle_add`, `handle_list`, `handle_done`) и оборачиваем каждую декоратором `@log_command`, который печатает в stderr, какая команда запускается и когда она завершилась — это первый шаг к нормальному логированию CLI (полноценное structured logging — уже за рамками курса, но идиома "декоратор на все команды" — типовой паттерн для CLI-инструментов). Заодно диспетчеризация команд переезжает с `if/elif` на словарь `{имя команды: функция-обработчик}` — прямая иллюстрация того, что функции в Python такие же значения, как строки или числа.
...
- `wrapper(*args, **kwargs)` вместо `wrapper(args)` — декоратор не делает предположений о сигнатуре обёрнутой функции; `namespace = args[0]` достаёт первый позиционный аргумент (у нас это всегда `argparse.Namespace`), но сам декоратор остаётся универсальным и переиспользуемым для функций с другой сигнатурой.
- `@functools.wraps(func)` над `wrapper` — без этой строки `handle_add.__name__` стало бы `"wrapper"` после декорирования, что сломало бы отладку (трейсбеки, `help()`, интроспекцию) и любой код, который полагается на имя функции.
...
1. Default-значение параметра в Python вычисляется **один раз**, в момент выполнения строки `def add_item(...)` (то есть при импорте модуля), а не заново при каждом вызове функции. Это значит, что все вызовы `add_item(x)` без явного `bucket=...` получают **один и тот же объект списка** — если один вызов мутирует его (`.append()`), это изменение видно во всех последующих вызовах, потому что это физически один и тот же объект в памяти, а не новый список "с нуля" для каждого вызова.
2. Python определяет "локальность" имени статически, разбирая **всё тело функции целиком** до выполнения: если где-либо в теле есть присваивание имени (а `counter += 1` — это присваивание, эквивалентное `counter = counter + 1`), это имя считается локальным для всей функции, с первой её строки. Поэтому попытка прочитать `counter` для вычисления `counter + 1` видит "ещё не инициализированную локальную переменную", а не переменную модуля — отсюда `UnboundLocalError`. В JS такой двусмысленности нет, потому что переменная явно объявляется один раз через `let`/`const`, и любое последующее использование в замыкании однозначно ссылается на эту конкретную объявленную переменную.
3. `global` говорит интерпретатору: "это имя — переменная модуля (глобальная), а не новая локальная — при присваивании изменяй именно её". `nonlocal` — то же самое, но для переменной **ближайшей внешней функции** (в замыкании), а не модуля. `nonlocal` нужен именно тогда, когда вложенная функция должна изменить переменную из объемлющей функции (как `count` в `make_counter`), а `global` не подойдёт, потому что искомая переменная — не на уровне модуля, а на уровне другой функции.
...
5. Без `functools.wraps(func)` `wrapper` (со своим собственным `__name__ == "wrapper"`, пустым `__doc__` и т.д.) становится тем объектом, который в итоге привязывается к имени декорированной функции. На практике это ломает: отладочные трейсбеки (в них будет фигурировать "wrapper", а не реальное имя функции, что усложняет чтение стектрейса), `help(func)`/интроспекцию в IDE (докстринг и сигнатура окажутся от `wrapper`, а не от исходной функции), и любой код, который явно проверяет `func.__name__` (например, некоторые тестовые фреймворки или системы роутинга, сопоставляющие обработчики по имени).

</details>

## Частая ошибка

Самая известная ловушка Python для новичков любого бэкграунда — мутабельный default-параметр (`def f(items=[]):`), но для JS-разработчика она особенно коварна, потому что интуиция работает ровно наоборот. В JS `function f(items = []) {}` создаёт **новый** пустой массив при каждом вызове, если `items` не передан явно — и JS-разработчик автоматически переносит эту привычку на Python, ожидая такого же поведения. В реальности Python создаёт список `[]` один раз, при определении функции, и делится этим одним объектом между всеми "неявными" вызовами. Баг обычно не проявляется сразу: код прекрасно работает в тестах, где функция вызывается один раз, и начинает "необъяснимо накапливать данные из предыдущих вызовов" именно в продакшене или в интеграционных тестах, где функция вызывается многократно за время жизни процесса — то есть ровно тогда, когда сложнее всего сопоставить симптом с причиной.

Второй частый момент — попытка присвоить значение переменной из объемлющей функции внутри замыкания без `nonlocal`, скопировав JS-рефлекс "просто переприсвой переменную из внешнего скоупа, замыкание же". В Python это не ошибка компиляции, а `UnboundLocalError` в рантайме, причём сообщение об ошибке (`local variable referenced before assignment`) не подсказывает напрямую, что решение — добавить `nonlocal`, если не знать про эту механику заранее.
