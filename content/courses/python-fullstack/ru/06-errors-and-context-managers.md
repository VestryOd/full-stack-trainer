...
**Группы исключений и `except*`.** В Python 3.11 появился способ бросить несколько исключений сразу. Класс `ExceptionGroup` оборачивает список исключений, а новый блок `except*` разбирает их внутри группы по типу.
...
Оба блока `except*` здесь выполняются, и в этом весь смысл. Обычный `except` выбирает ровно одну ветку, а `except*` обрабатывает каждую подходящую подгруппу одной и той же группы. Содержимое группы доступно кортежем `.exceptions`, а разобрать её на части руками помогают методы `.subgroup(condition)` и `.split(condition)`.

Разделение по `BaseException` из абзацев выше повторяется здесь на уровень выше. `ExceptionGroup` наследуется от `Exception`, поэтому `except Exception:` его поймает. `BaseExceptionGroup` наследуется от `BaseException` и может обернуть вообще что угодно, включая `KeyboardInterrupt`. Поэтому его `except Exception:` уже не поймает, и ровно для этого сделаны два класса, а не один.

Выбирать между ними обычно не приходится. Конструктор `BaseExceptionGroup` возвращает `ExceptionGroup`, если каждое вложенное исключение — экземпляр `Exception`. Конструктор `ExceptionGroup` строже: он бросает `TypeError`, если хотя бы одно вложенное исключение не наследник `Exception`.

На практике с группами сталкиваются через `asyncio.TaskGroup`, тоже новинку 3.11. Когда в одной группе падает несколько задач, их исключения собираются в группу и бросаются вместе. Падение одной задачи больше не скрывает остальные. Из этого правила есть одно исключение. `KeyboardInterrupt` или `SystemExit` внутри задачи перебрасывается сам собой, а не внутри группы.

Этот курс использует `asyncio.gather` (глава 12), а не `TaskGroup`. Функция `gather` появилась раньше групп. Она бросает первое исключение либо, с `return_exceptions=True`, собирает их в обычный список.
...
**`fcntl` работает только под Unix.** Стандартная библиотека объявляет `fcntl` доступным на Unix, поэтому под Windows `import fcntl` падает с `ModuleNotFoundError`. У Windows есть свой вызов в стандартной библиотеке: `msvcrt.locking(fd, mode, nbytes)`. Передайте `msvcrt.LK_LOCK` для блокирующего захвата и `msvcrt.LK_UNLCK` для освобождения.

Взаимозаменяемыми эти два вызова не назвать. Вызов `flock` блокирует файл целиком, а `msvcrt.locking` — только диапазон байтов от текущей позиции в файле.

Если вы работаете под Windows, рабочих путей два. Выполните задание внутри Windows Subsystem for Linux или в Linux-контейнере: остальной курс всё равно исходит из этого. Либо замените два вызова `fcntl` на пакет `portalocker` — сторонняя обёртка, которая скрывает это платформенное расхождение за одним API.
...
- `ExceptionGroup` вместе с `except*` (Python 3.11) ~ `AggregateError` в JS, которым `Promise.any()` отклоняется, когда отклонились все промисы. И там, и там внутри лежит список ошибок, а не одна. Синтаксиса вроде `except*` в JS нет, поэтому по `error.errors` приходится проходить вручную.
...
4. Создайте `logging_utils.py` с двумя вещами. Первая — класс `FileLock`, реализующий `__enter__`/`__exit__` вокруг `fcntl.flock` (эксклюзивная блокировка на запись). Вторая — функция `append_log(message: str) -> None`, использующая `FileLock` через `with`. Под Windows используйте вместо `fcntl` вызов `msvcrt.locking` или пакет `portalocker` — так, как описано в секции теории.
...
6. В Python 3.11 появились сразу два класса: `ExceptionGroup` и `BaseExceptionGroup`. Зачем два вместо одного, и какой из них поймает `except Exception:`?
...
6. `except Exception:` поймает `ExceptionGroup` и не поймает `BaseExceptionGroup`. Ровно для этого и сделаны два класса. `ExceptionGroup` наследуется от `Exception`, а `BaseExceptionGroup` — от `BaseException`. Это разделение повторяет логику вопроса 2 на уровень выше. Группа, обернувшая `KeyboardInterrupt`, не должна проглатываться широким `except Exception:` — так же, как не должен проглатываться одиночный `KeyboardInterrupt`. Обернуть что-то, что вообще не является `Exception`, разрешено только `BaseExceptionGroup`. Конструктор `ExceptionGroup` на такой попытке бросает `TypeError`. В обратную сторону выбор делается за вас: конструктор `BaseExceptionGroup` возвращает `ExceptionGroup`, когда всё вложенное — экземпляры `Exception`.
