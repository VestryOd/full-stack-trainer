export const translations = {
  nav: {
    theory:    { en: 'Theory',         ru: 'Теория' },
    questions: { en: 'Questions',      ru: 'Вопросы' },
    quiz:      { en: 'Quiz',           ru: 'Квиз' },
    tasks:     { en: 'Tasks',          ru: 'Задачи' },
    interview: { en: 'Interview',      ru: 'Интервью' },
    courses:   { en: 'Courses',        ru: 'Курсы' },
  },
  home: {
    subtitle: {
      en: 'Structured interview preparation for senior fullstack developers. Theory, questions, quizzes, and coding tasks — all in one place.',
      ru: 'Структурированная подготовка к интервью для senior fullstack разработчиков. Теория, вопросы, квизы и задачи — всё в одном месте.',
    },
    sections:       { en: 'Sections',       ru: 'Разделы' },
    topics:         { en: 'Topics',         ru: 'Темы' },
    courses:        { en: 'Courses',        ru: 'Курсы' },
    coverageLevels: { en: 'Coverage Levels', ru: 'Уровни покрытия' },
    btnQuiz:        { en: 'Take a Quiz',    ru: 'Пройти квиз' },
    btnTheory:      { en: 'Start with Theory', ru: 'Начать с теории' },
    theoryDesc: {
      en: 'In-depth articles on each topic with code examples.',
      ru: 'Подробные статьи по каждой теме с примерами кода.',
    },
    questionsDesc: {
      en: 'Interview questions with hidden answers — Junior to Advanced.',
      ru: 'Вопросы для интервью со скрытыми ответами — от Junior до Advanced.',
    },
    quizDesc: {
      en: 'Test yourself with multiple-choice quizzes and instant feedback.',
      ru: 'Проверь себя с помощью квизов с мгновенной обратной связью.',
    },
    tasksDesc: {
      en: 'Coding challenges with starter code and hidden solutions.',
      ru: 'Задачи по программированию со стартовым кодом и скрытыми решениями.',
    },
    interviewDesc: {
      en: 'Simulate a real interview — timed session with questions and coding tasks.',
      ru: 'Симулируй настоящее интервью — сессия с таймером, вопросами и задачами.',
    },
    coursesDesc: {
      en: 'Multi-chapter courses that build one project step by step, chapter by chapter.',
      ru: 'Многоглавые курсы, где один проект растёт шаг за шагом, от главы к главе.',
    },
    deepDesc: {
      en: '5 topics — comprehensive coverage with advanced questions',
      ru: '5 тем — исчерпывающее покрытие с продвинутыми вопросами',
    },
    mediumDesc: {
      en: '15 topics — solid fundamentals and common interview areas',
      ru: '15 тем — крепкие основы и типичные темы интервью',
    },
    lightDesc: {
      en: '7 topics — key concepts and essential knowledge',
      ru: '7 тем — ключевые концепции и базовые знания',
    },
  },
  footer: {
    tagline: {
      en: 'Interview preparation for senior fullstack developers',
      ru: 'Подготовка к интервью для senior fullstack разработчиков',
    },
  },
  theory: {
    title:        { en: 'Theory',        ru: 'Теория' },
    available:    { en: 'topics available', ru: 'тем доступно' },
    articles:     { en: 'articles',      ru: 'статей' },
    comingSoon:   { en: 'Coming soon',   ru: 'Скоро' },
    minRead:      { en: 'min',           ru: 'мин' },
    noArticlesYet: { en: 'No articles yet', ru: 'Пока нет статей' },
    comingSoonTopic: { en: 'Content for this topic is coming soon.', ru: 'Материалы по этой теме появятся позже.' },
  },
  courses: {
    title:        { en: 'Courses',       ru: 'Курсы' },
    available:    { en: 'courses available', ru: 'курсов доступно' },
    chapters:     { en: 'chapters',      ru: 'глав' },
    comingSoon:   { en: 'Coming soon',   ru: 'Скоро' },
    minRead:      { en: 'min',           ru: 'мин' },
    noChaptersYet: { en: 'No chapters yet', ru: 'Пока нет глав' },
    comingSoonCourse: { en: 'Content for this course is coming soon.', ru: 'Материалы этого курса появятся позже.' },
  },
  questions: {
    title:       { en: 'Questions',          ru: 'Вопросы' },
    subtitle:    { en: 'interview questions across', ru: 'вопросов для интервью по' },
    topicsWord:  { en: 'topics',             ru: 'темам' },
    questionsWord: { en: 'questions',        ru: 'вопросов' },
    search:      { en: 'Search questions...', ru: 'Поиск вопросов...' },
    markReviewed: { en: 'Mark as reviewed',  ru: 'Отметить изученным' },
    reviewed:    { en: 'Reviewed',           ru: 'Изучено' },
    progress:    { en: 'questions in',       ru: 'вопросов по' },
    reset:       { en: 'reset',              ru: 'сбросить' },
    tags:        { en: 'Tags',               ru: 'Теги' },
    showTags:    { en: 'Show tags',          ru: 'Показать теги' },
    hideTags:    { en: 'Hide tags',          ru: 'Скрыть теги' },
    showAnswer:  { en: 'Show answer',        ru: 'Показать ответ' },
    hideAnswer:  { en: 'Hide answer',        ru: 'Скрыть ответ' },
    noQuestionsYet: { en: 'No questions yet for this topic.', ru: 'Пока нет вопросов по этой теме.' },
  },
  quiz: {
    title:       { en: 'Quiz',               ru: 'Квиз' },
    subtitle:    { en: 'Test your knowledge with multiple-choice questions', ru: 'Проверь свои знания с помощью вопросов с выбором ответа' },
    startBtn:    { en: 'Start Quiz',         ru: 'Начать квиз' },
    questionOf:  { en: 'Question',           ru: 'Вопрос' },
    of:          { en: 'of',                 ru: 'из' },
    correct:     { en: 'Correct',            ru: 'Правильно' },
    explanation: { en: 'Explanation',        ru: 'Объяснение' },
    seeResults:  { en: 'See Results',        ru: 'Результаты' },
    nextQuestion: { en: 'Next Question',     ru: 'Следующий вопрос' },
    yourScore:   { en: 'You answered {correct} out of {total} questions correctly.', ru: 'Вы ответили правильно на {correct} из {total} вопросов.' },
    newQuiz:     { en: 'New Quiz',           ru: 'Новый квиз' },
    noQuestions:       { en: 'No quiz questions available for this topic yet.', ru: 'Для этой темы пока нет вопросов квиза.' },
    questions:         { en: 'questions',          ru: 'вопросов' },
    clickToStart:      { en: 'Click to configure', ru: 'Нажмите для настройки' },
    questionsAvailable: { en: 'questions available', ru: 'вопросов доступно' },
    selectCount:       { en: 'Select number of questions', ru: 'Выберите количество вопросов' },
    all:               { en: 'All',                ru: 'Все' },
    customCount:       { en: 'Or enter a custom amount', ru: 'Или введите произвольное количество' },
    randomTitle:       { en: 'Random Quiz',                           ru: 'Случайный квиз' },
    randomSubtitle:    { en: 'Mix questions from multiple topics',    ru: 'Вопросы из нескольких тем' },
    randomTopics:      { en: 'Select topics',                         ru: 'Выберите темы' },
    randomSelectAll:   { en: 'Select All',                            ru: 'Выбрать все' },
    randomDeselectAll: { en: 'Deselect All',                          ru: 'Снять все' },
    randomMixed:       { en: 'Mixed',                                 ru: 'Смешанный' },
    randomLoading:     { en: 'Preparing questions…',                  ru: 'Подготовка вопросов…' },
    randomNoSession:   { en: 'No quiz configuration found.',          ru: 'Конфигурация квиза не найдена.' },
  },
  tasks: {
    title:       { en: 'Tasks',              ru: 'Задачи' },
    subtitle:    { en: 'Coding challenges —', ru: 'Задачи по программированию —' },
    tasksWord:   { en: 'tasks across',       ru: 'задач по' },
    topicsWord:  { en: 'topics',             ru: 'темам' },
    tasksCount:  { en: 'tasks',              ru: 'задач' },
    showSolution: { en: 'Show Solution',     ru: 'Показать решение' },
    hideSolution: { en: 'Hide Solution',     ru: 'Скрыть решение' },
    starterCode: { en: 'Starter Code',       ru: 'Стартовый код' },
    revealSolution: { en: 'Reveal solution', ru: 'Открыть решение' },
    copy:        { en: 'Copy',               ru: 'Копировать' },
    copied:      { en: 'Copied!',            ru: 'Скопировано!' },
    backToTasks: { en: '← Tasks',            ru: '← Задачи' },
    noTasksYet:  { en: 'No tasks for this topic yet.', ru: 'Пока нет задач для этой темы.' },
    randomTitle:    { en: 'Random Tasks',    ru: 'Случайные задачи' },
    randomSubtitle: { en: 'Draw a set from the topics you pick.', ru: 'Собрать набор из выбранных тем.' },
    randomTopics:   { en: 'Topics',          ru: 'Темы' },
    randomSelectAll:   { en: 'Select all',   ru: 'Выбрать все' },
    randomDeselectAll: { en: 'Clear',        ru: 'Снять все' },
    selectCount:    { en: 'How many tasks',  ru: 'Сколько задач' },
    all:            { en: 'All',             ru: 'Все' },
    tasksAvailable: { en: 'tasks available', ru: 'задач доступно' },
    startBtn:       { en: 'Start',           ru: 'Начать' },
    randomOne:      { en: 'Random task',     ru: 'Случайная задача' },
    another:        { en: 'Another',         ru: 'Другая' },
    picking:        { en: 'Picking a task…', ru: 'Выбираем задачу…' },
    randomNoSet:    { en: 'No task set is running. Start one from the Tasks page.', ru: 'Набор задач не запущен. Начните его со страницы задач.' },
    setBadge:       { en: 'Random set',      ru: 'Случайный набор' },
    prev:           { en: '← Previous',      ru: '← Назад' },
    next:           { en: 'Next →',          ru: 'Дальше →' },
    exitSet:        { en: 'Exit set',        ru: 'Выйти из набора' },
    setDone:        { en: 'That was the last task in the set.', ru: 'Это была последняя задача набора.' },
    whatsNext:      { en: 'What next',       ru: 'Что дальше' },
    anotherInTopic: { en: 'Another task on this topic', ru: 'Ещё задача по этой теме' },
    anyRandom:      { en: 'Random task from any topic',  ru: 'Случайная задача из всех тем' },
  },
  interview: {
    title:            { en: 'Mock Interview',      ru: 'Симуляция интервью' },
    subtitle:         { en: 'Simulates a real interview: questions from easy to hard, followed by coding tasks.', ru: 'Симулирует настоящее интервью: вопросы от простого к сложному, затем задачи на программирование.' },
    duration:         { en: 'Duration',            ru: 'Продолжительность' },
    scope:            { en: 'Scope',               ru: 'Охват тем' },
    fullInterview:    { en: 'Full Interview',       ru: 'Полное интервью' },
    fullInterviewDesc: { en: 'All available topics for questions and tasks', ru: 'Все доступные темы для вопросов и задач' },
    custom:           { en: 'Custom',              ru: 'Настроить' },
    customDesc:       { en: 'Choose specific topics', ru: 'Выбрать определённые темы' },
    questionTopics:   { en: 'Question Topics',     ru: 'Темы вопросов' },
    taskTopics:       { en: 'Task Topics',         ru: 'Темы задач' },
    selectAll:        { en: 'Select All',          ru: 'Выбрать все' },
    deselectAll:      { en: 'Deselect All',        ru: 'Снять все' },
    startBtn:         { en: 'Start Interview',     ru: 'Начать интервью' },
    estimateLabel:    { en: 'Estimated session',   ru: 'Оценка сессии' },
    questions:        { en: 'questions',           ru: 'вопросов' },
    tasks:            { en: 'tasks',               ru: 'задач' },
    min:              { en: 'min',                 ru: 'мин' },
    itemOf:           { en: 'Item',                ru: 'Вопрос' },
    of:               { en: 'of',                  ru: 'из' },
    elapsed:          { en: 'Elapsed',             ru: 'Прошло' },
    endInterview:     { en: 'End Interview',       ru: 'Завершить' },
    endConfirmTitle:  { en: 'End interview early?', ru: 'Завершить интервью досрочно?' },
    endConfirmDesc:   { en: 'Your progress will be lost.', ru: 'Прогресс будет потерян.' },
    endConfirmYes:    { en: 'End Interview',       ru: 'Завершить' },
    endConfirmNo:     { en: 'Continue',            ru: 'Продолжить' },
    next:             { en: 'Next',                ru: 'Далее' },
    skip:             { en: 'Skip',                ru: 'Пропустить' },
    finish:           { en: 'Finish Interview',    ru: 'Завершить интервью' },
    showAnswer:       { en: 'Show Answer',         ru: 'Показать ответ' },
    hideAnswer:       { en: 'Hide Answer',         ru: 'Скрыть ответ' },
    summaryTitle:     { en: 'Interview Complete',  ru: 'Интервью завершено' },
    timeTaken:        { en: 'Time taken',          ru: 'Затрачено' },
    plannedDuration:  { en: 'Planned',             ru: 'Запланировано' },
    questionsReviewed: { en: 'Questions reviewed', ru: 'Вопросов просмотрено' },
    tasksAttempted:   { en: 'Tasks attempted',     ru: 'Задач просмотрено' },
    topicsCovered:    { en: 'Topics covered',      ru: 'Пройденные темы' },
    startNew:         { en: 'Start New Interview', ru: 'Новое интервью' },
    backHome:         { en: 'Back to Home',        ru: 'На главную' },
    noSession:        { en: 'No active session. Please start a new interview.', ru: 'Нет активной сессии. Начните новое интервью.' },
    taskLabel:        { en: 'Task',                ru: 'Задача' },
  },
  notFound: {
    title:    { en: 'Page not found',                      ru: 'Страница не найдена' },
    subtitle: { en: "The page you're looking for doesn't exist or has been moved.", ru: 'Страница, которую вы ищете, не существует или была перемещена.' },
    backHome: { en: '← Back to home',                      ru: '← На главную' },
  },
  error: {
    title:    { en: 'Something went wrong',                ru: 'Что-то пошло не так' },
    subtitle: { en: 'An unexpected error occurred while rendering this page.', ru: 'Произошла непредвиденная ошибка при отображении страницы.' },
    retry:    { en: 'Try again',                           ru: 'Попробовать снова' },
    backHome: { en: '← Back to home',                      ru: '← На главную' },
  },
  search: {
    title:       { en: 'Search',                            ru: 'Поиск' },
    placeholder: { en: 'Search theory, questions, tasks…',  ru: 'Поиск по теории, вопросам, задачам…' },
    hintType:    { en: 'Type at least 2 characters to search', ru: 'Введите минимум 2 символа для поиска' },
    empty:       { en: 'No results for',                    ru: 'Ничего не найдено по' },
    error:       { en: 'Could not load the search index.',  ru: 'Не удалось загрузить поисковый индекс.' },
    refine:      { en: 'Refine your query to see more',     ru: 'Уточните запрос, чтобы увидеть больше' },
    hintEnter:   { en: 'to open',                           ru: 'открыть' },
    hintNav:     { en: 'navigate',                          ru: 'навигация' },
    all:         { en: 'All',                               ru: 'Все' },
    recent:      { en: 'Recent',                            ru: 'Недавние' },
    clearRecent: { en: 'Clear',                             ru: 'Очистить' },
    type: {
      theory:   { en: 'Theory',    ru: 'Теория' },
      question: { en: 'Questions', ru: 'Вопросы' },
      task:     { en: 'Tasks',     ru: 'Задачи' },
      course:   { en: 'Courses',   ru: 'Курсы' },
    },
  },
} as const;

export type TranslationKey = typeof translations;
